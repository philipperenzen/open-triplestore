...
fn property_constraint(input: &str) -> IResult<&str, PropertyDecl> {
    ...
    loop {
        ...
        if let Ok((rest, _)) = tag_no_case::<&str, &str, nom::error::Error<&str>>("or")(input_tmp) {
            let (rest, _) = multispace0(rest)?;
...
fn shape_decl(input: &str) -> IResult<&str, ShapeDecl> {
    ...
    let (input, target) = opt(preceded(pair(tag("->"), multispace0), iri_or_prefixed))(input)?;
