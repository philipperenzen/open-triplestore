...
use super::merge;
use super::models::{
    CreateDraftRequest, CreateDataModelRequest, DiffParams, PatchVersionRequest,
...
pub async fn list_data_models(
    ...
    let filtered: Vec<_> = records
        ...
        .filter(|o| {
            state
                .auth_db
                .can_access_ontology(uid, o.is_public, o.owner_type.as_deref(), o.owner_id.as_deref())
                .unwrap_or(false)
        })
        .collect();
    Ok(Json(filtered))
...
pub async fn get_data_model(
    ...
    let uid = user.as_deref().map(|u| u.user_id.as_str());
    if !state.auth_db.can_access_ontology(uid, record.is_public, record.owner_type.as_deref(), record.owner_id.as_deref())
...
pub async fn delete_data_model(
    ...
    state.store.bulk_delete_graphs(&iri_refs).map_err(AppError::from)?;
...
pub async fn update_data_model(
    ...
    if !state
        .auth_db
        .can_write_ontology(&user.user_id, existing.owner_type.as_deref(), existing.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::Unauthorized(
...
pub async fn list_versions(
    ...
    let ontology = registry::get_data_model(&state.store, &state.base_url, &id)
        .ok_or_else(|| AppError::NotFound(format!("Data model '{id}' not found")))?;
    let uid = user.as_deref().map(|u| u.user_id.as_str());
    if !state.auth_db.can_access_ontology(uid, ontology.is_public, ontology.owner_type.as_deref(), ontology.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::NotFound(format!("Data model '{id}' not found")));
    }
...
pub async fn list_collaborators(
    ...
    if !state
        .auth_db
        .can_access_ontology(uid, record.is_public, record.owner_type.as_deref(), record.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::NotFound(format!("Data model '{id}' not found")));
    }
...
pub async fn update_version_notes(
    ...
    if !state
        .auth_db
        .can_write_ontology(&user.user_id, parent.owner_type.as_deref(), parent.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::Unauthorized(
            "Write access to this data model required".to_string(),
...
pub async fn get_version_data(
    ...
    let dump_format = if graphs_to_dump.len() == 1
        && matches!(format, RdfFormat::Turtle | RdfFormat::NTriples)
    {
        format
    ...
    let etag = format!("\"{}\"", version_revision(&state.store, &version_graphs(&record)));
...
pub async fn upload_version(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| {
        ...
        match name.as_str() {
            ...
            "kind" => {
                let k = field.text().await.map_err(|e| AppError::BadRequest(e.to_string()))?;
            ...
            "notes" => {
                let n = field.text().await.map_err(|e| AppError::BadRequest(e.to_string()))?;
            ...
            "message" => {
                let m = field.text().await.map_err(|e| AppError::BadRequest(e.to_string()))?;
    ...
    let (result, detected) = tokio::task::spawn_blocking(move || {
        ...
        if let Some(ref kind_str) = kind_override {
            crate::kind_detector::parse_kind_override(kind_str)
                .ok_or_else(|| format!("Invalid kind override: '{}'. Expected: data-model, vocabulary", kind_str))?;
        }
    ...
    let record = DataModelVersion {
        ...
        status: if is_public { VersionStatus::Published } else { VersionStatus::Draft },
        graph_iri,
...
pub async fn patch_version_data(
    ...
    for (graph, triples) in group_by_graph(&body.remove, &default_graph, &record) {
        ...
        if !block.is_empty() {
            ...
            state.store.update(&q).map_err(|e| AppError::BadRequest(e.to_string()))?;
...
pub async fn create_draft(
    ...
    if target_ver.is_empty() {
        return Err(AppError::BadRequest("targetVersion is required".to_string()));
    }
    ...
    let now = Utc::now().to_rfc3339();
    let graph_iri = format!("{}/data-model/{}/version/{}", state.base_url, id, target_ver);
    ...
    let mut affected = vec![record.graph_iri.clone()];
    affected.extend(record.sub_graphs.iter().cloned());
    let msg = body.message.as_deref().map(str::trim).filter(|m| !m.is_empty())
        .map(str::to_string)
...
pub async fn create_branch(
    ...
    if branch.is_empty() || branch == "main" {
        return Err(AppError::BadRequest("branch must be a non-empty name other than 'main'".to_string()));
    }
    registry::get_version(&state.store, &state.base_url, &id, &body.from_version)
        .ok_or_else(|| AppError::NotFound(format!("Source version '{}' not found", body.from_version)))?;
    ...
    let draft_sub_graphs = upload::clone_graphs_as_draft(
        &state.store, &state.base_url, &id, &body.from_version, &target_ver,
    ).map_err(AppError::from)?;
...
pub async fn list_branches(
    ...
    let views: Vec<BranchVersionView> = registry::list_versions(&state.store, &state.base_url, &id)
        ...
        .map(|v| BranchVersionView {
            ...
            sub_graphs: if v.sub_graphs.is_empty() { vec![v.graph_iri] } else { v.sub_graphs },
        })
        .collect();
...
pub async fn merge_preview(
    ...
    let parent = registry::get_data_model(&state.store, &state.base_url, &id)
        .ok_or_else(|| AppError::NotFound(format!("Data model '{id}' not found")))?;
    let uid = user.as_deref().map(|u| u.user_id.as_str());
    if !state.auth_db.can_access_ontology(uid, parent.is_public, parent.owner_type.as_deref(), parent.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::NotFound(format!("Data model '{id}' not found")));
    }
    let preview = compute_merge_preview(&state.store, &state.base_url, &id, &params.from, &params.into)?;
    Ok(Json(preview))
}
...
pub async fn merge_apply(
    ...
    let parent = registry::get_data_model(&state.store, &state.base_url, &id)
        .ok_or_else(|| AppError::NotFound(format!("Data model '{id}' not found")))?;
    if !state.auth_db.can_write_ontology(&user.user_id, parent.owner_type.as_deref(), parent.owner_id.as_deref())
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::Unauthorized("Write access to this data model required".to_string()));
    }
...
pub fn write_merged_graph(
    ...
    for chunk in triples.chunks(1000) {
        let block: String = chunk
            ...
            .join("\n");
        if !block.is_empty() {
            store.update(&format!("INSERT DATA {{ GRAPH <{graph_iri}> {{ {block} }} }}"))?;
        }
    }
    Ok(())
}
...
pub async fn publish_version(
    ...
    let prior_version_iri = if let Some(old_ver) = &data_model.latest_published {
        if old_ver != &ver {
            ...
            .map_err(AppError::from)?;
            Some(super::version_iri::build_version_iri(&data_model.namespace, old_ver))
        } else {
            None
        }
    } else {
...
async fn transition_sub_graph(
    ...
    let record = registry::get_version(&state.store, &state.base_url, &id, &ver)
        .ok_or_else(|| AppError::NotFound(format!("Version '{ver}' not found")))?;
    let sub_graph_iri = resolve_sub_graph(&record, &graph)
        .ok_or_else(|| AppError::BadRequest(format!("Subgraph '{graph}' not found in version '{ver}'")))?;
...
pub async fn stage_sub_graph(
    ...
    transition_sub_graph(state, user, id, ver, body.graph, VersionStatus::Staged, false).await
}
...
pub async fn publish_sub_graph(
    ...
    transition_sub_graph(state, user, id, ver, body.graph, VersionStatus::Published, true).await
}
...
pub async fn deprecate_sub_graph(
    ...
    transition_sub_graph(state, user, id, ver, body.graph, VersionStatus::Deprecated, true).await
}
...
pub async fn diff_versions(
    ...
    let from_record = registry::get_version(&state.store, &state.base_url, &id, &params.from)
        .ok_or_else(|| {
            AppError::NotFound(format!("Version '{}' not found", params.from))
        })?;

    let to_record = registry::get_version(&state.store, &state.base_url, &id, &params.to)
        .ok_or_else(|| {
            AppError::NotFound(format!("Version '{}' not found", params.to))
        })?;
