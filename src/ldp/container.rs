...
pub fn add_member(store: &TripleStore, container_iri: &str, member_iri: &str) -> Result<(), String> {
    let q = format!(
        "INSERT DATA {{ <{container_iri}> <{LDP_CONTAINS}> <{member_iri}> }}"
    );
    store.update(&q).map_err(|e| e.to_string())
}
...
pub fn remove_member(store: &TripleStore, container_iri: &str, member_iri: &str) -> Result<(), String> {
    let q1 = format!(
        "DELETE DATA {{ <{container_iri}> <{LDP_CONTAINS}> <{member_iri}> }}"
    );
...
pub fn add_direct_membership_triple(
    ...
    let q = format!(
        "INSERT DATA {{ <{membership_resource}> <{has_member_relation}> <{member_iri}> }}"
    );
    store.update(&q).map_err(|e| e.to_string())
}
...
pub fn remove_direct_membership_triple(
    ...
    let q = format!(
        "DELETE DATA {{ <{membership_resource}> <{has_member_relation}> <{member_iri}> }}"
    );
    store.update(&q).map_err(|e| e.to_string())
}
...
pub fn load_resource_jsonld(store: &TripleStore, _iri: &str, jsonld: &str) -> Result<(), String> {
    store
        .load_str(jsonld, oxigraph::io::RdfFormat::JsonLd { profile: Default::default() }, None)
        .map_err(|e| e.to_string())
}
...
mod tests {
    ...
    fn test_get_container_type_basic() {
        ...
        assert_eq!(get_container_type(&store, "http://ex.org/bc/"), ContainerType::Basic);
    }
    ...
    fn test_list_members_pagination() {
        ...
        for i in 1..=5 {
            add_member(&store, "http://ex.org/page/", &format!("http://ex.org/page/item{i}")).unwrap();
        }
    ...
    fn test_get_container_type_direct() {
        ...
        assert_eq!(get_container_type(&store, "http://ex.org/dc/"), ContainerType::Direct);
    }
    ...
    fn test_get_membership_info_direct() {
        ...
        let info = get_membership_info(&store, "http://ex.org/dc/").unwrap().unwrap();
    ...
    fn test_get_container_type_indirect() {
        ...
        assert_eq!(get_container_type(&store, "http://ex.org/ic/"), ContainerType::Indirect);
    }
    ...
    fn test_get_container_type_non_rdf() {
        ...
        assert_eq!(get_container_type(&store, "http://ex.org/img"), ContainerType::NonRdfSource);
    }
    ...
    fn test_store_binary_resource_round_trip() {
        ...
        let (ct, retrieved) = get_binary_resource(&store, "http://ex.org/img").unwrap().unwrap();
    ...
    fn test_binary_content_type_preserved() {
        ...
        let (ct, _) = get_binary_resource(&store, "http://ex.org/vid").unwrap().unwrap();
