...
fn constrained_by_link(base_url: &str) -> String {
    format!(
        "<{base_url}/ldp/constraints>; rel=\"http://www.w3.org/ns/ldp#constrainedBy\""
    )
}
...
impl PreferState {
    ...
    fn include_containment(&self) -> bool {
        ...
        if self.omit.iter().any(|u| u == LDP_PREFER_CONTAINMENT || u == LDP_PREFER_MINIMAL_CONTAINER) {
            return false;
        }
        // include=PreferMinimalContainer → exclude containment (same as minimal)
        if self.include.iter().any(|u| u == LDP_PREFER_MINIMAL_CONTAINER) {
            return false;
        }
        true
    }
    ...
    fn include_membership(&self) -> bool {
        ...
        if self.omit.iter().any(|u| u == LDP_PREFER_MEMBERSHIP || u == LDP_PREFER_MINIMAL_CONTAINER) {
            return false;
        }
        if self.include.iter().any(|u| u == LDP_PREFER_MINIMAL_CONTAINER) {
            return false;
...
fn parse_prefer(headers: &HeaderMap) -> PreferState {
    let val = match headers.get("prefer").and_then(|v| v.to_str().ok()) {
        Some(v) => v.to_string(),
        None => return PreferState { ret: PreferReturn::Representation, omit: vec![], include: vec![] },
    };
...
fn negotiate_ldp_format(headers: &HeaderMap) -> (oxigraph::io::RdfFormat, &'static str) {
    ...
    if a.contains("application/n-triples") {
        ...
        (oxigraph::io::RdfFormat::JsonLd { profile: oxigraph::io::JsonLdProfileSet::empty() }, "application/ld+json")
    } else {
...
fn reserialize_ntriples(nt: &[u8], target_format: oxigraph::io::RdfFormat) -> Vec<u8> {
    ...
    let Ok(nt_str) = std::str::from_utf8(nt) else { return nt.to_vec() };
    let Ok(tmp) = crate::store::TripleStore::in_memory() else { return nt.to_vec() };
    if tmp.load_str(nt_str, oxigraph::io::RdfFormat::NTriples, None).is_err() {
        return nt.to_vec();
    }
    tmp.dump(target_format, None).unwrap_or_else(|_| nt.to_vec())
}
    ...
    if !body.is_empty() {
        ...
        if is_turtle || is_jsonld || is_rdf_xml {
            ...
            let fmt = if is_jsonld {
                oxigraph::io::RdfFormat::JsonLd { profile: Default::default() }
            } else if is_rdf_xml {
            ...
            if let Err(e) = container::store_binary_resource(
                &state.store,
                &member_iri,
                content_type,
                &body,
        ...
        if let Ok(Some(info)) = container::get_membership_info(&state.store, &container_iri) {
            if re_read_ct == ContainerType::Indirect {
                ...
                if let Some(icr) = &info.inserted_content_relation {
                    let icr_val_q = format!(
                        "SELECT ?val WHERE {{ <{member_iri}> <{icr}> ?val }}"
                    );
