...
pub async fn begin_oidc_flow(
    ...
    debug!("OIDC flow started for provider '{}', state={}", provider.slug, &state_key[..8]);
...
pub async fn complete_oidc_flow(
    ...
    let expires = chrono::Utc::now() + chrono::Duration::days(jwt_config.refresh_expiry_days as i64);
