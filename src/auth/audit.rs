...
impl AuditEventBuilder {
    ...
    pub fn actor(mut self, id: impl Into<String>, username: impl Into<String>, role: impl Into<String>) -> Self {
    ...
    pub fn actor_id(mut self, id: impl Into<String>) -> Self { self.actor_id = Some(id.into()); self }
    pub fn actor_username(mut self, u: impl Into<String>) -> Self { self.actor_username = Some(u.into()); self }
    ...
    pub fn ip(mut self, ip: impl Into<String>) -> Self { self.ip_address = Some(ip.into()); self }
    pub fn user_agent(mut self, ua: impl Into<String>) -> Self { self.user_agent = Some(ua.into()); self }
    pub fn details(mut self, v: JsonValue) -> Self { self.details = Some(v); self }
    pub fn request_id(mut self, r: impl Into<String>) -> Self { self.request_id = Some(r.into()); self }
}
...
impl AuditLogger {
    ...
    pub fn log(&self, b: AuditEventBuilder) {
        let event_type = match b.event_type {
            Some(t) => t.as_str(),
            None => { warn!("audit log called without event_type — dropping"); return; }
        };
        let outcome = match b.outcome {
            Some(o) => o.as_str(),
            None => { warn!("audit log called without outcome — dropping"); return; }
        };
        ...
        let conn = match self.pool.get() {
            Ok(c) => c,
            Err(e) => { warn!("audit: pool acquire failed: {e}"); return; }
        };
        if let Err(e) = conn.execute(
            ...
            params![
                id, ts,
                b.actor_id, b.actor_username, b.actor_role,
                event_type, b.resource_type, b.resource_id, b.action, outcome,
                b.ip_address, b.user_agent, details_json, b.request_id,
            ],
        ) {
    ...
    pub fn list(
        ...
        if let Some(t) = event_type { sql.push_str(" AND event_type = ?"); args.push(Box::new(t.to_string())); }
        if let Some(a) = actor_id   { sql.push_str(" AND actor_id   = ?"); args.push(Box::new(a.to_string())); }
        if let Some(s) = since      { sql.push_str(" AND timestamp >= ?"); args.push(Box::new(s.to_string())); }
...
pub fn client_ip(headers: &axum::http::HeaderMap, connect_addr: Option<std::net::SocketAddr>) -> Option<String> {
...
pub fn spawn_pseudonymisation_task(logger: std::sync::Arc<AuditLogger>, pseudonymise_after_days: u64) {
    tokio::spawn(async move {
        let day = std::time::Duration::from_secs(86_400);
        loop {
            // Run on startup, then every 24h.
            let cutoff = (chrono::Utc::now() - chrono::Duration::days(pseudonymise_after_days as i64)).to_rfc3339();
            match logger.pseudonymise_older_than(&cutoff) {
                Ok(0) => tracing::debug!("audit: pseudonymisation found nothing older than {}", cutoff),
...
pub async fn admin_list_audit(
    ...
    let events = logger
        .list(limit, offset, q.event_type.as_deref(), q.actor_id.as_deref(), q.since.as_deref())
...
pub async fn admin_export_audit(
    ...
    let events = logger
        .list(100_000, 0, q.event_type.as_deref(), q.actor_id.as_deref(), q.since.as_deref())
        .map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    if q.format.as_deref() == Some("csv") {
        ...
        for e in &events {
            ...
            let details = e.details.as_ref().map(|v| v.to_string()).unwrap_or_default();
            let row = [
                e.id.as_str(), e.timestamp.as_str(),
                e.actor_id.as_deref().unwrap_or(""), e.actor_username.as_deref().unwrap_or(""),
                e.actor_role.as_deref().unwrap_or(""), e.event_type.as_str(),
                e.resource_type.as_deref().unwrap_or(""), e.resource_id.as_deref().unwrap_or(""),
                e.action.as_deref().unwrap_or(""), e.outcome.as_str(),
                e.ip_address.as_deref().unwrap_or(""), e.user_agent.as_deref().unwrap_or(""),
                e.request_id.as_deref().unwrap_or(""), details.as_str(),
            ];
...
mod tests {
    ...
    fn update_and_delete_are_blocked_by_trigger() {
        let db = AuthDb::in_memory().unwrap();
        let logger = AuditLogger::new(db.pool());
        logger.log(AuditEventBuilder::new(AuditEventType::Logout, AuditOutcome::Success));
