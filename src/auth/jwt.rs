...
pub fn is_weak_jwt_secret(secret: &str) -> bool {
    const WEAK: &[&str] = &[
        ...
        "dev-secret-change-me", "secret", "changemechangeme",
    ];
...
mod jwt_security_tests {
    ...
    fn weak_jwt_secrets_are_flagged() {
        for s in ["", "change-me-in-production", "  changeme ", "dev-secret-change-me", "secret"] {
            assert!(is_weak_jwt_secret(s), "{s:?} must be rejected as a weak/default JWT secret");
        }
    }
    ...
    fn strong_jwt_secret_is_allowed() {
        ...
        assert!(!is_weak_jwt_secret("9f3c2a1be7d04f5a8c6b0e2d4a6f8c1e3b5d7f9a1c3e5b7d9f1a3c5e7b9d1f3a5"));
    }
}
