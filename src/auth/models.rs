...
pub fn map_claims_to_role(claim_values: &[String], role_claim_map: Option<&str>) -> MappedClaims {
    ...
    for value in claim_values {
        let Some(mapped) = map.get(value) else { continue };
        if mapped == "publisher" {
...
impl OauthProviderCreate {
    pub fn scopes_or_default(&self) -> String {
        self.scopes.clone().unwrap_or_else(|| "openid email profile".to_string())
    }
    pub fn default_role_or_user(&self) -> String {
        self.default_role.clone().unwrap_or_else(|| "user".to_string())
    }
}
...
mod tests {
    ...
    fn map_claims_empty_or_absent_map_yields_nothing() {
        assert_eq!(map_claims_to_role(&vals(&["any"]), None), MappedClaims::default());
        assert_eq!(map_claims_to_role(&vals(&["any"]), Some("{}")), MappedClaims::default());
    ...
    fn access_level_is_ordered_and_capability_consistent() {
        ...
        assert_eq!(AccessLevel::Read.max(AccessLevel::Manage), AccessLevel::Manage);
