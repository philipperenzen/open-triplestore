...
pub async fn oidc_authorize(
    ...
    match begin_oidc_flow(&provider, &sessions, &redirect_uri, &state.jwt_config.secret).await {
...
pub async fn oidc_callback(
    ...
    let code = match params.code {
        Some(c) => c,
        None => {
            return (StatusCode::BAD_REQUEST, "{\"error\":\"Missing code parameter\"}").into_response()
        }
    };
    let state_key = match params.state {
        Some(s) => s,
        None => {
            return (StatusCode::BAD_REQUEST, "{\"error\":\"Missing state parameter\"}").into_response()
        }
    };
...
pub async fn saml_acs(
    ...
    .await
    {
        Ok((access, refresh)) => Json(serde_json::json!({
            "access_token": access,
