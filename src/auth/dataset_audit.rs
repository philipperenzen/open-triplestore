...
use super::db::AuthDb;
use super::dataset_graph::{build_dataset_metadata_ttl, dataset_metadata_graph_iri, write_dataset_metadata_graph};
...
pub fn validate_metadata(main_store: &TripleStore, metadata_ttl: &str) -> Option<ValidationReport> {
    let shapes_bytes = main_store.graph_store_get(Some(DATASET_STRUCTURE_GRAPH), RdfFormat::Turtle).ok()?;
    ...
    temp.graph_store_put(Some(DATASET_STRUCTURE_GRAPH), &shapes_ttl, RdfFormat::Turtle).ok()?;
    let data_graph = "urn:system:validate:dataset-metadata";
    temp.graph_store_put(Some(data_graph), metadata_ttl, RdfFormat::Turtle).ok()?;
...
pub fn seed_dataset_structure_shapes(store: &TripleStore, auth_db: &AuthDb) -> anyhow::Result<String> {
    ...
    store
        .graph_store_put(Some(DATASET_STRUCTURE_GRAPH), DATASET_STRUCTURE_TTL, RdfFormat::Turtle)
    ...
    let (targets, count) = crate::shacl_studio::run::analyze_shapes_graph(store, DATASET_STRUCTURE_GRAPH);
    studio.save_shape_graph_revision(&set.id, DATASET_STRUCTURE_TTL, &targets, count, Some("Built-in"), None)?;
...
pub fn audit_dataset_metadata(store: &TripleStore, auth_db: &AuthDb, base_url: &str) -> anyhow::Result<(usize, usize)> {
    ...
    for ds in datasets {
        ...
        let stored_bytes = store.graph_store_get(Some(&meta_iri), RdfFormat::Turtle).unwrap_or_default();
        let stored = String::from_utf8_lossy(&stored_bytes).into_owned();
        let conforming_now = if stored.trim().is_empty() {
            false
        } else {
            validate_metadata(store, &stored).map(|r| r.conforms).unwrap_or(true)
        };
        if conforming_now {
            continue;
        }
        ...
        let entries = auth_db.list_dataset_graph_entries(&ds.id).unwrap_or_default();
        write_dataset_metadata_graph(store, base_url, &ds, &entries);
        let rebuilt = build_dataset_metadata_ttl(base_url, &ds, &entries);
        let still_bad = validate_metadata(store, &rebuilt).map(|r| !r.conforms).unwrap_or(false);
        if still_bad {
            mark_nonconforming(store, base_url, &ds.id);
            failing += 1;
            tracing::warn!("dataset {} metadata still non-conforming after repair", ds.id);
        } else {
            repaired += 1;
        }
    }
    ...
    if repaired > 0 || failing > 0 {
        tracing::info!("dataset metadata audit: {repaired} repaired, {failing} still non-conforming");
    }
    Ok((repaired, failing))
}
...
<urn:g:1> otso:graphRole otso:Instances ."#;
        ...
        assert!(r.conforms, "complete metadata must conform: {:?}", r.results);
...
<urn:ds:2> a dcat:Dataset ; dct:title "T" ."#;
        ...
        assert!(!r.conforms, "metadata missing identifier/visibility must not conform");
    }
}
