...
pub fn write_org_metadata_graph(
    ...
    if let Some(parent) = org.parent_org_id.as_deref().filter(|s| !s.is_empty()) {
        clauses.push(format!("org:subOrganizationOf <{}>", org_iri(base_url, parent)));
    }
    ...
    let has_contact = org.contact_name.as_deref().filter(|s| !s.is_empty()).is_some()
        || org.contact_email.as_deref().filter(|s| !s.is_empty()).is_some()
        || org.contact_url.as_deref().filter(|s| !s.is_empty()).is_some();
    if has_contact {
