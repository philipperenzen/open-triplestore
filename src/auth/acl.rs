...
pub fn filter_quad_indices_by_label(
    ...
    let denied: std::collections::HashSet<usize> = labelled
        .into_iter()
        .filter(|(_, label_iri)| !check_graph_permission(user, label_iri, "read", auth_db))
        .map(|(idx, _)| idx)
        .collect();

    (0..quads.len()).filter(|i| !denied.contains(i)).collect()
}
...
mod tests {
    ...
    fn test_path_pattern_wildcard_segment() {
        assert!(matches_path_pattern(
            "/api/datasets/*/sparql",
            "/api/datasets/abc123/sparql"
        ));
        assert!(!matches_path_pattern(
            "/api/datasets/*/sparql",
            "/api/datasets/abc123/store"
        ));
    }
