...
pub fn build_dataset_metadata_ttl(
    ...
    if let Some(themes_json) = &dataset.themes {
        if let Ok(themes) = serde_json::from_str::<Vec<String>>(themes_json) {
            for theme in &themes {
                if !theme.is_empty() {
                    ttl.push_str(&format!("    dcat:theme <{}> ;\n", escape_sparql_iri(theme)));
                }
            }
        }
    }
    ...
    if let Some(kw_json) = &dataset.keywords {
        if let Ok(keywords) = serde_json::from_str::<Vec<String>>(kw_json) {
            for kw in &keywords {
                if !kw.is_empty() {
                    ttl.push_str(&format!("    dcat:keyword \"{}\"@en ;\n", escape_ttl_string(kw)));
                }
            }
        }
    }
    ...
    if let Some(status) = &dataset.adms_status {
        if !status.is_empty() {
            ttl.push_str(&format!("    adms:status <{}> ;\n", escape_sparql_iri(status)));
        }
    }
    if let Some(notes) = &dataset.version_notes {
        if !notes.is_empty() {
            ttl.push_str(&format!("    adms:versionNotes \"{}\" ;\n", escape_ttl_string(notes)));
        }
    }
    if let Some(spatial) = &dataset.spatial {
        if !spatial.is_empty() {
            ttl.push_str(&format!("    dct:spatial <{}> ;\n", escape_sparql_iri(spatial)));
        }
    }

    let landing = dataset.landing_page.as_deref()
        .filter(|s| !s.is_empty())
        .unwrap_or(&dataset_iri);
    ttl.push_str(&format!("    dcat:landingPage <{}> ;\n", escape_sparql_iri(landing)));

    ttl.push_str(&format!("    dct:issued \"{}\"^^xsd:dateTime ;\n", dataset.created_at));
    ttl.push_str(&format!("    dct:modified \"{}\"^^xsd:dateTime ;\n", dataset.updated_at));
    ttl.push_str(&format!("    void:sparqlEndpoint <{}/sparql> ;\n", base_url));

    // Contact point as blank node
    let has_contact = dataset.contact_name.as_deref().filter(|s| !s.is_empty()).is_some()
        || dataset.contact_email.as_deref().filter(|s| !s.is_empty()).is_some()
        || dataset.contact_url.as_deref().filter(|s| !s.is_empty()).is_some();
    ...
    if has_contact {
        ...
        if let Some(url) = dataset.contact_url.as_deref().filter(|s| !s.is_empty()) {
            ttl.push_str(&format!("    vcard:hasURL <{}> ;\n", escape_sparql_iri(url)));
        }
        ttl.push_str("    .\n");
    } else {
        ttl.push_str("    .\n");
    ...
    for entry in graph_entries {
        if !entry.graph_iri.starts_with("urn:system:") {
            ttl.push_str(&format!("<{}> void:subset <{}> .\n", dataset_iri, entry.graph_iri));
