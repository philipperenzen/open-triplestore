...
use super::db::AuthDb;
use super::models::{map_claims_to_role, OauthProvider, OauthProviderCreate, Role, SystemRole, User};
...
impl AuthExt {
    ...
    pub fn from_env() -> Self {
        ...
        let role_claim_map = std::env::var("OIDC_ROLE_CLAIM_MAP").ok().filter(|s| !s.trim().is_empty());
...
fn default_role_claims() -> Vec<String> {
    ...
    vec!["roles".to_string(), "realm_access.roles".to_string(), "groups".to_string()]
}
...
impl ExternalClaims {
    ...
    pub fn claim_strings(&self, names: &[String]) -> Vec<String> {
        ...
        for name in names {
            ...
            let Some(mut cur) = self.extra.get(first) else { continue };
            let mut ok = true;
            for seg in segs {
                match cur.get(seg) {
                    Some(next) => cur = next,
...
fn sync_org_memberships(
    ...
    for group in groups {
        let Some(slug) = group.strip_prefix(org_group_prefix) else { continue };
        if slug.is_empty() {
            continue;
        }
...
impl OidcVerifier {
    ...
    async fn jwks(&self, force: bool) -> anyhow::Result<JwkSet> {
        ...
        let jwks: JwkSet = self
            ...
            .json()
            .await?;
        *self.cache.write().await = Some(CachedJwks { jwks: jwks.clone(), fetched_at: Instant::now() });
        Ok(jwks)
    }
...
pub fn provision_from_claims(
    ...
    let cap_role = |r: SystemRole| if r == SystemRole::SuperAdmin { SystemRole::Admin } else { r };
...
mod tests {
    ...
    fn jit_provisioning_is_idempotent() {
        ...
        let provider = ensure_env_provider(&db, "https://idp.example/realms/example", "user").unwrap();
        ...
        let user = provision_from_claims(
            ...
            &provider,
            &ext,
            &claims("idp-subject-abc", "alice@example.com", serde_json::json!({})),
        )
        .unwrap();
    ...
    fn role_claim_maps_to_system_role() {
        ...
        let user = provision_from_claims(
            ...
            &provider,
            &ext,
            &claims("sub-admin", "boss@example.com", serde_json::json!({"groups": ["app-admins"]})),
        )
        .unwrap();
    ...
    fn publisher_claim_grants_can_publish_capability() {
        ...
        let user = provision_from_claims(
            ...
            &provider,
            &ext,
            &claims("sub-pub", "pub@example.com", serde_json::json!({"groups": ["team-pub"]})),
        )
        .unwrap();
        assert_eq!(user.role, SystemRole::User);
        ...
        .unwrap();
        assert!(again.can_publish, "can_publish must not be revoked on absent claims");
    }
    ...
    fn org_group_claim_adds_membership() {
        ...
        let user = provision_from_claims(
            ...
            &provider,
            &ext,
            &claims("sub-orgmember", "gis@example.com", serde_json::json!({"groups": ["org:example-gis", "other"]})),
        )
        .unwrap();

        assert_eq!(db.get_org_membership(&user.id, &org.id).unwrap(), Some(Role::Member));
    }
