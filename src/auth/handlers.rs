use axum::{
    ...
    http::{header::{CONTENT_TYPE, SET_COOKIE}, HeaderMap, StatusCode},
...
fn issue_tokens(
    ...
    let expires_at = chrono::Utc::now()
        + chrono::Duration::days(jwt_config.refresh_expiry_days as i64);
    auth_db
...
pub async fn register(
    ...
    if registration_disabled
        && db.count_users().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? > 0
    {
        return Err((StatusCode::FORBIDDEN, "Self-registration is disabled".to_string()));
    }

    // Check if username already exists
    if db.get_user_by_username(&req.username).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?.is_some() {
    ...
    let user = db.create_user(&id, &req.username, &req.email, &hash, role)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn refresh(
    ...
    let refresh_token_str: String = {
        ...
        if let Some(t) = body_token {
            ...
            headers
                ...
                .and_then(|c| {
                    c.split(';').find_map(|p| {
                        p.trim().strip_prefix("refresh_token=").map(str::to_string)
                    })
                })
    ...
    let claims = jwt::verify_token(&jwt_config, &refresh_token_str)
        .map_err(|_| (StatusCode::UNAUTHORIZED, "Invalid or expired refresh token".to_string()))?;

    if claims.token_type != "refresh" {
        return Err((StatusCode::UNAUTHORIZED, "Expected refresh token".to_string()));
    }
    ...
    let stored = db
        ...
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::UNAUTHORIZED, "Refresh token not found".to_string()))?;
    ...
    if !user.is_active {
        return Err((StatusCode::UNAUTHORIZED, "Account is deactivated".to_string()));
    }
...
pub async fn logout(
    ...
    Ok((StatusCode::NO_CONTENT, clear_auth_cookie_headers(cookie_config.secure)))
}
...
pub async fn me(
    ...
    let user = db.get_user_by_id(&current_user.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
...
pub async fn update_me(
    ...
    if username != user.username {
        if let Some(other) = db.get_user_by_username(username)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        {
    ...
    if email != user.email {
        if let Some(other) = db.get_user_by_email(email)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        {
            if other.id != current_user.user_id {
    ...
    let updated = db.get_user_by_id(&current_user.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "User not found".to_string()))?;
...
pub async fn list_api_tokens(
    ...
    let tokens = db.list_api_tokens(&current_user.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn create_api_token(
    ...
    if req.name.is_empty() || req.name.len() > 100 {
        return Err((StatusCode::BAD_REQUEST, "Token name must be 1-100 characters".to_string()));
    }

    let scopes: Vec<ApiScope> = req.scopes.iter()
        .filter_map(|s| ApiScope::from_str(s))
        .collect();
    if scopes.is_empty() {
        return Err((StatusCode::BAD_REQUEST, "At least one valid scope required (read, write, admin)".to_string()));
    }

    // Non-admins cannot request admin scope
    if scopes.contains(&ApiScope::Admin) && !current_user.is_admin() {
        return Err((StatusCode::FORBIDDEN, "Admin scope requires admin privileges".to_string()));
    }
    ...
    Ok((StatusCode::CREATED, Json(ApiTokenCreatedResponse {
        token: raw_token,
        id: token.id,
        name: token.name,
...
pub async fn revoke_api_token(
    ...
    let token = db.get_api_token_by_id(&token_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
...
pub async fn admin_list_users(
    ...
    let (users, total) = db.list_users_paginated(page, limit, search)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn admin_create_user(
    ...
    let role = req.role
        .as_deref()
        .map(|r| SystemRole::from_str(r).ok_or_else(|| (StatusCode::BAD_REQUEST, "Invalid role".to_string())))
        .transpose()?
    ...
    if !authz::can_grant_role(&current_user, role) {
        return Err((StatusCode::FORBIDDEN, "Cannot create user with higher role than your own".to_string()));
    }
...
pub async fn admin_list_user_identities(
    ...
    let identities = db.list_oauth_identities_for_user(&user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn admin_update_user(
    ...
    let target = db.get_user_by_id(&user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "User not found".to_string()))?;
    ...
    if !authz::can_administer_user(&current_user, &user_id, target.role) {
        return Err((StatusCode::FORBIDDEN, "Cannot modify user with equal or higher role".to_string()));
    }
    ...
    if let Some(ref role_str) = req.role {
        ...
        if !authz::can_grant_role(&current_user, new_role) {
            return Err((StatusCode::FORBIDDEN, "Cannot assign role higher than your own".to_string()));
        }
        ...
        {
            return Err((StatusCode::CONFLICT, "Cannot demote the last active super admin".to_string()));
        }
    ...
    if let Some(is_active) = req.is_active {
        ...
        if !is_active
            ...
            && db.count_active_super_admins().unwrap_or(0) <= 1
        {
            return Err((StatusCode::CONFLICT, "Cannot deactivate the last active super admin".to_string()));
        }
        ...
        let evt = if is_active { AuditEventType::UserActivated } else { AuditEventType::UserDeactivated };
    ...
    let updated = db.get_user_by_id(&user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "User not found".to_string()))?;
...
pub async fn admin_delete_user(
    ...
    if user_id == current_user.user_id {
        return Err((StatusCode::BAD_REQUEST, "Cannot deactivate yourself".to_string()));
    }
    ...
    if !authz::can_administer_user(&current_user, &user_id, target.role) {
        return Err((StatusCode::FORBIDDEN, "Cannot deactivate user with equal or higher role".to_string()));
    }
...
pub async fn admin_reset_password(
    ...
    if !authz::can_administer_user(&current_user, &user_id, target.role) {
        return Err((StatusCode::FORBIDDEN, "Cannot reset password for user with equal or higher role".to_string()));
    }
    ...
    {
        let mut b = AuditEventBuilder::new(AuditEventType::PasswordResetForced, AuditOutcome::Success)
            .actor_id(&current_user.user_id)
            .resource("user", &user_id);
        b.ip_address = audit::client_ip(&headers, None);
        b.user_agent = audit::user_agent(&headers);
...
pub async fn admin_purge_user(
    ...
    let target = db.get_user_by_id(&user_id)
        ...
        .ok_or_else(|| (StatusCode::NOT_FOUND, "User not found".to_string()))?;
    if target.is_active {
        return Err((StatusCode::CONFLICT, "User must be deactivated before permanent deletion".to_string()));
    }
    if !authz::can_administer_user(&current_user, &user_id, target.role) {
        return Err((StatusCode::FORBIDDEN, "Cannot purge user with equal or higher role".to_string()));
    }
...
pub async fn create_organisation(
    ...
    if !current_user.is_admin() {
        return Err((StatusCode::FORBIDDEN, "Admin access required to create an organisation".to_string()));
    }
    // A parent, if given, must reference an existing organisation.
    let parent = req.parent_org_id.as_deref().filter(|s| !s.is_empty());
    if let Some(pid) = parent {
        if db.get_organisation(pid).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?.is_none() {
            return Err((StatusCode::BAD_REQUEST, "Parent organisation not found".to_string()));
        }
    }
    let id = uuid::Uuid::new_v4().to_string();
    let org = db.create_organisation(&id, &req.name, &req.slug, req.description.as_deref(), parent)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn list_organisations(
    ...
    let mut orgs = match user {
        Some(Extension(ref u)) if u.is_admin() => {
            db.list_organisations()
        }
        Some(Extension(ref u)) => {
            db.list_user_organisations(&u.user_id)
        }
        None => {
            Ok(vec![])
        }
...
fn public_dataset_orgs(db: &AuthDb) -> anyhow::Result<Vec<crate::auth::models::Organisation>> {
    ...
    for d in db.list_datasets()? {
        if matches!(d.owner_type, OwnerType::Organisation) && matches!(d.visibility, Visibility::Public) {
...
pub async fn get_organisation(
    ...
    let is_admin = user_opt.as_ref().map(|Extension(u)| u.is_admin()).unwrap_or(false);
    ...
    if !is_admin && !is_member && !org_has_public_dataset(&db, &org_id) {
        ...
        return Err(match user_opt {
            ...
            None => (StatusCode::UNAUTHORIZED, "Authentication required".to_string()),
        });
    }
...
pub async fn update_organisation(
    ...
    let current_user = require_user(user_opt)?;
    if !current_user.is_admin() {
        match db.get_org_membership(&current_user.user_id, &org_id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
            Some(Role::Admin) => {},
    ...
    let existing = db.get_organisation(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Organisation not found".to_string()))?;
    let new_parent = req.parent_org_id.as_deref().map(str::trim).filter(|s| !s.is_empty());
    if let Some(pid) = new_parent {
        if pid == org_id {
            return Err((StatusCode::BAD_REQUEST, "An organisation cannot be its own parent".to_string()));
        }
        if db.get_organisation(pid).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?.is_none() {
            return Err((StatusCode::BAD_REQUEST, "Parent organisation not found".to_string()));
        }
        // Reject cycles: the chosen parent must not be a descendant of this org.
        if db.is_org_ancestor(&org_id, pid).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
            return Err((StatusCode::BAD_REQUEST, "Cannot set a descendant organisation as the parent (would create a cycle)".to_string()));
        }
    }
    ...
    if let Some(p) = old_parent { refresh_org_metadata(&state, p); }
    if let Some(p) = new_parent {
        if Some(p) != old_parent { refresh_org_metadata(&state, p); }
    }
...
pub async fn delete_organisation(
    ...
    let current_user = require_user(user_opt)?;
    if !current_user.is_admin() {
        match state.auth_db.get_org_membership(&current_user.user_id, &org_id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
            Some(Role::Admin) => {},
            _ => return Err((StatusCode::FORBIDDEN, "Admin access required".to_string())),
        }
    ...
    let dataset_ids = state.auth_db.list_dataset_ids_by_owner(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    for dataset_id in &dataset_ids {
        let dataset = state.auth_db.get_dataset(dataset_id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        let graph_iris = state.auth_db.list_dataset_graphs(dataset_id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        for iri in &graph_iris {
            let shared = state.auth_db.graph_has_other_dataset_refs(iri.as_str(), dataset_id)
                .unwrap_or(false);
            if !shared {
        ...
        state.auth_db.delete_dataset(dataset_id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    }
    ...
    let former_parent = state.auth_db.get_organisation(&org_id)
        .ok().flatten()
        .and_then(|o| o.parent_org_id)
        .filter(|p| !p.is_empty());
    let children = state.auth_db.list_child_organisations(&org_id).unwrap_or_default();
    // Orphan direct children (promote to top-level) so the FK reference clears.
    for child in &children {
        let _ = state.auth_db.update_organisation(
            &child.id, &child.name, child.description.as_deref(), child.homepage.as_deref(),
            child.identifier.as_deref(), child.contact_name.as_deref(), child.contact_email.as_deref(),
            child.contact_url.as_deref(), child.org_type.as_deref(), None,
        );
    }

    state.auth_db.delete_organisation(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    if let Some(p) = former_parent { refresh_org_metadata(&state, &p); }
    for child in &children { refresh_org_metadata(&state, &child.id); }
...
pub async fn list_org_members(
    ...
    let members = db.list_org_members(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    // Flat shape consumed by both frontends (user_id / username / role).
    let response: Vec<serde_json::Value> = members.into_iter().map(|(user, role)| {
        serde_json::json!({
            "user_id": user.id,
...
pub async fn add_org_member(
    ...
    if db.get_user_by_id(&req.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .is_none()
    {
...
pub async fn remove_org_member(
    ...
    if let Some(u) = target {
        if u.role == SystemRole::SuperAdmin {
            return Err((StatusCode::FORBIDDEN, "Super admins cannot be removed from an organisation".to_string()));
        }
    }
...
pub async fn update_org_member_role(
    ...
    let role = req["role"].as_str()
        ...
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "Invalid or missing role".to_string()))?;
...
pub async fn create_group(
    ...
    let id = uuid::Uuid::new_v4().to_string();
    let group = db.create_group(&id, &org_id, &req.name, req.parent_group_id.as_deref())
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn list_groups(
    ...
    let groups = db.list_org_groups(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_group(
    ...
    let group = db.get_group(&group_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Group not found".to_string()))?;

    if group.org_id != org_id {
        return Err((StatusCode::NOT_FOUND, "Group not found in this organisation".to_string()));
    }
...
pub async fn list_group_members(
    ...
    let members = db.list_group_members(&group_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn remove_group_member(
    ...
    if let Some(u) = target {
        if u.role == SystemRole::SuperAdmin {
            return Err((StatusCode::FORBIDDEN, "Super admins cannot be removed from a group".to_string()));
        }
    }
...
pub async fn create_dataset(
    ...
    let dataset = db.create_dataset(&id, &req.name, req.description.as_deref(), owner_type, &req.owner_id, visibility, graph_role)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    if let Err(report) =
        dataset_graph::write_dataset_metadata_graph_checked(&state.store, &state.base_url, &dataset, &[])
    {
        return Err((
            StatusCode::UNPROCESSABLE_ENTITY,
            serde_json::to_string(&report).unwrap_or_else(|_| "dataset metadata failed validation".to_string()),
        ));
    }
...
pub async fn list_datasets(
    ...
    let datasets = db.list_accessible_datasets(user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let views: Vec<DatasetView> = datasets.into_iter().map(|ds| {
        ...
        DatasetView { dataset: ds, can_write, can_manage, effective_role, roles }
    }).collect();
...
pub async fn get_dataset(
    ...
    let dataset = db.get_dataset(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Dataset not found".to_string()))?;

    let user_id = user.as_ref().map(|u| u.user_id.as_str());
    if !db.can_access_dataset(user_id, &dataset).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        return Err((StatusCode::FORBIDDEN, "Access denied".to_string()));
    }
    ...
    Ok(Json(DatasetView { dataset, can_write, can_manage, effective_role, roles }))
}
...
pub async fn my_dataset_usage(
    ...
    let current_user = require_user(user_opt)?;
    let stats = db.dataset_usage_for_user(&current_user.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(stats))
}
...
pub async fn admin_dataset_usage(
    ...
    let stats = db.dataset_usage_all(q.since.as_deref(), limit)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(stats))
}
...
pub async fn update_dataset(
    ...
    if !db.can_write_dataset(&current_user.user_id, &dataset).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
    ...
    if visibility != dataset.visibility
        && !db.can_manage_dataset(&current_user.user_id, &dataset)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
    {
        return Err((StatusCode::FORBIDDEN, "Manage access required to change visibility".to_string()));
    }

    db.update_dataset(&dataset_id, &req.name, req.description.as_deref(), visibility)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let themes_json = req.themes.as_ref()
        .map(|v| serde_json::to_string(v).unwrap_or_default());
    let keywords_json = req.keywords.as_ref()
        .map(|v| serde_json::to_string(v).unwrap_or_default());
    ...
    let updated = db.get_dataset(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Dataset not found".to_string()))?;
...
pub async fn delete_dataset(
    ...
    if !state.auth_db.can_write_dataset(&current_user.user_id, &dataset).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        return Err((StatusCode::FORBIDDEN, "Write access required".to_string()));
    }
...
pub async fn list_dataset_graphs(
    ...
    let user_id = user.as_ref().map(|u| u.user_id.as_str());
    let effective_role = db.effective_dataset_role(user_id, &dataset)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let entries = db.list_dataset_graph_entries(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let graphs: Vec<serde_json::Value> = entries.into_iter()
...
pub async fn list_dataset_commits(
    ...
    let graphs = db.list_dataset_graphs(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn remove_dataset_graph(
    ...
    state.auth_db.remove_dataset_graph(&dataset_id, &req.graph_iri)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let shared = state.auth_db.graph_has_other_dataset_refs(&req.graph_iri, "")
...
pub async fn create_service(
    ...
    let id = uuid::Uuid::new_v4().to_string();
    let service = db.create_sparql_service(&id, &dataset_id, &req.name, &req.slug, req.description.as_deref())
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn list_services(
    ...
    let services = db.list_dataset_services(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_service(
    ...
    let service = db.get_sparql_service(&service_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
...
pub async fn update_service(
    ...
    db.update_sparql_service(&service_id, &req.name, req.description.as_deref(), req.is_active)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn list_service_graphs(
    ...
    let graphs = db.list_service_graphs(&service_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn list_public_users(
    ...
    let response: Vec<PublicUser> = users
        ...
        .map(|u| PublicUser { id: u.id, username: u.username, avatar_key: u.avatar_key })
        .collect();
...
pub async fn delete_user(
    ...
    if user_id == current_user.user_id {
        return Err((StatusCode::BAD_REQUEST, "Cannot delete your own account here".to_string()));
    }
    ...
    if !authz::can_administer_user(&current_user, &user_id, target.role) {
        return Err((StatusCode::FORBIDDEN, "Cannot delete user with equal or higher role".to_string()));
    }
    ...
    if target.role == SystemRole::SuperAdmin
        ...
        && db.count_active_super_admins().unwrap_or(0) <= 1
    {
        return Err((StatusCode::CONFLICT, "Cannot delete the last active super admin".to_string()));
    }
...
pub async fn update_dataset_shacl(
    ...
    db.update_dataset_shacl(&dataset_id, req.shacl_on_write, req.shapes_graph_iri.as_deref())
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn update_dataset_role(
    ...
    let graph_role = req["graph_role"].as_str()
        .and_then(GraphKind::from_str);
    ...
    if let Some(role) = graph_role {
        ...
        if role_str == "model" || role_str == "vocabulary" {
            ...
            if !registry_id.is_empty() {
                ...
                let title = if dataset.name.is_empty() { dataset_id.as_str() } else { dataset.name.as_str() };
                let description = dataset.description.as_deref();

                if role_str == "model" {
                    if !crate::data_models::registry::data_model_exists(&state.store, &state.base_url, &registry_id) {
                        if let Err(e) = crate::data_models::registry::insert_data_model(
                            ...
                            tracing::warn!("failed to auto-register data model for dataset {dataset_id}: {e}");
                        }
                    }
                } else {
                    // role_str == "vocabulary"
                    if !crate::vocabularies::registry::vocabulary_exists(&state.store, &state.base_url, &registry_id) {
                        if let Err(e) = crate::vocabularies::registry::insert_vocabulary(
                            ...
                            tracing::warn!("failed to auto-register vocabulary for dataset {dataset_id}: {e}");
                        }
                    }
                }
            }
...
pub(crate) fn unique_dataset_slug(db: &AuthDb, name: &str) -> String {
    let base = {
        let s = slugify(name);
        if s.is_empty() { "dataset".to_string() } else { s }
    };
...
pub async fn list_access(
    ...
    let users = db.list_dataset_access_users(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
fn validate_grant_principal(
    ...
    let role = ResourceRole::from_str(role)
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "Invalid role (expected viewer | editor | admin)".to_string()))?;
    match principal_type {
        "user" => {
            let exists = db.get_user_by_id(principal_id)
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
                .is_some();
            if !exists {
        ...
        "group" => {
            let exists = db.get_group(principal_id)
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
                .is_some();
            if !exists {
        ...
        "organisation" => {
            let exists = db.get_organisation(principal_id)
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
                .is_some();
            if !exists {
        ...
        _ => return Err((StatusCode::BAD_REQUEST, "principal_type must be 'user', 'group', or 'organisation'".to_string())),
    }
    Ok(role)
}
...
pub async fn list_dataset_grants(
    ...
    let dataset = db.get_dataset(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Dataset not found".to_string()))?;
    if !db.can_manage_dataset(&current_user.user_id, &dataset)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        return Err((StatusCode::FORBIDDEN, "Manage access required".to_string()));
    }
    let grants = db.list_resource_grants("dataset", &dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(grants))
}
...
pub async fn set_dataset_grant(
    ...
    let grant = db.set_resource_grant("dataset", &dataset_id, &req.principal_type, &req.principal_id, role, &current_user.user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn upload_user_avatar(
    ...
    if let Some(field) = multipart.next_field().await
        ...
        if !is_allowed_image_type(&content_type) {
            return Err((StatusCode::BAD_REQUEST, "Only PNG, JPEG, GIF, or WebP images are allowed".to_string()));
        }
        let data = field.bytes().await.map_err(|e| (StatusCode::BAD_REQUEST, e.to_string()))?;
        if data.len() > 5 * 1024 * 1024 {
            return Err((StatusCode::PAYLOAD_TOO_LARGE, "Image must be under 5 MB".to_string()));
        }
        let ext = content_type.split('/').nth(1).unwrap_or("jpg");
        let key = format!("avatars/{}.{}", current_user.user_id, ext);
        state.object_store.upload(&key, data, &content_type).await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        state.auth_db.update_user_avatar(&current_user.user_id, Some(&key))
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_user_avatar(
    ...
    let user = state.auth_db.get_user_by_id(&user_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "User not found".to_string()))?;
    let key = user.avatar_key.ok_or_else(|| (StatusCode::NOT_FOUND, "No avatar set".to_string()))?;
    let (data, content_type) = state.object_store.download(&key).await
...
pub async fn get_org_image(
    ...
    let org = state.auth_db.get_organisation(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Organisation not found".to_string()))?;
    let key = org.image_key.ok_or_else(|| (StatusCode::NOT_FOUND, "No image set".to_string()))?;
    let (data, content_type) = state.object_store.download(&key).await
...
pub async fn upload_dataset_image(
    ...
    if let Some(field) = multipart.next_field().await
        ...
        state.object_store.upload(&key, data, &content_type).await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        state.auth_db.update_dataset_image(&dataset_id, Some(&key))
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        return Ok(Json(serde_json::json!({ "image_key": key })));
    }
    Err((StatusCode::BAD_REQUEST, "No file provided".to_string()))
...
pub async fn get_dataset_image(
    ...
    let key = dataset.image_key.ok_or_else(|| (StatusCode::NOT_FOUND, "No image set".to_string()))?;
    let (data, content_type) = state.object_store.download(&key).await
        .map_err(|_| (StatusCode::NOT_FOUND, "Image not found".to_string()))?;
    Ok((StatusCode::OK, [(CONTENT_TYPE, content_type)], data).into_response())
}
...
pub async fn get_org_banner(
    ...
    let org = state.auth_db.get_organisation(&org_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Organisation not found".to_string()))?;
    let key = org.banner_key.ok_or_else(|| (StatusCode::NOT_FOUND, "No banner set".to_string()))?;
    let (data, content_type) = state.object_store.download(&key).await
...
pub async fn upload_dataset_banner(
    ...
    if let Some(field) = multipart.next_field().await
        ...
        state.object_store.upload(&key, data, &content_type).await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        state.auth_db.update_dataset_banner(&dataset_id, Some(&key))
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        return Ok(Json(serde_json::json!({ "banner_key": key })));
    }
    Err((StatusCode::BAD_REQUEST, "No file provided".to_string()))
...
pub async fn get_dataset_banner(
    ...
    let key = dataset.banner_key.ok_or_else(|| (StatusCode::NOT_FOUND, "No banner set".to_string()))?;
    let (data, content_type) = state.object_store.download(&key).await
        .map_err(|_| (StatusCode::NOT_FOUND, "Banner not found".to_string()))?;
    Ok((StatusCode::OK, [(CONTENT_TYPE, content_type)], data).into_response())
}
