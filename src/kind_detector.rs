...
fn classify(ev: Evidence) -> Detected {
    ...
    if total_schema == 0 && abox == 0 {
        return Detected { primary: None, mixed: false, evidence: ev };
    }
    ...
    if entail > 0 && entail >= tbox.max(shapes).max(vocab).max(abox) {
        ...
        return Detected { primary: Some(RegistryKind::Entailment), mixed, evidence: ev };
    }
    ...
    if shapes > 0 && tbox == 0 && shapes >= vocab * 3 && shapes >= abox * 3 {
        ...
        return Detected { primary: Some(RegistryKind::Shapes), mixed, evidence: ev };
    }

    // Instance-dominant: mostly instance data with very little schema.
    if abox > total_schema * 3 {
        let mixed = total_schema > 0;
        return Detected { primary: Some(RegistryKind::Instances), mixed, evidence: ev };
    }
    ...
    if vocab > 0 && vocab >= schema_for_vocab * 3 && abox < vocab {
        ...
        return Detected { primary: Some(RegistryKind::Vocabulary), mixed, evidence: ev };
    }
    ...
    if tbox > 0 && tbox >= vocab * 3 && abox <= tbox.max(shapes) {
        ...
        return Detected { primary: Some(RegistryKind::DataModel), mixed, evidence: ev };
    }
    ...
    if vocab > 0 && schema_for_vocab > 0 {
        if vocab >= schema_for_vocab * 3 {
            return Detected { primary: Some(RegistryKind::Vocabulary), mixed: true, evidence: ev };
        }
        if schema_for_vocab >= vocab * 3 {
            return Detected { primary: Some(RegistryKind::DataModel), mixed: true, evidence: ev };
        }
    }

    Detected { primary: None, mixed: true, evidence: ev }
}
...
pub fn parse_kind_override(s: &str) -> Option<RegistryKind> {
    match s {
        "model" | "data-model" | "datamodel" | "data_model" | "tbox" => Some(RegistryKind::DataModel),
...
mod tests {
    ...
    #[test]
    fn override_parsing() {
        assert_eq!(parse_kind_override("data-model"), Some(RegistryKind::DataModel));
        assert_eq!(parse_kind_override("tbox"), Some(RegistryKind::DataModel));
        assert_eq!(parse_kind_override("vocabulary"), Some(RegistryKind::Vocabulary));
    ...
    fn to_graph_role_mapping() {
        ...
        assert_eq!(RegistryKind::Vocabulary.to_graph_role(), GraphKind::Vocabulary);
        assert_eq!(RegistryKind::Shapes.to_graph_role(), GraphKind::Shapes);
        assert_eq!(RegistryKind::Entailment.to_graph_role(), GraphKind::Entailment);
        assert_eq!(RegistryKind::Instances.to_graph_role(), GraphKind::Instances);
    }
}
