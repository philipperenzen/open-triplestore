...
fn parse_xml(source_data: &str, iterator: &str) -> Result<Vec<Row>, String> {
    ...
    let segments: Vec<&str> = iterator.split('/')
        .filter(|s| !s.is_empty())
        .collect();
    ...
    loop {
        match reader.read_event() {
            ...
            Ok(Event::Text(e)) => {
                if let (Some(ref mut row), Some(ref field)) = (current_row.as_mut(), current_field.as_ref()) {
                    let text = e.unescape()
                        .map(|t| t.to_string())
                        .unwrap_or_default();
                    if !text.is_empty() {
            ...
            Ok(Event::End(e)) => {
                ...
                if path_matches(&element_stack, &segments) {
                    ...
                    && current_field.as_deref() == Some(name.as_str()) {
                        current_field = None;
                    }
