...
pub fn parse_rml(turtle: &str) -> Result<RmlMapping, String> {
    let store = TripleStore::in_memory()
        .map_err(|e| format!("Failed to create temp store: {e}"))?;
    store.load_str(turtle, oxigraph::io::RdfFormat::Turtle, None)
...
pub fn parse_from_store(store: &TripleStore, graph: Option<&str>) -> Result<RmlMapping, String> {
    ...
    let q = format!(
        "SELECT ?tm WHERE {{ {graph_clause} ?tm a <{RR}TriplesMap> {graph_close} }}"
    );
...
fn parse_triples_map(store: &TripleStore, tm_iri: &str, graph: Option<&str>) -> Result<TriplesMap, String> {
    let g = |pred: &str| -> Vec<String> {
        get_objects(store, tm_iri, pred, graph)
    };

    // Logical source (rml:logicalSource)
    let ls_nodes = g(&format!("{RML}logicalSource"));
    let ls_iri = ls_nodes.into_iter().next()
...
fn parse_logical_source(store: &TripleStore, ls_iri: &str, graph: Option<&str>) -> Result<LogicalSource, String> {
    ...
    let source_val = source_vals.into_iter().next()
        .ok_or("Missing rml:source")?;
    ...
    let formulation = rf_vals.into_iter().next()
...
fn parse_subject_map(store: &TripleStore, tm_iri: &str, graph: Option<&str>) -> Result<SubjectMap, String> {
    ...
    let term_map = if let Some(sm_node) = sm_nodes.into_iter().next() {
        ...
        let val = subjects.into_iter().next().ok_or("Missing rr:subjectMap or rr:subject")?;
        TermMap {
...
fn parse_pom(store: &TripleStore, pom_node: &str, graph: Option<&str>) -> Result<PredicateObjectMap, String> {
    // Predicate map
    ...
    let predicate_map = if let Some(pm) = pm_nodes.into_iter().next() {
        ...
        TermMap { kind: TermMapKind::Constant(pred), term_type: TermType::IRI, datatype: None, language: None }
    };
    ...
    let object_map = if let Some(om) = om_nodes.into_iter().next() {
        ...
        TermMap { kind: TermMapKind::Constant(obj), term_type: TermType::Literal, datatype: None, language: None }
    };

    // Optional graph map
    let graph_map = parse_optional_term_map(store, pom_node, &format!("{RR}graphMap"), graph);

    Ok(PredicateObjectMap { predicate_map, object_map, graph_map })
}

fn parse_term_map(store: &TripleStore, node: &str, graph: Option<&str>, default_type: TermType) -> Result<TermMap, String> {
    ...
    let kind = if let Some(c) = get(&format!("{RR}constant")).into_iter().next() {
        ...
        return Err(format!("TermMap <{node}> has no constant, template, or reference"));
    };

    // Determine TermType (default depends on context)
    let term_type_iris = get(&format!("{RR}termType"));
    let term_type = term_type_iris.into_iter().next()
    ...
    Ok(TermMap { kind, term_type, datatype, language })
}

fn parse_optional_term_map(store: &TripleStore, subject: &str, pred: &str, graph: Option<&str>) -> Option<TermMap> {
...
fn get_objects(store: &TripleStore, subject: &str, predicate: &str, graph: Option<&str>) -> Vec<String> {
