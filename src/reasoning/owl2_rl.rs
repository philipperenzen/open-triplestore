...
use super::common::{
    count_graph, ReasoningError, ReasoningReport, OWL2_RL_ENTAILMENT_GRAPH,
};
...
impl<'a> Owl2RLReasoner<'a> {
    ...
    fn rule_prp_irp(&self) -> Result<(), ReasoningError> {
        let q = format!(
            "ASK {{ ?p <{RDF_TYPE}> <{OWL_IRREFLEXIVE_PROP}> . ?x ?p ?x }}"
        );
        if self.ask(&q)? {
            return Err(ReasoningError::Inconsistency(
    ...
    fn rule_prp_asyp(&self) -> Result<(), ReasoningError> {
        let q = format!(
            "ASK {{ ?p <{RDF_TYPE}> <{OWL_ASYMMETRIC_PROP}> . ?x ?p ?y . ?y ?p ?x }}"
        );
        if self.ask(&q)? {
            return Err(ReasoningError::Inconsistency(
...
mod tests {
    ...
    fn test_rl_inverse() {
        let s =
            store_with("ex:parent owl:inverseOf ex:childOf . ex:a ex:parent ex:b .");
        Owl2RLReasoner::new(&s).materialize().unwrap();
        assert!(ask(
            &s,
            &format!(
    ...
    fn test_rl_scm_int_intersection_superclass() {
        let s = store_with(
            "ex:AB owl:intersectionOf ( ex:A ex:B ) .",
        );
        Owl2RLReasoner::new(&s).materialize().unwrap();
        // AB ⊑ A and AB ⊑ B
        assert!(ask(
