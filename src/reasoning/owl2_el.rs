...
use super::common::{
    count_graph, ReasoningError, ReasoningReport, OWL2_EL_ENTAILMENT_GRAPH,
};
...
impl<'a> El2Classifier<'a> {
    ...
    pub fn classify(&self) -> Result<ReasoningReport, ReasoningError> {
        ...
        loop {
            ...
            debug!("EL iteration {}: +{} triples", iterations, after.saturating_sub(before));
        ...
        info!(
            ...
            final_count, iterations, start.elapsed().as_millis()
        );
