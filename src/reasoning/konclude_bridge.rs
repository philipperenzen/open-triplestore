...
impl KoncludeReasoner {
    ...
    fn run_command(
        ...
        let output = child.wait_with_output().map_err(|e| {
            ReasoningError::Store(format!("Konclude process error: {}", e))
        })?;
...
"#;
        ...
        assert!(ttl.contains("rdfs:subClassOf"), "Should produce subClassOf triple");
    }
    ...
    fn test_unavailable_returns_not_supported() {
        ...
        let result = k.classify("@prefix owl: <http://www.w3.org/2002/07/owl#> . [] a owl:Ontology .");
        assert!(
