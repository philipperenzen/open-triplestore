...
pub fn extract_for(
    ...
    match kind {
        ...
        AssetKind::Spreadsheet => {
            ...
            let ext = file_ext(filename).unwrap_or_default();
            if ext == "csv" || ext == "tsv" {
                meta.row_count = Some(csv_row_count(bytes, if ext == "tsv" { b'\t' } else { b',' }));
...
fn zip_metadata(bytes: &[u8], meta: &mut AssetMetadata) {
    ...
    if zip_ratio_is_suspicious(total, bytes.len() as u64) {
        tracing::warn!("asset zip: suspicious uncompressed ratio ({} from {} bytes); size withheld",
                       total, bytes.len());
    } else {
...
fn spreadsheet_metadata(bytes: &[u8], meta: &mut AssetMetadata) {
    ...
    use std::io::Cursor;
    let Ok(mut wb) = calamine::open_workbook_from_rs::<Xlsx<_>, _>(Cursor::new(bytes.to_vec())) else {
        return;
    };
...
pub fn scan_clamav(bytes: &[u8], clamd_addr: &str) -> ScanVerdict {
    ...
    for chunk in bytes.chunks(8192) {
        ...
        if stream.write_all(&len).and_then(|_| stream.write_all(chunk)).is_err() {
    ...
    if resp.contains("FOUND") {
        let sig = resp.trim().trim_end_matches(" FOUND")
            .rsplit(": ").next().unwrap_or("unknown").to_string();
...
fn exif_metadata(bytes: &[u8], meta: &mut AssetMetadata) {
    ...
    if let Some(raw) = exif.get_field(Tag::DateTimeOriginal, In::PRIMARY)
...
fn symphonia_metadata(bytes: &[u8], filename: &str, meta: &mut AssetMetadata) {
    ...
    let Some(track) = probed.format.default_track() else { return };
...
mod tests {
    ...
    fn kml_coordinates_bbox() {
        ...
        let m = extract_for(AssetKind::GeoData, kml, "application/vnd.google-earth.kml+xml", "x.kml");
        let wkt = m.bbox_wkt.unwrap();
    ...
    fn model3d_format_from_extension() {
        let m = extract_for(AssetKind::Model3D, b"glTF binary...", "model/gltf-binary", "scene.glb");
        assert_eq!(m.format.as_deref(), Some("glb"));
    }

    #[test]
    fn generic_has_no_detail() {
        let m = extract_for(AssetKind::Generic, b"\x00\x01", "application/octet-stream", "blob");
    ...
    fn pointcloud_las_point_count() {
        ...
        let m = extract_for(AssetKind::PointCloud, &las, "application/vnd.las", "survey.las");
    ...
    fn cad_format_from_extension() {
        let m = extract_for(AssetKind::CadModel, b"ISO-10303-21;", "application/x-step", "model.ifc");
    ...
    fn tsv_uses_tab_delimiter() {
        let tsv = b"a\tb\tc\n1\t2\t3\n";
        let m = extract_for(AssetKind::Spreadsheet, tsv, "text/tab-separated-values", "x.tsv");
