...
fn is_cad(content_type: &str, ext: &str) -> bool {
    matches!(
        ...
        ext,
        "dwg" | "dxf" | "ifc" | "ifczip" | "rvt" | "step" | "stp"
    )
}
...
mod tests {
    ...
    fn classify_pdf() {
        let pdf = b"%PDF-1.4\n1 0 obj<<>>endobj\n";
        assert_eq!(
            classify("application/pdf", "doc.pdf", pdf),
            AssetKind::Document
        );
    }
    ...
    fn classify_geojson_by_extension_and_content() {
        ...
        assert_eq!(
            classify("application/json", "x.geojson", gj),
            AssetKind::GeoData
        );
        // Even named .json, the structural sniff catches it.
        assert_eq!(
            classify("application/json", "x.json", gj),
            AssetKind::GeoData
        );
    }
    ...
    fn classify_3d_model_by_extension() {
        assert_eq!(
            classify("application/octet-stream", "m.glb", b"glTF..."),
            AssetKind::Model3D
        );
    ...
    fn classify_pointcloud_by_extension() {
        assert_eq!(
            classify("application/octet-stream", "survey.las", b"LASF"),
            AssetKind::PointCloud
        );
    ...
    fn classify_cad_by_extension() {
        assert_eq!(
            classify("application/octet-stream", "plan.dwg", b"AC10"),
            AssetKind::CadModel
        );
        assert_eq!(
            classify("", "model.ifc", b"ISO-10303-21;"),
            AssetKind::CadModel
        );
        // A .dxf is CAD, not the generic 3D bucket.
        assert_eq!(
            classify("", "drawing.dxf", b"0\nSECTION"),
            AssetKind::CadModel
        );
    }
    ...
    fn classify_spreadsheet_by_extension() {
        assert_eq!(
            classify("text/csv", "log.csv", b"a,b,c\n1,2,3"),
            AssetKind::Spreadsheet
        );
        ...
        assert_eq!(
            classify("", "book.xlsx", b"PK\x03\x04"),
            AssetKind::Spreadsheet
        );
        assert_eq!(
            classify("", "sheet.ods", b"PK\x03\x04"),
            AssetKind::Spreadsheet
        );
    }
    ...
    fn classify_archive_by_extension_and_magic() {
        assert_eq!(
            classify("", "bundle.zip", b"PK\x03\x04"),
            AssetKind::Archive
        );
        assert_eq!(
            classify("application/gzip", "logs.gz", b"\x1f\x8b"),
            AssetKind::Archive
        );
    ...
    fn classify_generic_fallback() {
        assert_eq!(
            classify("application/octet-stream", "blob", b"\x00\x01\x02"),
            AssetKind::Generic
        );
    }
    ...
    fn sanitize_keeps_ordinary_names() {
        assert_eq!(
            sanitize_filename("bridge-survey_2026.las"),
