...
mod tests {
    ...
    fn admin_token(state: &AppState) -> String {
        state.auth_db
            .create_user("admin0", "admin", "admin@test.com", "hash", SystemRole::SuperAdmin)
            .unwrap();
        issue_access_token(&JwtConfig::new(TEST_JWT_SECRET.to_string(), 30, 30), "admin0", "admin", "super_admin")
            .unwrap()
    }
    ...
    async fn test_service_description_hides_private_graph() {
        ...
        state.auth_db
            .create_dataset("d1", "Priv", None, OwnerType::Organisation, "o1", Visibility::Private, None)
            .unwrap();
        state.auth_db.add_dataset_graph("d1", "https://private.example.com/graph").unwrap();
        state.store
            .update("INSERT DATA { GRAPH <https://private.example.com/graph> { <s:s> <p:p> <o:o> } }")
            .unwrap();
        ...
        assert!(body.contains("void:triples 0"),
    ...
    async fn test_service_description_shows_public_graph() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db
            .create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None)
            .unwrap();
        state.auth_db.add_dataset_graph("d1", "https://public.example.com/graph").unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("public.example.com"),
        ...
        let resp = test_app(test_state())
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from("update=INSERT+DATA+%7B+%3Cs%3Aa%3E+%3Cp%3Ab%3E+%3Co%3Ac%3E+%7D"))
                    .unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_sparql_query_cannot_read_private_graph() {
        ...
        state.auth_db
            .create_dataset("d1", "Priv", None, OwnerType::Organisation, "o1", Visibility::Private, None)
            .unwrap();
        state.auth_db.add_dataset_graph("d1", "https://secret.example.com/graph").unwrap();
        state.store
    ...
    async fn test_sparql_query_can_read_public_graph() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("open data"),
    ...
    async fn test_csp_header_on_all_responses() {
        ...
        assert!(csp.to_str().unwrap().contains("default-src 'self'"),
            "CSP must contain default-src 'self', got: {:?}", csp);
    }
    ...
    async fn test_internal_error_message_not_leaked() {
        ...
        assert!(resp.status().is_client_error(),
            "Invalid SPARQL must return a 4xx, got {}", resp.status());
        let body = body_text(resp.into_body()).await;
        assert!(!body.to_lowercase().contains("oxigraph"),
    ...
    async fn test_endpoint_acl_deny_blocks_user() {
        ...
        state.auth_db
            .create_user("u1", "bob", "bob@test.com", "hash", crate::auth::models::SystemRole::User)
            .unwrap();
        ...
        state.auth_db
            .create_user("adm", "admin2", "adm@test.com", "hash", crate::auth::models::SystemRole::Admin)
            .unwrap();

        // Insert a deny rule for user u1 on /api/browse/*
        state.auth_db.create_endpoint_acl_rule(
            "rule1", "user", "u1",
            "/api/browse/**", "*", "deny", 10, "adm",
        ).unwrap();
    ...
    async fn test_endpoint_acl_allow_no_effect_on_others() {
        let state = test_state();
        state.auth_db
            .create_user("u1", "alice2", "alice2@test.com", "hash", crate::auth::models::SystemRole::User)
            .unwrap();
        state.auth_db
            .create_user("u2", "carol", "carol@test.com", "hash", crate::auth::models::SystemRole::User)
            .unwrap();
        state.auth_db
            .create_user("adm", "admin3", "adm3@test.com", "hash", crate::auth::models::SystemRole::Admin)
            .unwrap();
    ...
    async fn test_graph_acl_grants_read_access() {
        let state = test_state();
        state.auth_db.create_user("u1", "dave", "dave@test.com", "hash", crate::auth::models::SystemRole::User).unwrap();
        state.auth_db.create_user("adm", "admin4", "adm4@test.com", "hash", crate::auth::models::SystemRole::Admin).unwrap();
        state.auth_db.create_organisation("o1", "Acme4", "acme4", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Private", None,
            crate::auth::models::OwnerType::Organisation, "o1",
            crate::auth::models::Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "https://secret.example.com/restricted").unwrap();
        state.store
        ...
        state.auth_db.grant_graph_permission(
            "gacl1",
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("graph-acl-value"),
    ...
    async fn test_graph_acl_read_only_blocks_write() {
        let state = test_state();
        state.auth_db.create_user("u1", "eve", "eve@test.com", "hash", crate::auth::models::SystemRole::User).unwrap();
        state.auth_db.create_user("adm", "admin5", "adm5@test.com", "hash", crate::auth::models::SystemRole::Admin).unwrap();
    ...
    async fn test_acl_endpoints_require_admin() {
        let state = test_state();
        state.auth_db
            .create_user("u1", "frank", "frank@test.com", "hash", crate::auth::models::SystemRole::User)
            .unwrap();
        let user_token = issue_access_token(
            &JwtConfig::new(TEST_JWT_SECRET.to_string(), 30, 30),
            "u1", "frank", "user",
