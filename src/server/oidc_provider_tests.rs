...
mod tests {
    ...
    fn token_form(code: &str, verifier: &str) -> String {
        format!(
            "grant_type=authorization_code&code={code}&redirect_uri={}&client_id=web-app&code_verifier={verifier}",
    ...
    async fn authorize_checks_client_redirect_and_consent() {
        ...
        let bad2 = serde_json::json!({
            "client_id": "web-app", "redirect_uri": "http://evil.example/cb",
            "code_challenge": challenge, "code_challenge_method": "S256",
        });
        let (s, _) = send(
            &st,
        ...
        let check = serde_json::json!({
            "client_id": "web-app", "redirect_uri": REDIRECT,
        ...
        assert_eq!(body["client_name"], "Web App");
    ...
    async fn pkce_and_redirect_are_enforced() {
        ...
        let code = mint_code(&st, &session, &challenge).await;
        let form = format!(
            "grant_type=authorization_code&code={code}&redirect_uri=http%3A%2F%2Fevil.example%2Fcb&client_id=web-app&code_verifier={}",
            "a".repeat(43),
        );
        let (s, body) = send(
            &st,
