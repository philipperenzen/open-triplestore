...
pub fn graph_store_read_routes() -> Router<AppState> {
    Router::new()
        .route("/store", get(graph_store_get))
}
...
async fn sparql_query_get(
    ...
    execute_query(&state, user.as_deref(), &query, accept, params.entailment.as_deref()).await
}
...
async fn sparql_post(
    ...
    if content_type.starts_with("application/sparql-query") {
        ...
        if user.is_none() {
            return Err(AppError::Unauthorized("Authentication required for SPARQL updates".to_string()));
        }
        ...
        let query = params.iter().find(|(k, _)| k == "query").map(|(_, v)| v.as_str());
        let update = params.iter().find(|(k, _)| k == "update").map(|(_, v)| v.as_str());
...
async fn execute_query(
    ...
    if let Some(u) = user {
        match state.auth_db.get_graph_acl_readable_iris(&u.user_id, u.role.as_str()) {
            Ok(acl_iris) => {
                for iri in acl_iris {
    ...
    let query = if let Some(ref idx) = state.text_index {
        ...
        query_after_regex_pushdown = crate::text_search::sparql_fn::preprocess_regex_pushdown(
            &query_after_text_search,
            idx,
        );
    ...
    let query = if let Some(regime) = entailment {
        ...
        if let Some(iri) = graph_iri {
            entailment_query = inject_from_clauses(
                query,
                &format!("FROM <{iri}>\nFROM NAMED <{iri}>\n"),
            );
    ...
    let effective_query_str = effective_query
        .as_deref()
        .unwrap_or(query)
        .to_string();
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut writer = ChannelWriter { tx: chunk_tx.clone() };
...
fn authorize_update(
    ...
    Ok((access.write_iris.into_iter().collect(), access.requires_admin))
}
...
async fn graph_store_get(
    ...
    if let Some(iri) = params.graph_iri() {
        ...
        if !is_admin {
            ...
            let allowed = check_graph_read_access(&state, user_id, iri)
                .map_err(|e| AppError::Internal(e.to_string()))?;
            if !allowed {
                return Err(AppError::Unauthorized("Access denied to this graph".to_string()));
            }
        }
    }
    ...
    if needs_label_filter {
        ...
        let filtered = apply_triple_label_filter(
            user.as_deref(),
            bytes,
            iri,
            format,
...
fn apply_triple_label_filter(
    ...
    let filtered_lines: Vec<&str> = nt_str
        ...
        .filter(|l| !l.trim().is_empty() && !l.starts_with('#'))
        .enumerate()
        .filter_map(|(i, line)| if allowed_indices.contains(&i) { Some(line) } else { None })
        .collect();
    ...
    if !filtered_nt.is_empty() {
        out_store.load_str(&filtered_nt, oxigraph::io::RdfFormat::NTriples, Some(graph_iri))?;
    }
...
pub(crate) fn validate_on_write(
    ...
    // during the transition.
    {
        ...
        if let Err(report) = crate::shacl_studio::gate::check_write_gates(
            &state.store, &state.auth_db, &studio, &state.base_url, iri, data, format,
        ) {
    ...
    let shapes_data = state.store.dump(oxigraph::io::RdfFormat::Turtle, Some(&shapes_graph_iri))
    ...
    temp.load_str(&shapes_turtle, oxigraph::io::RdfFormat::Turtle, Some(&shapes_graph_iri))
...
async fn graph_store_put(
    ...
    state.store.graph_store_put(params.graph_iri(), &data, format)?;
    #[cfg(feature = "text-search")]
    state.mark_text_dirty();
    Ok(StatusCode::NO_CONTENT.into_response())
}
...
async fn graph_store_post(
    ...
    state.store.graph_store_post(params.graph_iri(), &data, format)?;
    #[cfg(feature = "text-search")]
    state.mark_text_dirty();
    Ok(StatusCode::NO_CONTENT.into_response())
}
...
async fn service_description_handler(
    ...
    let accessible_graph_iris: Vec<String> = if is_admin {
        state.store.named_graphs().unwrap_or_default()
            .into_iter().map(|g| g.as_str().to_string()).collect()
    } else {
        let cached_graphs = state
            .auth_db
            .get_accessible_graph_iris_cached(user_id)
...
async fn health_check(State(state): State<AppState>) -> impl IntoResponse {
    // Triplestore
    let (store_ok, store_triples, store_graphs) = match (state.store.len(), state.store.named_graphs()) {
    ...
    let http_status = if overall == "ok" { StatusCode::OK } else { StatusCode::SERVICE_UNAVAILABLE };
...
fn validate_iri(iri: &str) -> Result<(), AppError> {
    if iri.contains('>') || iri.contains('<') || iri.contains('"') || iri.contains('`') || iri.contains('^') || iri.contains('\n') || iri.contains('\r') {
        return Err(AppError::BadRequest("Invalid IRI: contains disallowed characters".to_string()));
    }
...
fn validate_substring(val: &str) -> Result<(), AppError> {
    if val.is_empty() {
        return Err(AppError::BadRequest("Substring filter is empty".to_string()));
    }
    if val.len() > 200 {
        return Err(AppError::BadRequest("Substring filter is too long".to_string()));
    }
    if val.contains('\\')
        || val.contains('\n')
        || val.contains('\r')
...
fn validate_regex(val: &str) -> Result<(), AppError> {
    ...
    if val.contains('\n') || val.contains('\r') {
        return Err(AppError::BadRequest("Regex filter contains disallowed characters".to_string()));
    }
    Ok(())
}
...
enum QTok { And, Or, Xor, Not, LParen, RParen, Term(String) }
...
fn tokenize_q(q: &str) -> Vec<QTok> {
    ...
    while i < chars.len() {
        ...
        if c == '(' { toks.push(QTok::LParen); i += 1; continue; }
        if c == ')' { toks.push(QTok::RParen); i += 1; continue; }
        if c == '"' {
            i += 1;
            let start = i;
...
struct QParser { toks: Vec<QTok>, pos: usize }
impl QParser {
    fn peek(&self) -> Option<QTok> { self.toks.get(self.pos).cloned() }
    fn parse_or(&mut self) -> Result<String, ()> {
        let mut left = self.parse_xor()?;
        while self.peek() == Some(QTok::Or) { self.pos += 1; let r = self.parse_xor()?; left = format!("({left} || {r})"); }
        Ok(left)
    }
    fn parse_xor(&mut self) -> Result<String, ()> {
        let mut left = self.parse_and()?;
        while self.peek() == Some(QTok::Xor) { self.pos += 1; let r = self.parse_and()?; left = format!("(({left} || {r}) && !({left} && {r}))"); }
        Ok(left)
    }
    fn parse_and(&mut self) -> Result<String, ()> {
        ...
        loop {
            match self.peek() {
                Some(QTok::And) => { self.pos += 1; let r = self.parse_not()?; left = format!("({left} && {r})"); }
                // Implicit AND when an atom (term / NOT / "(") follows with no operator.
                Some(QTok::Term(_)) | Some(QTok::Not) | Some(QTok::LParen) => { let r = self.parse_not()?; left = format!("({left} && {r})"); }
                _ => break,
            }
        }
        Ok(left)
    ...
    fn parse_not(&mut self) -> Result<String, ()> {
        if self.peek() == Some(QTok::Not) { self.pos += 1; let a = self.parse_not()?; Ok(format!("(!{a})")) }
    ...
    fn parse_atom(&mut self) -> Result<String, ()> {
        match self.peek() {
            Some(QTok::LParen) => {
                self.pos += 1;
                let e = self.parse_or()?;
                if self.peek() != Some(QTok::RParen) { return Err(()); }
                self.pos += 1;
                Ok(e)
            }
            Some(QTok::Term(t)) => { self.pos += 1; Ok(q_leaf(&t)) }
            _ => Err(()),
        }
    }
}
...
fn build_q_filter(q: &str) -> Result<Option<String>, AppError> {
    ...
    for t in &toks {
        if let QTok::Term(s) = t { validate_substring(s)?; }
    }
    let mut p = QParser { toks, pos: 0 };
    match p.parse_or() {
        Ok(expr) if p.pos == p.toks.len() => Ok(Some(expr)),
        _ => Err(AppError::BadRequest(
            "Invalid search syntax — check AND/OR/XOR/NOT operators and matching parentheses".to_string(),
        )),
    }
}
...
fn resolve_scope_graphs(
    ...
    let versions_map = params.versions.as_deref().map(parse_versions_map).unwrap_or_default();

    if let Some(graph) = params.graph.as_ref() {
        validate_iri(graph)?;
        if !is_admin {
            let cached = state.auth_db
                .get_accessible_graph_iris_cached(user_id)
                .map_err(|e| AppError::Internal(e.to_string()))?;
    ...
    if params.dataset_ids.is_some() || params.org_id.is_some() || params.dataset_id.is_some() {
        let ds_ids: Vec<String> = if let Some(ref ids_csv) = params.dataset_ids {
            ids_csv.split(',').map(str::trim).filter(|s| !s.is_empty()).map(str::to_string).collect()
        ...
        return Ok(if scoped.is_empty() { ScopeGraphs::Empty } else { ScopeGraphs::Set(scoped) });
    }
...
pub async fn browse_graphs(
    ...
    let accessible_set: std::collections::HashSet<String> = if is_admin {
        state.store.named_graphs()?.iter().map(|g| g.as_str().to_string()).collect()
    } else {
        let cached_graphs = state.auth_db
            .get_accessible_graph_iris_cached(user_id)
            .map_err(|e| AppError::Internal(e.to_string()))?;
    ...
    for id in ds_ids {
        if let Some(v) = versions.get(id) {
            if is_pinned_version(v) {
                ...
                let ds = state.auth_db.get_dataset(id)
                    .map_err(|e| AppError::Internal(e.to_string()))?;
                let allowed = match &ds {
                    Some(d) => state.auth_db.can_access_dataset(user_id, d)
                        .map_err(|e| AppError::Internal(e.to_string()))?,
                    None => false,
                };
                if !allowed { continue; }
        ...
        let live = state.auth_db.list_dataset_graphs(id)
            .map_err(|e| AppError::Internal(e.to_string()))?;
        match &accessible {
...
fn is_authorized_version_graph(
    ...
    for (ds_id, v) in versions {
        if !is_pinned_version(v) { continue; }
        if let Some(snap) = version_snapshot_graphs(state, ds_id, Some(v.as_str()))? {
            if snap.iter().any(|g| g == graph) {
                let ds = state.auth_db.get_dataset(ds_id)
                    .map_err(|e| AppError::Internal(e.to_string()))?;
                if let Some(d) = ds {
                    if state.auth_db.can_access_dataset(user_id, &d)
                        .map_err(|e| AppError::Internal(e.to_string()))? {
                        return Ok(true);
                    }
                }
...
pub async fn browse_triples(
    ...
    if contains_mode {
        if let Some(ref s) = params.subject { validate_substring(s)?; }
        if let Some(ref p) = params.predicate { validate_substring(p)?; }
        if let Some(ref o) = params.object { validate_substring(o)?; }
        if let Some(ref g) = params.graph { validate_substring(g)?; }
    } else {
        if let Some(ref s) = params.subject { validate_iri(s)?; }
        if let Some(ref p) = params.predicate { validate_iri(p)?; }
        ...
        if let Some(ref g) = params.graph { validate_iri(g)?; }
    }
    ...
    if chips.len() > 64 {
        return Err(AppError::BadRequest("too many filter chips (max 64)".to_string()));
    }
    ...
    for chip in &chips {
        if chip.value.is_empty() { continue; }
        ...
        match chip.field.as_str() {
            ...
            _ => return Err(AppError::BadRequest(
                "filter field must be subject, predicate, object, graph, or vocabulary".to_string())),
        }
    }
    ...
    let exact_graph = if contains_mode || has_graph_chips { None } else { params.graph.as_ref() };
    ...
    if contains_mode {
        ...
        if let Some(ref s) = params.subject {
            filters.push(format!("FILTER(CONTAINS(LCASE(STR(?s)), LCASE(\"{}\")))", s));
        }
        if let Some(ref p) = params.predicate {
            filters.push(format!("FILTER(CONTAINS(LCASE(STR(?p)), LCASE(\"{}\")))", p));
        }
        if let Some(ref o) = params.object {
            filters.push(format!("FILTER(CONTAINS(LCASE(STR(?o)), LCASE(\"{}\")))", o));
        }
        if let Some(ref g) = params.graph {
            filters.push(format!("FILTER(CONTAINS(LCASE(STR(?g)), LCASE(\"{}\")))", g));
        }
    } else {
        if let Some(ref s) = params.subject {
    ...
    if !subj_clauses.is_empty()  { filters.push(format!("FILTER({})", subj_clauses.join(" || "))); }
    if !pred_clauses.is_empty()  { filters.push(format!("FILTER({})", pred_clauses.join(" || "))); }
    if !obj_clauses.is_empty()   { filters.push(format!("FILTER({})", obj_clauses.join(" || "))); }
    if !graph_clauses.is_empty() { filters.push(format!("FILTER({})", graph_clauses.join(" || "))); }
    if !vocab_clauses.is_empty() { filters.push(format!("FILTER({})", vocab_clauses.join(" || "))); }
    ...
    let fc = if filter_clause.is_empty() { String::new() } else { format!(" . {}", filter_clause) };
...
pub async fn browse_facets(
    ...
    let (open, close) = match resolve_scope_graphs(&state, &params, user_id, is_admin)? {
        ...
        ScopeGraphs::Set(graphs) => {
            ...
            (format!("VALUES ?g {{ {values}}} GRAPH ?g {{ "), " }".to_string())
        }
    };
...
pub async fn browse_resource(
    ...
    let mut search_graphs: Vec<String> = if let Some(ref graph) = params.graph {
        ...
        let visible: &std::collections::HashSet<String> =
            if is_admin { &cached_graphs.1 } else { &cached_graphs.0 };
        if visible.is_empty() {
    ...
    if let Some(bnode_label) = iri.strip_prefix("_:") {
        let (outgoing, bnodes, incoming) =
            resolve_blank_node(state.store.store(), bnode_label, &search_graphs, MAX_BNODE_DEPTH);
        return Ok(Json(serde_json::json!({
            "iri": iri,
            "label": null,
            "outgoing": outgoing,
    ...
    if let Some(ds_id) = dataset_id_from_iri(iri, &state.base_url) {
        if let Ok(Some(ds)) = state.auth_db.get_dataset(&ds_id) {
            if state.auth_db.can_access_dataset(user_id, &ds).unwrap_or(false) {
                search_graphs.push(crate::auth::dataset_graph::dataset_metadata_graph_iri(&ds_id));
    ...
    let incoming_query = build_query(&if let Some(ref graph) = params.graph {
        format!("SELECT ?s ?p WHERE {{ GRAPH <{}> {{ ?s ?p <{}> }} }} ORDER BY ?s ?p", graph, iri)
    } else {
    ...
    let label = if let Ok(oxigraph::sparql::QueryResults::Solutions(mut solutions)) = state.store.query(&label_query) {
        solutions.next()
            .and_then(|r| r.ok())
...
pub async fn browse_stats(
    ...
    let (total_triples, named_graph_count) = if is_admin {
        ...
        let total: usize = if accessible.is_empty() {
            ...
            if let Ok(oxigraph::sparql::QueryResults::Solutions(mut solutions)) = state.store.query(&count_query) {
                solutions.next()
                    .and_then(|r| r.ok())
                    .and_then(|s| s.get("count").and_then(|v| {
                        let str_val = v.to_string();
                        str_val.trim_matches('"').split('"').next()
...
pub async fn browse_suggest(
    ...
    if (prefix.starts_with("http://") || prefix.starts_with("https://") || prefix.starts_with("urn:"))
        && prefix.contains('>') {
    ...
    let var = match params.field.as_str() {
        ...
        "object"    => "o",
        "graph"     => "g",
        _ => return Err(AppError::BadRequest("field must be subject, predicate, object, or graph".to_string())),
    };
    ...
    let pattern = if params.field == "graph" {
        ...
        format!("GRAPH ?g {{ ?s {pred} ?o{} }}", filter, pred = predicate_term)
    };
    ...
    let scoped_graphs: Option<Vec<String>> = if let Some(ds_id) = params.dataset.as_deref() {
        match state.auth_db.get_dataset(ds_id).map_err(|e| AppError::Internal(e.to_string()))? {
            Some(ds) if state.auth_db.can_access_dataset(user_id, &ds).unwrap_or(false) => Some(
                state.auth_db
...
fn version_snapshot_graphs(
    ...
    let ver = crate::dataset_versions::registry::get_version(
        &state.store, state.base_url.as_str(), dataset_id, v,
    )
...
pub async fn dataset_sparql_query(
    ...
    execute_dataset_query(&state, user.as_deref(), &dataset_id, &service_slug, &query, params.version.as_deref(), accept).await
}
...
pub async fn dataset_sparql_post(
    ...
    if content_type.starts_with("application/sparql-query") {
        execute_dataset_query(&state, user.as_deref(), &dataset_id, &service_slug, &body_str, version, accept).await
    } else if content_type.starts_with("application/x-www-form-urlencoded") {
        let form: Vec<(String, String)> = url::form_urlencoded::parse(body_str.as_bytes())
            .map(|(k, v)| (k.to_string(), v.to_string()))
            .collect();
        let query = form.iter().find(|(k, _)| k == "query").map(|(_, v)| v.as_str())
        ...
        let form_version = form.iter().find(|(k, _)| k == "version").map(|(_, v)| v.as_str());
        execute_dataset_query(&state, user.as_deref(), &dataset_id, &service_slug, query, form_version.or(version), accept).await
    } else {
        execute_dataset_query(&state, user.as_deref(), &dataset_id, &service_slug, &body_str, version, accept).await
    }
}
...
async fn execute_dataset_query(
    ...
    let dataset = state.auth_db.get_dataset(dataset_id)
        .map_err(|e| AppError::Internal(e.to_string()))?
        .ok_or_else(|| AppError::NotFound("Dataset not found".to_string()))?;

    let user_id = user.map(|u| u.user_id.as_str());
    if !state.auth_db.can_access_dataset(user_id, &dataset)
        .map_err(|e| AppError::Internal(e.to_string()))? {
        return Err(AppError::NotFound("Dataset not found".to_string()));
    }

    // Find the service and its graphs
    let service = state.auth_db.get_sparql_service_by_slug(dataset_id, service_slug)
        .map_err(|e| AppError::Internal(e.to_string()))?
    ...
    let graphs = if let Some(snapshot) = version_snapshot_graphs(state, dataset_id, version)? {
        ...
        let service_graphs = state.auth_db.list_service_graphs(&service.id)
            .map_err(|e| AppError::Internal(e.to_string()))?;
        if service_graphs.is_empty() {
            state.auth_db.list_dataset_graphs(dataset_id)
                .map_err(|e| AppError::Internal(e.to_string()))?
        } else {
            service_graphs
        }
    ...
    if let Some(graph_var) = detect_graph_listing_query(query) {
        let sorted: Vec<String> = if graphs.is_empty() {
            ...
            let mut filtered: Vec<String> = graphs
                .iter()
                .filter(|iri| state.store.graph_count_cached(Some(iri.as_str())).unwrap_or(1) > 0)
                .cloned()
                .collect();
            filtered.sort();
            filtered
...
pub(crate) async fn run_scoped_sparql(
    ...
    tokio::task::spawn_blocking(move || {
        let results = match store.query(&final_query_str) {
            Ok(r) => r,
            Err(e) => { let _ = ct_tx.send(Err(AppError::from(e))); return; }
        };

        enum Mode { Tabular(ResultFormat), Graph(GraphFormat) }
        let mode = match &results {
            oxigraph::sparql::QueryResults::Solutions(_)
            | oxigraph::sparql::QueryResults::Boolean(_) => {
                Mode::Tabular(negotiate_result_format(&accept_owned))
        ...
        let content_type = match mode { Mode::Tabular(f) => f.content_type(), Mode::Graph(f) => f.content_type() };
        if ct_tx.send(Ok(content_type)).is_err() { return; }
        ...
        if let Err(msg) = result {
            let _ = chunk_tx.blocking_send(Err(std::io::Error::new(std::io::ErrorKind::Other, msg)));
        }
    });
...
fn detect_graph_listing_query(query: &str) -> Option<String> {
    ...
    let where_pos = q_upper[select_pos..].find("WHERE").map(|i| select_pos + i)?;
...
fn inject_from_clauses(query: &str, from_clauses: &str) -> String {
    ...
    let sep = if head.is_empty() || head.ends_with(char::is_whitespace) { "" } else { "\n" };
...
fn extract_and_strip_dataset(head: &str) -> (Vec<String>, String) {
    ...
    (iris, String::from_utf8(out).unwrap_or_else(|_| head.to_string()))
}
...
mod query_scoping_tests {
    ...
    fn finds_where_case_insensitively_at_word_boundary() {
        ...
        assert_eq!(first_top_level_where("SELECT ?whereabouts { ?s ?p ?o }"), None);
    }
    ...
    fn strips_caller_from_clauses_and_collects_iris() {
        ...
        assert!(!head.contains("FROM"), "dataset clauses must be removed: {head:?}");
    ...
    fn scopes_query_without_where_keyword() {
        ...
        assert!(from_pos < brace_pos, "FROM must precede the group `{{`: {scoped}");
    }
    ...
    fn from_inside_string_literal_is_not_stripped() {
        ...
        assert!(scoped.contains("\"data FROM <x>\""), "literal corrupted: {scoped}");
    }
}
...
mod blank_node_resolution_tests {
    ...
    fn respects_graph_authorization_scope() {
        ...
        let label = match store
            ...
            .unwrap()
        {
            QueryResults::Solutions(mut sols) => match sols.next().unwrap().unwrap().get("o").unwrap() {
                oxigraph::model::Term::BlankNode(bn) => bn.as_str().to_string(),
        ...
        assert!(outgoing.is_empty(), "must not leak across graphs: {outgoing:?}");
        assert!(incoming.is_empty(), "must not leak across graphs: {incoming:?}");
    }
}
...
mod dataset_iri_tests {
    ...
    fn parses_canonical_dataset_iri() {
        ...
        assert_eq!(dataset_id_from_iri("http://localhost:7878/dataset/spatial", b), Some("spatial".into()));
        // Trailing slash on the configured base is tolerated.
        assert_eq!(dataset_id_from_iri("http://localhost:7878/dataset/bridge-inventory", "http://localhost:7878/"), Some("bridge-inventory".into()));
    }
    ...
    fn rejects_non_dataset_iris() {
        ...
        assert_eq!(dataset_id_from_iri("http://localhost:7878/dataset/spatial/geo", b), None);
        // The aggregate VoID dataset and empty id are not per-dataset IRIs.
        assert_eq!(dataset_id_from_iri("http://localhost:7878/dataset", b), None);
        assert_eq!(dataset_id_from_iri("http://localhost:7878/dataset/", b), None);
        // A plural REST-ish path must not be mistaken for the singular IRI.
        assert_eq!(dataset_id_from_iri("http://localhost:7878/datasets/spatial", b), None);
    }
}
...
fn insert_asset_triples(
    ...
    let dataset_iri_str = format!("{}/dataset/{}", state.base_url.trim_end_matches('/'), dataset_id);
    let display_title = asset.title.as_deref().unwrap_or(&asset.filename);
    let title_escaped = sparql_escape(display_title);
    let media_type = asset.content_type.split(';').next()
        .unwrap_or(&asset.content_type).trim();
    let media_iri = format!("https://www.iana.org/assignments/media-types/{}", media_type);
    ...
    if let Some(w) = meta.width {
        detail.push_str(&format!("\n        <http://schema.org/width> \"{}\"^^xsd:integer ;", w));
    }
    if let Some(h) = meta.height {
        detail.push_str(&format!("\n        <http://schema.org/height> \"{}\"^^xsd:integer ;", h));
    }
    if let Some(p) = meta.pages {
        detail.push_str(&format!(
            "\n        <http://schema.org/numberOfPages> \"{}\"^^xsd:integer ;", p));
    }
    if let Some(d) = meta.duration_secs {
        detail.push_str(&format!(
            "\n        <http://schema.org/duration> \"PT{}S\"^^xsd:duration ;", d));
    }
    if let Some(fmt) = &meta.format {
        detail.push_str(&format!(
            "\n        <http://schema.org/encodingFormat> \"{}\" ;", sparql_escape(fmt)));
    }
    ...
    if let Some(bbox) = &meta.bbox_wkt {
        ...
        detail.push_str(&format!(
            "\n        dcat:bbox \"{}\"^^geo:wktLiteral ;", sparql_escape(&lit)));
    ...
    if meta.is_panorama {
        detail.push_str(&format!(
            "\n        <{}panorama> true ;\
             \n        <http://schema.org/encodingFormat> \"equirectangular\" ;", ASSET_NS));
    }
    // Image capture timestamp (EXIF DateTimeOriginal).
    if let Some(ts) = &meta.captured_at {
        detail.push_str(&format!(
            "\n        <http://schema.org/dateCreated> \"{}\"^^xsd:dateTime ;", sparql_escape(ts)));
    }
...
pub async fn upload_asset(
    ...
    if !state.auth_db.can_write_dataset(&current_user.user_id, &dataset)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        return Err((StatusCode::FORBIDDEN, "Write access required".to_string()));
    }

    if !state.object_store.is_configured() {
        return Err((StatusCode::SERVICE_UNAVAILABLE, "S3 storage not configured".to_string()));
    }
    ...
    if let Some(mut field) = multipart.next_field().await
        ...
        let declared_type = field.content_type().unwrap_or("application/octet-stream").to_string();
        ...
        #[cfg(feature = "asset-clamav")]
        {
            ...
            if !verdict.allows_storage() {
                ...
                tracing::warn!(
                    ...
                    dataset_id, file_name, signature
                );
                return Err((
        ...
        state.object_store.upload(&s3_key, data, &content_type).await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

        let asset = state.auth_db.create_asset(
            &asset_id, &dataset_id, &file_name, &content_type, &s3_key, size, &current_user.user_id, false,
        ).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        ...
        if let Err(e) = state.auth_db.add_dataset_graph(&dataset_id, &graph) {
            tracing::warn!("Failed to register assets graph for dataset {}: {}", dataset_id, e);
        }
        let iri = asset_iri(&state.base_url, &dataset_id, &asset.id);
        ...
        if let Some(png) = thumb_png {
            if let Err(e) = store_thumbnail(&state, &dataset_id, &asset.id, &iri,
                                            png, &current_user.user_id).await {
...
async fn store_thumbnail(
    ...
    let thumb = state
        .auth_db
        .create_asset(&thumb_id, dataset_id, &filename, "image/png", &s3_key, size, user_id, false)
        .map_err(|e| e.to_string())?;
    // Type the thumbnail node (schema:ImageObject + dimensions), then link parent → thumb.
    let kind = crate::assets::AssetKind::Image;
    let (data, _ct) = state.object_store.download(&s3_key).await.map_err(|e| e.to_string())?;
    ...
    let update = format!(
        ...
        g = graph, p = parent_iri, t = thumb_iri,
    );
    state
        .store
...
pub async fn list_assets(
    ...
    let assets = state.auth_db.list_dataset_assets(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn download_asset(
    ...
    if !state.auth_db.can_access_dataset(Some(&current_user.user_id), &dataset)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
        return Err((StatusCode::FORBIDDEN, "Access denied".to_string()));
    }

    let asset = state.auth_db.get_asset(&asset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Asset not found".to_string()))?;

    if asset.dataset_id != dataset_id {
        return Err((StatusCode::NOT_FOUND, "Asset not in this dataset".to_string()));
    }

    let (data, content_type) = state.object_store.download(&asset.s3_key).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let ct_lower = content_type.split(';').next().unwrap_or("").trim().to_ascii_lowercase();
...
pub async fn delete_asset(
    ...
    state.object_store.delete(&asset.s3_key).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    state.auth_db.delete_asset(&asset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    match state.auth_db.list_dataset_assets(&dataset_id) {
        Ok(remaining) if remaining.is_empty() => {
            if let Err(e) = state.auth_db.remove_dataset_graph(&dataset_id, &graph) {
                tracing::warn!("Failed to unregister assets graph for dataset {}: {}", dataset_id, e);
            }
        }
        _ => {}
    }
...
pub async fn patch_asset_metadata(
    ...
    let title: Option<&str> = body.get("title").and_then(|v| v.as_str())
        .map(|s| s.trim()).filter(|s| !s.is_empty());
    let description: Option<&str> = body.get("description").and_then(|v| v.as_str())
        .map(|s| s.trim()).filter(|s| !s.is_empty());

    let updated = state.auth_db.update_asset_metadata(&asset_id, title, description)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let iri = asset_iri(&state.base_url, &dataset_id, &asset_id);
    if let Err(e) = remove_dcat_triples(&state, &iri, &graph) {
        tracing::warn!("DCAT remove failed during metadata update for asset {}: {}", asset_id, e);
    }
    if let Err(e) = rebuild_asset_triples(&state, &updated, &dataset_id).await {
        tracing::warn!("Asset metadata insert failed during update for asset {}: {}", asset_id, e);
    }

    let iri = asset_iri(&state.base_url, &dataset_id, &asset_id);
    Ok(Json(AssetResponse { asset: updated, iri }))
}
...
pub async fn update_asset_visibility(
    ...
    let public = body.get("public")
        .and_then(|v| v.as_bool())
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "Missing 'public' boolean field".to_string()))?;
    ...
    state.auth_db.update_asset_public(&asset_id, public)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn asset_metadata(
    ...
    let q = format!(
        ...
        iri = iri, spdx = "http://spdx.org/rdf/terms#"
    );
    let results = state.store.query(&q)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
        for sol in solutions.flatten() {
            let (Some(p), Some(o)) = (sol.get("p"), sol.get("o")) else { continue };
            let pred = match p { Term::NamedNode(n) => n.as_str().to_string(), _ => continue };
            ...
            match pred.as_str() {
                ...
                _ if pred == format!("{}width", SCHEMA) => { insert_u64(&mut out, "width", as_u64()); }
                _ if pred == format!("{}height", SCHEMA) => { insert_u64(&mut out, "height", as_u64()); }
                _ if pred == format!("{}numberOfPages", SCHEMA) => { insert_u64(&mut out, "pages", as_u64()); }
                _ if pred == format!("{}duration", SCHEMA) => { out.insert("duration".into(), lit.clone().into()); }
                _ if pred == format!("{}encodingFormat", SCHEMA) => { out.insert("format".into(), lit.clone().into()); }
                _ if pred == format!("{}thumbnail", SCHEMA) => { out.insert("thumbnail".into(), lit.clone().into()); }
                _ if pred == format!("{}pointCount", ASSET_NS) => { insert_u64(&mut out, "point_count", as_u64()); }
                _ if pred == format!("{}entryCount", ASSET_NS) => { insert_u64(&mut out, "entry_count", as_u64()); }
                _ if pred == format!("{}uncompressedSize", ASSET_NS) => { insert_u64(&mut out, "uncompressed_size", as_u64()); }
                _ if pred == format!("{}sheetCount", ASSET_NS) => { insert_u64(&mut out, "sheet_count", as_u64()); }
                _ if pred == format!("{}rowCount", ASSET_NS) => { insert_u64(&mut out, "row_count", as_u64()); }
                _ if pred == format!("{}panorama", ASSET_NS) => { out.insert("panorama".into(), (lit == "true").into()); }
                _ if pred == format!("{}sampleRate", ASSET_NS) => { insert_u64(&mut out, "sample_rate", as_u64()); }
                _ if pred == format!("{}dateCreated", SCHEMA) => { out.insert("captured_at".into(), lit.clone().into()); }
                _ if pred == format!("{}checksumValue", SPDX) => { out.insert("sha256".into(), lit.clone().into()); }
                _ if pred == format!("{}bbox", DCAT) => { out.insert("bbox".into(), lit.clone().into()); }
                _ if pred == format!("{}byteSize", DCAT) => { insert_u64(&mut out, "byte_size", as_u64()); }
                _ => {}
            }
        }
    }
    if let Some(c) = classes.into_iter().find(|c| c != &format!("{}MediaObject", SCHEMA)) {
        out.insert("schema_class".into(), c.into());
    }
    Ok(Json(serde_json::Value::Object(out)))
}

fn pred_starts(s: &str, prefix: &str) -> bool { s.starts_with(prefix) }
...
pub async fn linked_data_asset(
    ...
    if wants_rdf {
        ...
        let results = state.store.query(&sparql)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn validate_dataset(
    ...
    let _permit = state.expensive_semaphore.acquire().await
    ...
    let shapes_graph_iri = if let Some(ref iri) = dataset.shapes_graph_iri {
        iri.clone()
    } else if let (Some(ref onto_id), Some(ref onto_ver)) = (&dataset.conforms_to_ontology, &dataset.conforms_to_version) {
        ...
        return Err((StatusCode::BAD_REQUEST, "No shapes graph configured and no ontology linked".to_string()));
    };

    // Get data graphs for this dataset
    let data_graphs = state.auth_db.list_dataset_graphs(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    // Run SHACL validation
    let report = crate::shacl::validate(
        &state.store,
        &shapes_graph_iri,
        &data_graphs,
    ).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let _ = state.auth_db.record_dataset_usage(&dataset_id, Some(&current_user.user_id), "validate");

    // Persist this run so status + history survive reloads.
    let summary = persist_validation_run(&state, &dataset_id, &report, Some(current_user.user_id.as_str()))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_latest_validation_run(
    ...
    let run = state.auth_db.get_latest_validation_run(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(run))
}
...
pub async fn get_validation_history(
    ...
    let limit = params.get("limit").and_then(|s| s.parse::<i64>().ok()).unwrap_or(50).clamp(1, 50);
    let runs = state.auth_db.list_validation_run_summaries(&dataset_id, limit)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(runs))
}
...
pub async fn get_validation_run(
    ...
    let run = state.auth_db.get_validation_run(&run_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
...
pub async fn list_latest_validation_runs(
    ...
    for id in &req.dataset_ids {
        if let Some(ds) = state.auth_db.get_dataset(id)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
            if state.auth_db.can_access_dataset(Some(&current_user.user_id), &ds)
                .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))? {
                accessible.push(id.clone());
            }
        }
    }
    let runs = state.auth_db.list_latest_run_summaries(&accessible)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(runs))
}
...
pub async fn get_shapes(
    ...
    let shapes_iri = dataset.shapes_graph_iri.as_deref()
        .ok_or_else(|| (StatusCode::NOT_FOUND, "No shapes graph configured".to_string()))?;

    // Detect SHACLC format request via query param or Accept header
    let want_shaclc = fmt_params.get("format").map(|v| v == "shaclc").unwrap_or(false)
        || headers.get(ACCEPT).and_then(|v| v.to_str().ok())
            .map(|a| a.contains("text/shaclc")).unwrap_or(false);
    ...
    if want_shaclc {
        ...
        return Ok((
            StatusCode::OK,
            [(CONTENT_TYPE, "text/shaclc")],
            shaclc,
        ).into_response());
    }

    let data = state.store.graph_store_get(Some(shapes_iri), oxigraph::io::RdfFormat::Turtle)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn put_shapes(
    ...
    let content_type = _headers.get(CONTENT_TYPE)
        .and_then(|v| v.to_str().ok())
        .unwrap_or("text/turtle");
    ...
    state.store.graph_store_put(Some(&shapes_iri), &data, oxigraph::io::RdfFormat::Turtle)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    // Update dataset to point to shapes graph
    state.auth_db.update_dataset_shacl(&dataset_id, dataset.shacl_on_write, Some(&shapes_iri))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn infer_dataset(
    ...
    let shapes_graph_iri = dataset.shapes_graph_iri.as_deref()
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "No shapes graph configured".to_string()))?;

    let data_graphs = state.auth_db.list_dataset_graphs(&dataset_id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    let count = crate::shacl::infer(
        &state.store,
        shapes_graph_iri,
        &data_graphs,
    ).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn shaclc_parse(body: Bytes) -> Result<Response, (StatusCode, String)> {
    let input = String::from_utf8(body.to_vec())
        .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid UTF-8".to_string()))?;
    let turtle = crate::shaclc::parse(&input)
...
pub async fn shaclc_serialize(
    ...
    let shapes_iri = if body_str.trim().starts_with('{') {
        ...
        v["shapesGraphIri"].as_str()
            .ok_or_else(|| (StatusCode::BAD_REQUEST, "Missing shapesGraphIri field".to_string()))?
            .to_string()
    } else {
...
pub async fn put_rml_mapping(
    ...
    state.store.graph_store_put(Some(&mapping_graph), &mapping_turtle, oxigraph::io::RdfFormat::Turtle)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn get_rml_mapping(
    ...
    let mapping_graph = format!("urn:dataset:{}:rml-mappings", dataset_id);
    let data = state.store.graph_store_get(Some(&mapping_graph), oxigraph::io::RdfFormat::Turtle)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    if data.is_empty() {
        return Err((StatusCode::NOT_FOUND, "No RML mapping stored for this dataset".to_string()));
    }
...
pub async fn execute_rml_mapping(
    ...
    let target_graph = params.get("graph").cloned()
    ...
    let mut source_data: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    ...
    let mapping_turtle = if let Some(override_turtle) = mapping_turtle_override {
        ...
        let data = state.store.graph_store_get(Some(&mapping_graph), oxigraph::io::RdfFormat::Turtle)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        if data.is_empty() {
            return Err((StatusCode::BAD_REQUEST, "No RML mapping stored. Upload one first or provide it as the 'mapping' part.".to_string()));
        }
        String::from_utf8(data)
            .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "Stored mapping is not valid UTF-8".to_string()))?
    };
    ...
    if preview {
        ...
        let temp = crate::store::TripleStore::in_memory()
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Temp store error: {e}")))?;
        let count = crate::rml::execute(&mapping, &source_data, &temp, Some(&target_graph))
            .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
        let turtle_bytes = temp.dump(oxigraph::io::RdfFormat::Turtle, Some(&target_graph))
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        let turtle = String::from_utf8(turtle_bytes)
            .unwrap_or_default();
        return Ok(Json(serde_json::json!({
            "preview": true,
            "triples_count": count,
...
pub async fn rml_preview(
    ...
    let turtle = mapping_turtle
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "Missing 'mapping' part".to_string()))?;
    ...
    let turtle_bytes = temp.dump(oxigraph::io::RdfFormat::Turtle, None)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
fn format_term(term: &Term) -> serde_json::Value {
    ...
    match term {
        ...
        #[cfg(feature = "rdf-12")]
        ModelTerm::Triple(t) => {
            crate::sparql::rdf12_functions::triple_term_to_json(t)
        }
...
fn format_sparql_results_as_triples(
    ...
    if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
        let triples: Vec<serde_json::Value> = solutions
            .filter_map(|s| s.ok())
            .map(|solution| {
                let s = solution.get("s").map(format_term).unwrap_or(serde_json::Value::Null);
                let p = solution.get("p").map(format_term).unwrap_or(serde_json::Value::Null);
                let o = solution.get("o").map(format_term).unwrap_or(serde_json::Value::Null);
                let g = solution.get("g").map(format_term).unwrap_or(serde_json::Value::Null);
...
fn split_resource_closure(
    results: oxigraph::sparql::QueryResults,
) -> Result<(Vec<serde_json::Value>, serde_json::Map<String, serde_json::Value>), AppError> {
    use oxigraph::model::Term;
    let mut outgoing: Vec<serde_json::Value> = Vec::new();
    let mut bnodes: serde_json::Map<String, serde_json::Value> = serde_json::Map::new();
    if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
        for sol in solutions.filter_map(|s| s.ok()) {
            let p = sol.get("p").map(format_term).unwrap_or(serde_json::Value::Null);
            let o = sol.get("o").map(format_term).unwrap_or(serde_json::Value::Null);
...
fn resolve_blank_node(
    ...
    graphs: &[String],
    max_depth: usize,
) -> (Vec<serde_json::Value>, serde_json::Map<String, serde_json::Value>, Vec<serde_json::Value>) {
    ...
    let graph_nodes: Vec<NamedNode> =
        graphs.iter().filter_map(|g| NamedNode::new(g).ok()).collect();
...
fn format_sparql_results_as_pairs(
    ...
    if let oxigraph::sparql::QueryResults::Solutions(solutions) = results {
        let pairs: Vec<serde_json::Value> = solutions
            .filter_map(|s| s.ok())
            .map(|solution| {
                let v1 = solution.get(key1).map(format_term).unwrap_or(serde_json::Value::Null);
                let v2 = solution.get(key2).map(format_term).unwrap_or(serde_json::Value::Null);
...
async fn reasoning_materialize(
    ...
    let target = body.target_graph.unwrap_or_else(|| {
        match body.regime.as_str() {
    ...
    let report: Option<crate::reasoning::ReasoningReport> = match body.regime.as_str() {
        ...
        "rdfs" => {
            ...
            Some(m.materialize().map_err(|e| AppError::Internal(e.to_string()))?)
        }
        #[cfg(feature = "owl2-rl")]
        "owl2-rl" => {
            let m = crate::reasoning::owl2_rl::Owl2RLReasoner::new(&state.store).with_target(&target);
            Some(m.materialize().map_err(|e| AppError::Internal(e.to_string()))?)
        }
        #[cfg(feature = "owl2-el")]
        "owl2-el" => {
            let m = crate::reasoning::owl2_el::El2Classifier::new(&state.store).with_target(&target);
            Some(m.classify().map_err(|e| AppError::Internal(e.to_string()))?)
        }
        #[cfg(feature = "owl2-ql")]
        "owl2-ql" => {
            let rw = crate::reasoning::owl2_ql::QLQueryRewriter::new(&state.store);
            Some(rw.materialize_tbox().map_err(|e| AppError::Internal(e.to_string()))?)
        }
        #[cfg(feature = "owl2-dl")]
        "owl2-dl" => {
    ...
    match report {
        Some(r) => Ok((StatusCode::OK, Json(serde_json::json!({
            "regime": r.regime,
        ...
        None => Err(AppError::BadRequest(format!("Unknown reasoning regime: {}", body.regime))),
    }
}

/// GET /api/reasoning/status — counts of entailed triples per graph.
async fn reasoning_status(
    State(state): State<AppState>,
) -> Result<Response, AppError> {
    let graphs = [
...
async fn reasoning_rewrite(
    ...
    let rewritten = match regime {
        ...
        "owl2-ql" => {
            let rw = crate::reasoning::owl2_ql::QLQueryRewriter::new(&state.store);
            rw.rewrite_query(&body.query).map_err(|e| AppError::Internal(e.to_string()))?
        }
        _ => body.query.clone(),
    };
    Ok((StatusCode::OK, Json(serde_json::json!({ "rewritten": rewritten }))).into_response())
}
...
async fn text_search_reindex(
    ...
    if !user.is_admin() {
        return Err(AppError::Forbidden("Admin access required to reindex".to_string()));
    }
    ...
    #[cfg(feature = "text-search")]
    {
        if let Some(ref idx) = state.text_index {
            let count = idx.reindex_from_store(&state.store)
                .map_err(|e| AppError::Internal(e.to_string()))?;
            return Ok((StatusCode::OK, Json(serde_json::json!({ "indexed": count }))).into_response());
        }
    }
...
pub fn shex_routes() -> Router<AppState> {
    Router::new()
        .route("/api/datasets/:dataset_id/shex/validate", post(shex_validate))
...
async fn shex_validate(
    ...
    let schema = crate::shex::parse_shexc(&body.schema)
        .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
...
pub fn swrl_routes() -> Router<AppState> {
    Router::new()
        .route("/api/swrl/execute", post(swrl_execute))
}
...
#[cfg(feature = "swrl")]
fn default_swrl_format() -> String { "text".to_string() }
...
async fn swrl_execute(
    ...
    let result = crate::swrl::execute_rules(
        &state.store,
        &rules,
        max_iter,
        body.target_graph.as_deref(),
    ).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
...
pub async fn detect_shapes(
    ...
    let accessible_datasets = state.auth_db
...
pub async fn list_accessible_shape_graphs(
    ...
    let shape_graphs: Vec<serde_json::Value> = datasets
        .into_iter()
        .filter_map(|d| {
            d.shapes_graph_iri.clone().map(|iri| serde_json::json!({
                "dataset_id": d.id,
                "dataset_name": d.name,
