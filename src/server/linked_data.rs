...
async fn void_handler(
    ...
    let user_id = user.as_deref().map(|u| u.user_id.as_str());
    let void_turtle = crate::dcat::generate_dcat_catalog(&state.base_url, &state.store, &state.auth_db, user_id);
...
fn compute_dereference_allowed_graphs(
    ...
    for v in crate::vocabularies::registry::list_vocabularies(&state.store) {
        let can = state
            ...
            .unwrap_or(false);
        for ver in crate::vocabularies::registry::list_versions(&state.store, &state.base_url, &v.id) {
            all_ontology_graphs.insert(ver.graph_iri.clone());
            for g in &ver.sub_graphs {
                all_ontology_graphs.insert(g.clone());
            }
