...
use crate::auth::middleware::{endpoint_acl_guard, optional_auth, require_auth, require_admin, require_publisher};
...
impl KeyExtractor for SmartIpExtractor {
    ...
    fn extract<B>(&self, req: &Request<B>) -> Result<IpAddr, GovernorError> {
        ...
        let peer_is_trusted = peer_ip.map(|ip| {
...
fn build_cors_layer(cors_origins: &str) -> CorsLayer {
    ...
    if origins.is_empty() {
        ...
        let allowed: Vec<HeaderValue> = origins
            ...
            .collect();
        if allowed.is_empty() {
            tracing::error!("CORS_ORIGINS set but no valid origins parsed; allowing same-origin only.");
            return layer;
        }
    ...
    pub fn test_default_with_store(store: TripleStore) -> Self {
        ...
        AppState {
            ...
            object_store: Arc::new(ObjectStore::local(
                std::env::temp_dir().join("triplestore-test-objects"),
            ).unwrap()),
...
impl axum::extract::FromRef<AppState> for CookieConfig {
    fn from_ref(state: &AppState) -> Self {
        CookieConfig { secure: state.secure_cookies }
    }
}
...
pub fn build_router(state: AppState, cors_origins: &str, trusted_cidrs: Vec<IpNet>) -> Router {
    ...
    let make_rate_conf = |period_secs: u64, burst: u32| {
        let (period_secs, burst) = if disable_rate_limit { (1, 1_000_000) } else { (period_secs, burst) };
        Arc::new(
            GovernorConfigBuilder::default()
                .key_extractor(SmartIpExtractor { trusted_cidrs: trusted_cidrs.clone() })
    ...
    let auth_public_routes = Router::new()
        ...
        .route_layer(GovernorLayer { config: auth_rate_conf.clone() })
    ...
    let auth_sensitive_routes = Router::new()
        ...
        .route("/api/auth/tokens", get(handlers::list_api_tokens).post(handlers::create_api_token))
        .route("/api/auth/tokens/:token_id", delete(handlers::revoke_api_token))
    ...
    let admin_routes = Router::new()
        ...
        .route("/api/admin/users/:user_id", get(handlers::admin_get_user).put(handlers::admin_update_user).delete(handlers::admin_delete_user))
        .route("/api/admin/users/:user_id/identities", get(handlers::admin_list_user_identities))
        .route("/api/admin/users/:user_id/reset-password", post(handlers::admin_reset_password))
        .route("/api/admin/users/:user_id/purge", post(handlers::admin_purge_user))
        .route("/api/admin/dataset-usage", get(handlers::admin_dataset_usage))
        .route("/api/admin/audit", get(crate::auth::audit::admin_list_audit))
        .route("/api/admin/audit/export", get(crate::auth::audit::admin_export_audit))
        .route("/api/admin/backup", get(crate::backup::admin_list_backups).post(crate::backup::admin_create_backup))
        .route("/api/admin/backup/:id/verify", post(crate::backup::admin_verify_backup))
    ...
    let org_mixed_routes = Router::new()
        .route("/api/organisations", get(handlers::list_organisations).post(handlers::create_organisation))
        .route("/api/organisations/:org_id", get(handlers::get_organisation).put(handlers::update_organisation).delete(handlers::delete_organisation))
    ...
    let org_routes = Router::new()
        .route("/api/organisations/:org_id/members", get(handlers::list_org_members).post(handlers::add_org_member))
        .route("/api/organisations/:org_id/members/:user_id", delete(handlers::remove_org_member).put(handlers::update_org_member_role))
        .route("/api/organisations/:org_id/groups", post(handlers::create_group).get(handlers::list_groups))
        .route("/api/organisations/:org_id/groups/:group_id", get(handlers::get_group).put(handlers::update_group).delete(handlers::delete_group))
        .route("/api/organisations/:org_id/groups/:group_id/members", get(handlers::list_group_members).post(handlers::add_group_member))
        .route("/api/organisations/:org_id/groups/:group_id/members/:user_id", delete(handlers::remove_group_member))
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
        .with_state(state.clone());
    ...
    let user_routes = Router::new()
        ...
        .route("/api/users/:user_id", get(handlers::get_user).delete(handlers::delete_user))
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
        .with_state(state.clone());
    ...
    let dataset_mixed_routes = Router::new()
        .route("/api/datasets", get(handlers::list_datasets).post(handlers::create_dataset))
        .route("/api/datasets/:dataset_id", get(handlers::get_dataset).put(handlers::update_dataset).delete(handlers::delete_dataset))
        .route("/api/datasets/:dataset_id/graphs", get(handlers::list_dataset_graphs).post(handlers::add_dataset_graph).patch(handlers::patch_dataset_graph_role).delete(handlers::remove_dataset_graph))
        .route("/api/datasets/:dataset_id/commits", get(handlers::list_dataset_commits))
        .route("/api/datasets/:dataset_id/services", get(handlers::list_services).post(handlers::create_service))
        .route("/api/datasets/:dataset_id/services/:service_id", get(handlers::get_service).put(handlers::update_service).delete(handlers::delete_service))
        .route("/api/datasets/:dataset_id/services/:service_id/graphs", get(handlers::list_service_graphs).post(handlers::add_service_graph).delete(handlers::remove_service_graph))
        .route_layer(middleware::from_fn_with_state(state.clone(), optional_auth))
        .with_state(state.clone());
    ...
    let dataset_protected_routes = Router::new()
        .route("/api/datasets/:dataset_id/shacl", put(handlers::update_dataset_shacl))
        .route("/api/datasets/:dataset_id/role", put(handlers::update_dataset_role))
        .route("/api/datasets/:dataset_id/access", get(handlers::list_access).post(handlers::grant_access))
        .route("/api/datasets/:dataset_id/access/:user_id", delete(handlers::revoke_access))
        .route("/api/datasets/:dataset_id/grants", get(handlers::list_dataset_grants).put(handlers::set_dataset_grant))
        .route("/api/datasets/:dataset_id/grants/:principal_type/:principal_id", delete(handlers::revoke_dataset_grant))
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
        .with_state(state.clone());
    ...
    let asset_routes = Router::new()
        .route("/api/datasets/:dataset_id/assets", get(routes::list_assets).post(routes::upload_asset))
        .route("/api/datasets/:dataset_id/assets/:asset_id", get(routes::download_asset).delete(routes::delete_asset).patch(routes::patch_asset_metadata))
        .route("/api/datasets/:dataset_id/assets/:asset_id/visibility", put(routes::update_asset_visibility))
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
    ...
    let dataset_sparql_routes = Router::new()
        .route("/api/datasets/:dataset_id/services/:service_slug/sparql", get(routes::dataset_sparql_query).post(routes::dataset_sparql_post))
        .route_layer(middleware::from_fn_with_state(state.clone(), optional_auth))
        .route_layer(GovernorLayer { config: sparql_rate_conf.clone() })
        .with_state(state.clone());
    ...
    let org_image_routes = Router::new()
        .route("/api/organisations/:org_id/image", put(handlers::upload_org_image).get(handlers::get_org_image))
        .route("/api/organisations/:org_id/banner", put(handlers::upload_org_banner).get(handlers::get_org_banner))
        .route_layer(middleware::from_fn_with_state(state.clone(), optional_auth))
        .with_state(state.clone());

    // Dataset image
    let dataset_image_routes = Router::new()
        .route("/api/datasets/:dataset_id/image", put(handlers::upload_dataset_image).get(handlers::get_dataset_image))
        .route("/api/datasets/:dataset_id/banner", put(handlers::upload_dataset_banner).get(handlers::get_dataset_banner))
        .route_layer(middleware::from_fn_with_state(state.clone(), optional_auth))
        .with_state(state.clone());

    // SHACL validation routes
    let shacl_routes = Router::new()
        .route("/api/datasets/:dataset_id/validate", post(routes::validate_dataset))
        .route("/api/datasets/:dataset_id/validation/latest", get(routes::get_latest_validation_run))
        .route("/api/datasets/:dataset_id/validation/history", get(routes::get_validation_history))
        .route("/api/datasets/:dataset_id/validation/runs/:run_id", get(routes::get_validation_run))
        .route("/api/shacl/validation/latest", post(routes::list_latest_validation_runs))
        .route("/api/datasets/:dataset_id/shapes", get(routes::get_shapes).put(routes::put_shapes))
        .route("/api/datasets/:dataset_id/infer", post(routes::infer_dataset))
        ...
        .route("/api/shacl/dataset-shape-graphs", get(routes::list_accessible_shape_graphs))
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
        .with_state(state.clone());
    ...
    let rml_routes = Router::new()
        .route("/api/datasets/:dataset_id/mappings",
            get(routes::get_rml_mapping).put(routes::put_rml_mapping))
        .route("/api/datasets/:dataset_id/mappings/execute",
    ...
    let browse_routes = Router::new()
        ...
        .route_layer(middleware::from_fn_with_state(state.clone(), endpoint_acl_guard))
    ...
    let bulk_import_routes = Router::new()
        ...
        .route_layer(GovernorLayer { config: bulk_import_rate_conf })
        .route_layer(middleware::from_fn_with_state(state.clone(), require_auth))
    ...
    let asset_ld_routes = Router::new()
        .route("/datasets/:dataset_id/assets/:asset_id", axum::routing::get(routes::linked_data_asset))
        // Typed-metadata JSON view — same optional-auth access control as the linked-data endpoint.
        .route("/api/datasets/:dataset_id/assets/:asset_id/metadata",
    ...
    let acl_admin_routes = Router::new()
        .route("/api/admin/acl/endpoints",
            get(acl_handlers::list_endpoint_acl_rules)
            .post(acl_handlers::create_endpoint_acl_rule))
        .route("/api/admin/acl/endpoints/:id",
            put(acl_handlers::update_endpoint_acl_rule)
            .delete(acl_handlers::delete_endpoint_acl_rule))
        .route("/api/admin/acl/graphs",
            get(acl_handlers::list_graph_acl_rules)
            .post(acl_handlers::grant_graph_permission))
        .route("/api/admin/acl/graphs/:id",
            delete(acl_handlers::revoke_graph_permission))
        .route("/api/admin/acl/triples",
            get(acl_handlers::list_triple_security_labels)
            .post(acl_handlers::create_triple_security_label))
        .route("/api/admin/acl/triples/:id",
    ...
    let oauth_admin_routes = Router::new()
        .route("/api/admin/oauth/providers",
            get(oauth_handlers::admin_list_providers)
            .post(oauth_handlers::admin_create_provider))
        .route("/api/admin/oauth/providers/:id",
    ...
    let oauth_flow_routes = Router::new()
        .route("/api/auth/oauth/providers",
            get(oauth_handlers::list_active_providers))
        .route("/api/auth/oauth/:slug/authorize",
            get(oauth_handlers::oidc_authorize))
        .route("/api/auth/oauth/:slug/callback",
            get(oauth_handlers::oidc_callback))
        .route("/api/auth/saml/:slug/metadata",
            get(oauth_handlers::saml_metadata))
        .route("/api/auth/saml/:slug/acs",
            post(oauth_handlers::saml_acs))
...
pub async fn run(
    ...
    let backup = {
        ...
        let sqlite = std::env::var("AUTH_DB_PATH").unwrap_or_else(|_| "data/auth.sqlite".to_string());
        let retention: usize = std::env::var("BACKUP_RETENTION_COUNT").ok().and_then(|s| s.parse().ok()).unwrap_or(7);
        let encrypt = std::env::var("BACKUP_ENCRYPT").map(|v| v == "true" || v == "1").unwrap_or(false);
        ...
        match crate::backup::BackupManager::new(
            ...
            Ok(m) => Some(Arc::new(m)),
            Err(e) => { tracing::warn!("backup: disabled — init failed: {}", e); None }
        }
    };
    ...
    if let Some(verifier) = auth_ext.oidc.as_ref() {
        match crate::auth::oidc_rs::ensure_env_provider(&auth_db, verifier.issuer(), &auth_ext.default_role) {
            Ok(_) => tracing::info!(
    ...
    if let Some(ref mgr) = backup {
        let hours: u64 = std::env::var("BACKUP_SCHEDULE_HOURS").ok().and_then(|s| s.parse().ok()).unwrap_or(24);
    ...
    // start the pipeline scheduler.
    {
        ...
        tokio::task::spawn_blocking(move || {
            if let Err(e) = crate::shacl_studio::seed::seed_shacl_shacl(&migrate_store, &migrate_auth) {
                tracing::warn!("shacl_studio: SHACL-SHACL seed failed: {e}");
            }
            if let Err(e) = crate::shacl_studio::migrate::migrate_legacy(&migrate_store, &migrate_auth, &migrate_base) {
                tracing::warn!("shacl_studio: legacy migration failed: {e}");
            }
            // Built-in per-standard shape graphs + validation pipelines (idempotent).
            if let Err(e) = crate::shacl_studio::seed_standards::seed_standards(&migrate_store, &migrate_auth) {
            ...
            let _ = crate::auth::dataset_audit::seed_dataset_structure_shapes(&migrate_store, &migrate_auth);
            if let Err(e) = crate::auth::dataset_audit::audit_dataset_metadata(&migrate_store, &migrate_auth, &migrate_base) {
        ...
        crate::shacl_studio::scheduler::spawn_scheduler(state.store.clone(), state.auth_db.clone(), state.base_url.to_string());
    }
