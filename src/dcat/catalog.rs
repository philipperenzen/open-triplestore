...
pub fn generate_dcat_catalog(
    ...
    writeln!(out, "    dct:description \"DCAT 2 catalog for the Open Triplestore instance.\" ;").unwrap();
    writeln!(out, "    dct:language <http://id.loc.gov/vocabulary/iso639-1/en> ;").unwrap();
    ...
    if datasets.is_empty() {
        ...
        for (i, ds) in datasets.iter().enumerate() {
            ...
            writeln!(out, "    dcat:dataset <{base_url}/dataset/{id}>{comma}", id = ds.id).unwrap();
        }
    }
    writeln!(out).unwrap();
    ...
    let distinct_subjects = count_via_sparql(store, "SELECT (COUNT(DISTINCT ?s) AS ?c) WHERE { ?s ?p ?o }");
    let distinct_predicates = count_via_sparql(store, "SELECT (COUNT(DISTINCT ?p) AS ?c) WHERE { ?s ?p ?o }");
    let distinct_objects = count_via_sparql(store, "SELECT (COUNT(DISTINCT ?o) AS ?c) WHERE { ?s ?p ?o }");
    ...
    writeln!(out, "    sd:supportedLanguage sd:SPARQL11Query, sd:SPARQL11Update .").unwrap();
    writeln!(out).unwrap();
...
pub fn generate_org_dcat_catalog(
    ...
    writeln!(out, "    a dcat:Catalog ;").unwrap();
    writeln!(out, "    dct:title \"{}\" ;", escape_turtle(&format!("{} Catalog", org.name))).unwrap();
    ...
    let has_contact = org.contact_name.is_some() || org.contact_email.is_some() || org.contact_url.is_some();
    if has_contact {
...
fn emit_dataset_entry(
    ...
    writeln!(out, "    dct:modified \"{}\"^^xsd:dateTime ;", ds.updated_at).unwrap();

    // Visibility
    let access_rights = match ds.visibility {
        crate::auth::models::Visibility::Public => "http://publications.europa.eu/resource/authority/access-right/PUBLIC",
        crate::auth::models::Visibility::Members => "http://publications.europa.eu/resource/authority/access-right/RESTRICTED",
        crate::auth::models::Visibility::Private => "http://publications.europa.eu/resource/authority/access-right/NON_PUBLIC",
    };
    ...
    let graph_entries = auth_db.list_dataset_graph_entries(&ds.id).unwrap_or_default();
    ...
    writeln!(out, "        dcat:mediaType <https://www.iana.org/assignments/media-types/text/turtle>").unwrap();
    writeln!(out, "    ] ;").unwrap();
    ...
    if let Some(ref onto_id) = ds.conforms_to_ontology {
        if let Some(ref onto_ver) = ds.conforms_to_version {
            writeln!(out, "    dct:conformsTo <{base_url}/data-model/{onto_id}/version/{onto_ver}> ;").unwrap();
        } else {
            writeln!(out, "    dct:conformsTo <{base_url}/data-model/{onto_id}> ;").unwrap();
        }
    }
    ...
    // Contact point
    let has_contact = ds.contact_name.as_deref().filter(|s| !s.is_empty()).is_some()
        || ds.contact_email.as_deref().filter(|s| !s.is_empty()).is_some()
        || ds.contact_url.as_deref().filter(|s| !s.is_empty()).is_some();
    if has_contact {
        writeln!(out, "    dcat:contactPoint [").unwrap();
        writeln!(out, "        a vcard:Organization ;").unwrap();
    ...
    let landing = ds.landing_page.as_deref().filter(|s| !s.is_empty()).unwrap_or(base_url);
    ...
    for entry in &graph_entries {
        if !entry.graph_iri.starts_with("urn:system:") {
            if let Some(role) = entry.graph_role {
                ...
                writeln!(out, "    <https://opentriplestore.org/ontology/graphRole> <{role_iri}> .").unwrap();
                writeln!(out).unwrap();
            }
        }
    }
...
mod tests {
    ...
    fn dataset_with_conformance(
        ...
        db.create_dataset("ds-1", "Library Catalogue 2025", None, OwnerType::User, "u1", Visibility::Public, None)
            .unwrap();
