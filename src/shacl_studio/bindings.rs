    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(q) {
        for row in sols.flatten() {
            ...
            if let (Some(Term::NamedNode(s)), Some(Term::NamedNode(o))) =
                (vals.first().and_then(|t| t.as_ref()), vals.get(1).and_then(|t| t.as_ref()))
            {
...
pub fn version_validation_graph(base_url: &str, dataset_id: &str, version: &str) -> String {
    format!("{}/dataset/{}/version/{}/validation", base_url.trim_end_matches('/'), dataset_id, version)
}
...
pub fn snapshot_dataset_bindings(
    ...
    for t in &targets {
        for ss in bindings_for_target(store, t) {
            body.push_str(&format!("<{t}> ots:validatedBy <{ss}> ; dct:conformsTo <{ss}> .\n"));
            any = true;
        }
    }
    if !any {
...
mod tests {
    ...
    fn mk_set(studio: &ShaclStudioStore, name: &str, graph_iri: &str) -> ShapeGraph {
        studio
            .create_shape_graph(name, None, OwnerType::User, "u1", Visibility::Private, graph_iri, &[], ShapeSource::Manual, None)
            .unwrap()
    }
    ...
    fn binding_round_trip() {
        ...
        assert_eq!(targets_for_shape_graph(&store, ss), vec![target.to_string()]);
    ...
    fn effective_unions_dataset_and_graph_bindings_deduped() {
        ...
        let dataset = auth
            .create_dataset("d1", "D1", None, OwnerType::User, "u1", Visibility::Private, None)
            .unwrap();
        ...
        want.sort();
        assert_eq!(ids, want, "union of dataset-level + graph-inherited shape graphs");
    ...
    fn snapshot_then_restore_bindings_round_trips() {
        ...
        let snap = snapshot_dataset_bindings(&store, base, "d1", "1.0.0", &["urn:data:g1".to_string()])
            .unwrap()
        ...
        assert_eq!(bindings_for_target(&store, &ds_iri), vec![ds_set.graph_iri.clone()]);
        assert_eq!(bindings_for_target(&store, "urn:data:g1"), vec![g_set.graph_iri.clone()]);

        // Tolerant: a binding whose shape graph is gone is skipped on restore.
        studio.delete_shape_graph(&g_set.id).unwrap();
        remove_binding(&store, "urn:data:g1", &g_set.graph_iri).unwrap();
        let applied = restore_dataset_bindings(&store, &studio, base, "d1", "1.0.0").unwrap();
        assert_eq!(applied, 1, "only the still-existing shape graph is re-applied");
        assert!(bindings_for_target(&store, "urn:data:g1").is_empty(), "missing shape graph was skipped");
    }
