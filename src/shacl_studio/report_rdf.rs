...
pub fn report_to_turtle(report: &ValidationReport, report_iri: &str) -> String {
    ...
    for (i, r) in report.results.iter().enumerate() {
        ...
        t.push_str(&format!("        sh:resultSeverity {} ;\n", severity_iri(&r.severity)));
        ...
        if !r.source_shape.is_empty() {
            t.push_str(&format!("        sh:sourceShape {} ;\n", term(&r.source_shape)));
        }
        ...
        t.push_str(&format!("        sh:resultMessage \"{}\"\n", esc(&r.message)));
...
mod tests {
    ...
    fn report_turtle_round_trips() {
        ...
        store
            .graph_store_put(Some("urn:test:report"), &ttl, oxigraph::io::RdfFormat::Turtle)
    ...
    fn empty_report_is_valid() {
        let r = ValidationReport { conforms: true, results: vec![], results_count: 0 };
        let ttl = report_to_turtle(&r, "urn:system:reports:test#run-2");
        let store = TripleStore::in_memory().unwrap();
        store
            .graph_store_put(Some("urn:test:empty"), &ttl, oxigraph::io::RdfFormat::Turtle)
