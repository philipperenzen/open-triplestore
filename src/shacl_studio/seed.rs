...
pub fn seed_shacl_shacl(store: &TripleStore, auth_db: &AuthDb) -> anyhow::Result<String> {
    ...
    store
        .graph_store_put(Some(SHACL_SHACL_GRAPH), SHACL_SHACL_TTL, oxigraph::io::RdfFormat::Turtle)
    ...
    studio.save_shape_graph_revision(&set.id, SHACL_SHACL_TTL, &targets, count, Some("Built-in"), None)?;
...
mod tests {
    ...
    fn validate_against_meta(store: &TripleStore, data_graph: &str) -> crate::shacl_studio::run::RunOutcome {
    ...
    fn meta_validation_flags_bad_named_shape() {
        ...
        assert!(!outcome.report.conforms, "a malformed named shape must not conform");
        assert!(
            outcome.violation_count >= 2,
            "expected ≥2 violations (bad sh:nodeKind + non-boolean sh:closed), got {}: {:?}",
            outcome.violation_count, outcome.report.results
        );
    }
    ...
    fn inline_blank_property_shapes_are_meta_validated() {
        ...
        "#;
        store
            .graph_store_put(Some("urn:test:inline"), bad_inline, oxigraph::io::RdfFormat::Turtle)
            .unwrap();
