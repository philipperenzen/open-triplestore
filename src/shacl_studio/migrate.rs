...
pub fn migrate_legacy(store: &TripleStore, auth_db: &AuthDb, base_url: &str) -> anyhow::Result<usize> {
    let studio = ShaclStudioStore::new(auth_db.pool());
    let datasets = auth_db.list_datasets()?;
    let mut created = 0usize;

    for d in datasets {
        let Some(iri) = d.shapes_graph_iri.clone().filter(|s| !s.is_empty()) else { continue };
        ...
        if studio.get_shape_graph_by_iri(&iri)?.is_none() {
            ...
            let ttl = store
                ...
                .unwrap_or_default();
            studio.save_shape_graph_revision(&set.id, &ttl, &targets, count, Some("Initial import"), None)?;
            created += 1;
        }
        ...
        if let Err(e) = super::bindings::add_binding(store, &target_iri, &iri) {
            tracing::warn!("shacl_studio: failed to write legacy binding for dataset {}: {e}", d.id);
        }
    }
