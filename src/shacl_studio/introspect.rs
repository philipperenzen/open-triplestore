...
fn local_name(iri: &str) -> String {
    iri.rsplit(['#', '/']).next().filter(|s| !s.is_empty()).unwrap_or(iri).to_string()
}
...
pub fn derive_shapes(
    ...
    for class in &classes {
        let n_instances = super::run::scalar_count(
            store,
            &format!("SELECT (COUNT(DISTINCT ?s) AS ?n) WHERE {{ {} }}", wrap_graph(&values, &format!("?s a <{class}>"))),
        );
        if n_instances == 0 {
            continue;
        }
