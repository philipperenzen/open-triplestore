...
const SHAPE_GRAPH_COLS: &str =
    "id, name, description, owner_type, owner_id, visibility, graph_iri, tags, \
...
impl ShaclStudioStore {
    ...
    pub fn list_shape_graphs(&self) -> anyhow::Result<Vec<ShapeGraph>> {
        ...
        let mut stmt = conn.prepare(&format!(
            "SELECT {SHAPE_GRAPH_COLS} FROM shape_sets ORDER BY name"
        ))?;
        let rows = stmt
            .query_map([], read_shape_graph)?
    ...
    pub fn get_shape_graph_revision(
        &self,
        id: &str,
        revision: i64,
    ) -> anyhow::Result<Option<ShapeGraphRevision>> {
        let conn = self.pool.get()?;
        conn.query_row(
    ...
    pub fn list_pipelines(&self) -> anyhow::Result<Vec<ValidationPipeline>> {
        let conn = self.pool.get()?;
        let mut stmt = conn.prepare(&format!(
            "SELECT {PIPE_COLS} FROM validation_pipelines ORDER BY name"
        ))?;
        let rows = stmt
            .query_map([], read_pipeline)?
            .collect::<Result<Vec<_>, _>>()?;
        Ok(rows)
    }
    ...
    pub fn delete_pipeline(&self, id: &str) -> anyhow::Result<()> {
        let conn = self.pool.get()?;
        conn.execute(
            "DELETE FROM validation_pipelines WHERE id = ?1",
            params![id],
        )?;
        Ok(())
    }
