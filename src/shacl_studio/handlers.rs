...
fn parse_visibility(s: &Option<String>) -> Visibility {
    s.as_deref().and_then(Visibility::from_str).unwrap_or(Visibility::Private)
}
...
fn resolve_owner(
    ...
    match owner_type.as_deref().and_then(OwnerType::from_str) {
        Some(OwnerType::Organisation) | Some(OwnerType::Group) => {
            let oid = owner_id.clone().ok_or((StatusCode::BAD_REQUEST, "owner_id required".into()))?;
            if !user.is_admin() && !org_ids(state, &user.user_id).iter().any(|o| o == &oid) {
                return Err((StatusCode::FORBIDDEN, "Not a member of the owning organisation".into()));
            }
...
pub async fn create_shape_graph(
    ...
    let source = body.source.as_deref().map(ShapeSource::from_str_or_manual).unwrap_or(ShapeSource::Manual);
    ...
    let version = st
        .save_shape_graph_revision(&set.id, &turtle, &targets, count, Some("Created"), Some(&user.user_id))
        .map_err(e500)?;
    record_shape_graph_commit(&state, &set.id, &graph_iri, version, Some("Created"), &user.user_id);
...
pub async fn update_shape_graph(
    ...
    studio(&state)
        .update_shape_graph_meta(&id, &body.name, body.description.as_deref(), parse_visibility(&body.visibility), &body.tags)
        .map_err(e500)?;
...
pub async fn delete_shape_graph(
    ...
    let _ = state.store.update(&format!("CLEAR SILENT GRAPH <{}>", set.graph_iri));
...
pub async fn get_shape_graph_turtle(
    ...
    let want_shaclc = q.get("format").map(|v| v == "shaclc").unwrap_or(false)
        || headers.get(ACCEPT).and_then(|v| v.to_str().ok()).map(|a| a.contains("text/shaclc")).unwrap_or(false);
    if want_shaclc {
...
pub async fn put_shape_graph_turtle(
    ...
    let set = load_set_checked(&state, &user, &id, true).await?;
    let raw = String::from_utf8(body.to_vec()).map_err(|_| (StatusCode::BAD_REQUEST, "Invalid UTF-8".into()))?;
    let ct = headers.get(CONTENT_TYPE).and_then(|v| v.to_str().ok()).unwrap_or("text/turtle");
    let turtle = if ct.contains("shaclc") {
        crate::shaclc::parse(&raw).map_err(|e| (StatusCode::BAD_REQUEST, format!("SHACLC parse error: {e}")))?
    } else {
        raw
    };
    let version = write_shapes_revision(&state, &set.graph_iri, &id, &turtle, Some("Edited"), &user.user_id)?;
...
fn shape_graph_subject_iri(base_url: &str, set_id: &str) -> String {
    format!("{}/shacl/shape-graphs/{}", base_url.trim_end_matches('/'), set_id)
}
...
fn record_shape_graph_commit(state: &AppState, set_id: &str, graph_iri: &str, version: i64, note: Option<&str>, by: &str) {
...
pub async fn list_shape_graph_revisions(
    ...
    let revs = studio(&state).list_shape_graph_revisions(&id).map_err(e500)?;
    Ok(Json(revs))
}
...
pub async fn restore_shape_graph_revision(
    ...
    let snapshot = studio(&state)
        ...
        .map_err(e500)?
        .ok_or((StatusCode::NOT_FOUND, "Revision not found".into()))?;
    let version = write_shapes_revision(&state, &set.graph_iri, &id, &snapshot.turtle, Some(&format!("Restored revision {rev}")), &user.user_id)?;
    Ok(Json(serde_json::json!({ "version": version })))
}
...
pub async fn clone_shape_graph(
    ...
    let set = st
        .create_shape_graph(&name, src.description.as_deref(), OwnerType::User, &user.user_id, Visibility::Private, &graph_iri, &src.tags, ShapeSource::Manual, Some(&user.user_id))
        .map_err(e500)?;
    write_shapes_revision(&state, &graph_iri, &set.id, &turtle, Some(&format!("Cloned from {}", src.id)), &user.user_id)?;
    let set = st.get_shape_graph(&set.id).map_err(e500)?;
    Ok((StatusCode::CREATED, Json(set)))
}
...
async fn transition_shape_graph(
    ...
    let allowed = match to {
        ...
        VersionStatus::Published => matches!(set.status, VersionStatus::Draft | VersionStatus::Staged),
    ...
    if !allowed {
        return Err((
            ...
            format!("Cannot transition shape graph from {} to {}", set.status.as_str(), to.as_str()),
        ));
    }
...
async fn resolve_target_for_write(
    ...
    match target.kind {
        TargetKind::Dataset => {
            ...
            if !state.auth_db.can_write_dataset(&user.user_id, &ds).map_err(e500)? {
                return Err((StatusCode::FORBIDDEN, "Write access denied for dataset".into()));
            }
            Ok(super::bindings::dataset_target_iri(&state.base_url, &target.id))
        }
        TargetKind::Graph => {
            if !crate::auth::acl::check_graph_permission(Some(user), &target.id, "write", &state.auth_db) {
                return Err((StatusCode::FORBIDDEN, format!("Write access denied for graph <{}>", target.id)));
            }
            Ok(target.id.clone())
        }
    ...
    match target.kind {
        TargetKind::Dataset => {
            let ds = state
                ...
                .map_err(e500)?
                .ok_or((StatusCode::NOT_FOUND, "Dataset not found".into()))?;
            if !state.auth_db.can_access_dataset(Some(&user.user_id), &ds).map_err(e500)? {
                return Err((StatusCode::FORBIDDEN, "Access denied".into()));
            }
            Ok(super::bindings::dataset_target_iri(&state.base_url, &target.id))
        }
...
fn record_binding_commit(state: &AppState, set: &ShapeGraph, target_iri: &str, added: bool, by: &str) {
...
pub async fn list_bindings(
    ...
    if let Some(set_id) = q.shape_graph_id {
        ...
        return Ok(Json(serde_json::json!({ "shape_graph_id": set.id, "targets": targets })));
    }
    let (Some(kind), Some(id)) = (q.target_kind, q.target_id) else {
        return Err((StatusCode::BAD_REQUEST, "Provide either shape_graph_id or target_kind+target_id".into()));
    };
    let target = ValidationTarget { kind: TargetKind::from_str_or_dataset(&kind), id };
    ...
    let sets: Vec<ShapeGraph> = super::bindings::bindings_for_target(&state.store, &target_iri)
        ...
        .filter(|s| can_access_set(s, Some(&user.user_id), &orgs))
        .collect();
    Ok(Json(serde_json::json!({ "target": target_iri, "shape_graphs": sets })))
}
...
pub async fn dataset_effective_shapes(
    ...
    let sets = super::bindings::effective_shape_graphs_for_dataset(
        &state.store, &state.auth_db, &studio(&state), &state.base_url, &ds,
    );
    Ok(Json(sets))
}
...
pub async fn list_shapes_catalog(
    ...
    if let Some(graph) = params.get("graph") {
        if hidden(graph) {
            return Err((StatusCode::FORBIDDEN, "Access denied for that shape graph".into()));
        }
        ...
        let shapes: Vec<serde_json::Value> = super::catalog::catalog_shapes(&state.store, graph)
            ...
            .collect();
        return Ok(Json(serde_json::json!({ "graph": graph, "shapes": shapes })));
    }
...
pub async fn import_shapes(
    ...
    let refs: Vec<(String, String)> =
        body.shapes.iter().map(|r| (r.source_graph.clone(), r.shape.clone())).collect();
    let copied = super::catalog::import_shapes_into(&state.store, &set.graph_iri, &refs).map_err(e500)?;
    ...
    let note = body.note.unwrap_or_else(|| format!("Imported {copied} shape(s)"));
    let (targets, count) = super::run::analyze_shapes_graph(&state.store, &set.graph_iri);
    let version = studio(&state)
        .save_shape_graph_revision(&set.id, &turtle, &targets, count, Some(&note), Some(&user.user_id))
        .map_err(e500)?;
    record_shape_graph_commit(&state, &set.id, &set.graph_iri, version, Some(&note), &user.user_id);
    let updated = studio(&state).get_shape_graph(&set.id).map_err(e500)?;
    Ok(Json(serde_json::json!({ "imported": copied, "version": version, "shape_graph": updated })))
}
...
pub async fn register_shape_graph(
    ...
    if !crate::auth::acl::check_graph_permission(Some(&user), &body.graph_iri, "read", &state.auth_db) {
        return Err((StatusCode::FORBIDDEN, format!("Read access denied for graph <{}>", body.graph_iri)));
    }
    let (targets, count) = super::run::analyze_shapes_graph(&state.store, &body.graph_iri);
    if count == 0 {
        return Err((StatusCode::BAD_REQUEST, "Graph contains no SHACL shapes".into()));
    }
    let (owner_type, owner_id) = resolve_owner(&state, &user, &body.owner_type, &body.owner_id)?;
    let set = st
        .create_shape_graph(
    ...
    let turtle = String::from_utf8(turtle).map_err(|_| e500("graph is not valid UTF-8"))?;
    let version = st
        .save_shape_graph_revision(&set.id, &turtle, &targets, count, Some("Registered existing graph"), Some(&user.user_id))
        .map_err(e500)?;
    record_shape_graph_commit(&state, &set.id, &body.graph_iri, version, Some("Registered existing graph"), &user.user_id);
    let set = st
        .get_shape_graph(&set.id)
        .map_err(e500)?
        .ok_or((StatusCode::INTERNAL_SERVER_ERROR, "shape graph vanished after create".into()))?;
    Ok((StatusCode::CREATED, Json(set)))
}
...
fn validate_cron(cron: &Option<String>) -> Result<(), ApiErr> {
    if let Some(c) = cron.as_deref().filter(|s| !s.is_empty()) {
        if c.split_whitespace().count() != 5 {
            return Err((StatusCode::BAD_REQUEST, "schedule_cron must be a 5-field cron expression".into()));
        }
    }
    Ok(())
}
...
fn authorize_pipeline_targets(
    ...
    if pipeline.run_inference {
        let st = studio(state);
        write_targets.extend(super::exec::resolve_data_graphs(&state.auth_db, &st, pipeline));
    }
    write_targets.sort();
    write_targets.dedup();
    for g in &write_targets {
        if !check_graph_permission(Some(user), g, "write", &state.auth_db) {
            return Err((StatusCode::FORBIDDEN, format!("Write access denied for graph <{g}>")));
        }
    }
    Ok(())
}
...
pub async fn create_pipeline(
    ...
    let pipeline = ValidationPipeline {
        ...
        severity_threshold: body.severity_threshold.as_deref().map(SeverityThreshold::from_str_or_default).unwrap_or(SeverityThreshold::Violation),
        ...
        inferred_target_graph: body.inferred_target_graph.clone().filter(|s| !s.trim().is_empty()),
        results_target: body.results_target.unwrap_or_default(),
        results_target_graph: body.results_target_graph.clone().filter(|s| !s.trim().is_empty()),
        last_run_at: None,
        last_conforms: None,
...
pub async fn update_pipeline(
    ...
    let updated = ValidationPipeline {
        ...
        target_classes: body.target_classes,
        shape_graph_ids: body.shape_graph_ids,
        severity_threshold: body.severity_threshold.as_deref().map(SeverityThreshold::from_str_or_default).unwrap_or(existing.severity_threshold),
        run_inference: body.run_inference,
        max_results: body.max_results,
        trigger_on_write: body.trigger_on_write,
...
pub async fn run_pipeline(
    ...
    let test = q.get("test").map(|v| v == "true" || v == "1").unwrap_or(false);
    ...
    let run = tokio::task::spawn_blocking(move || {
        ...
        if test {
            super::exec::execute_pipeline_dry(&store, &auth_db, &st, &base_url, &pipeline, Some(&actor))
        } else {
            super::exec::execute_pipeline(&store, &auth_db, &st, &base_url, &pipeline, "manual", Some(&actor))
        }
    })
    .await
    .map_err(|e| e500(e))?
    ...
    if !test {
        for ds in &dataset_ids {
            let _ = state.auth_db.record_dataset_usage(ds, Some(&user.user_id), "pipeline");
        }
    }
    Ok(Json(run))
}
...
pub async fn list_pipeline_runs(
    ...
    let runs = studio(&state).list_pipeline_runs(&id, limit).map_err(e500)?;
    Ok(Json(runs))
}
...
async fn resolve_scope(
    ...
    if let Some(graphs) = q.get("graphs") {
        return Ok(graphs.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect());
    }
    Ok(vec![])
}
...
pub async fn model_context(
    ...
    let store = state.store.clone();
    let ctx = tokio::task::spawn_blocking(move || super::introspect::model_context(&store, &graphs))
        .await
        .map_err(e500)?;
    Ok(Json(ctx))
}
...
pub async fn derive_shapes(
    ...
    let store = state.store.clone();
    let (turtle, stats) = tokio::task::spawn_blocking(move || super::introspect::derive_shapes(&store, &graphs, &targets))
        .await
        .map_err(e500)?;
    Ok(Json(serde_json::json!({ "turtle": turtle, "stats": stats })))
}
...
pub async fn form_manifest(
    ...
    let dataset = state
        ...
        .map_err(e500)?
        .ok_or((StatusCode::NOT_FOUND, "Dataset not found".into()))?;
    if !state.auth_db.can_access_dataset(uid, &dataset).map_err(e500)? {
        return Err((StatusCode::FORBIDDEN, "Access denied".into()));
    }
    let manifest = super::manifest::build_manifest(&state.store, &state.auth_db, &state.base_url, &studio(&state), &dataset);
    Ok(Json(manifest))
}
