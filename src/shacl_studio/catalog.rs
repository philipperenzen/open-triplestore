...
pub fn catalog_graph_summary(store: &TripleStore) -> Vec<GraphSummary> {
    let q = r#"
        ...
        ORDER BY ?g
    "#;
    let mut by_graph: std::collections::BTreeMap<String, GraphSummary> = std::collections::BTreeMap::new();
    if let Ok(QueryResults::Solutions(sols)) = store.query(q) {
        for row in sols.flatten() {
            let vals = row.values();
            let get = |i: usize| vals.get(i).and_then(|t| t.as_ref());
            let Some(Term::NamedNode(g)) = get(0) else { continue };
            let is_property = matches!(get(1), Some(Term::NamedNode(n)) if n.as_str().ends_with("PropertyShape"));
            let n: usize = match get(2) {
                Some(Term::Literal(l)) => l.value().parse().unwrap_or(0),
                _ => 0,
            };
            let e = by_graph.entry(g.as_str().to_string()).or_insert_with(|| GraphSummary {
...
pub fn catalog_shapes(store: &TripleStore, graph: &str) -> Vec<CatalogShape> {
    ...
    let mut acc: std::collections::BTreeMap<String, CatalogShape> = std::collections::BTreeMap::new();
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        for row in sols.flatten() {
            let vals = row.values();
            let get = |i: usize| vals.get(i).and_then(|t| t.as_ref());
            let Some(Term::NamedNode(shape)) = get(0) else { continue };
            let shape_iri = shape.as_str().to_string();
            let entry = acc.entry(shape_iri.clone()).or_insert_with(|| CatalogShape {
...
mod tests {
    ...
    fn summary_counts_by_graph_and_skips_system() {
        ...
        assert!(graphs.contains(&"urn:shapes:a"), "dedicated shape graph listed");
        assert!(graphs.contains(&"urn:data:mixed"), "data graph with shapes listed");
        assert!(!graphs.contains(&"urn:system:shapes:shacl-shacl"), "urn:system: graph skipped");
    ...
    fn shapes_are_scoped_to_one_graph() {
        ...
        assert!(!iris.contains(&"http://ex/MixedShape"), "shape from a different graph not included");

        let person = shapes.iter().find(|s| s.shape == "http://ex/PersonShape").unwrap();
        assert_eq!(person.kind, "node");
        assert_eq!(person.target_classes, vec!["http://ex/Person".to_string()]);
        let name = shapes.iter().find(|s| s.shape == "http://ex/NameShape").unwrap();
    ...
    fn import_copies_shape_with_blank_node_closure() {
        ...
        assert!(matches!(store.query(q), Ok(QueryResults::Boolean(true))), "blank-node closure copied");

        import_shapes_into(&store, "urn:shapes:dst", &[("urn:shapes:src".into(), "http://ex/PersonShape".into())]).unwrap();
        assert_eq!(catalog_shapes(&store, "urn:shapes:dst").len(), 1, "re-import is idempotent");
    }
}
