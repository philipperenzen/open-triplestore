...
pub fn resolve_shape_graphs(
    ...
    for ds_id in dataset_ids {
        if let Ok(Some(ds)) = auth_db.get_dataset(ds_id) {
            for set in bindings::effective_shape_graphs_for_dataset(store, auth_db, studio, base_url, &ds) {
                graphs.push(set.graph_iri);
            }
        }
    }
    ...
    if pipeline.targets.iter().any(|t| t.kind == TargetKind::ShapeGraph) {
...
fn owner_can_write(auth_db: &AuthDb, created_by: &Option<String>, graph: &str) -> bool {
    let Some(uid) = created_by.as_deref() else { return false };
...
pub fn execute_pipeline(
    ...
    let infer_ok = pipeline.run_inference
        && data_graphs.iter().all(|g| owner_can_write(auth_db, &pipeline.created_by, g));
    ...
    persist_derived(main_store, auth_db, base_url, pipeline, &data_graphs, &inferred_quads, &outcome.report, &run.id);

    Ok(PipelineRun { report: Some(outcome.report), ..run })
}
...
fn persist_derived(
    ...
    if pipeline.run_inference && !inferred.is_empty() && pipeline.inferred_target == WriteTarget::NewGraph {
        let explicit = pipeline.inferred_target_graph.as_deref().is_some_and(|s| !s.trim().is_empty());
        let target = pipeline
    ...
    if pipeline.results_target.is_enabled() {
        let explicit = matches!(pipeline.results_target, ResultsTarget::NewGraph)
            && pipeline.results_target_graph.as_deref().is_some_and(|s| !s.trim().is_empty());
        ...
        if explicit && !owner_can_write(auth_db, &pipeline.created_by, &target) {
            ...
            if matches!(pipeline.results_target, ResultsTarget::InPlace | ResultsTarget::NewGraph) {
    ...
    if pipeline.inferred_target == WriteTarget::NewVersion || pipeline.results_target == ResultsTarget::NewVersion {
...
fn write_quads_to_graph(store: &TripleStore, quads: &[Quad], target: &str) {
    let Ok(nn) = NamedNode::new(target) else { return };
    let gn = GraphName::NamedNode(nn);
    let rehomed: Vec<Quad> = quads
        .iter()
        .map(|q| Quad::new(q.subject.clone(), q.predicate.clone(), q.object.clone(), gn.clone()))
        .collect();
...
fn snapshot_affected_versions(
    ...
    for (ds_id, source_graphs) in by_ds {
        let source_map = match crate::dataset_versions::snapshot::snapshot_graphs(
            store, base_url, &ds_id, &version, &source_graphs,
        ) {
            Ok(m) => m,
            Err(e) => {
                tracing::warn!("pipeline {} version snapshot for {ds_id} failed: {e}", pipeline.id);
                continue;
            }
        };
        ...
        let record = crate::dataset_versions::models::DatasetVersion {
            ...
            derived_from: None,
            notes: Some(format!("Auto-versioned by validation pipeline '{}'", pipeline.name)),
            branch: None,
        };
        if let Err(e) = crate::dataset_versions::registry::insert_version(store, base_url, &record) {
            tracing::warn!("pipeline {} insert_version for {ds_id} failed: {e}", pipeline.id);
            continue;
        }
        let _ = crate::dataset_versions::registry::update_latest_draft(store, base_url, &ds_id, &version);
    }
}
