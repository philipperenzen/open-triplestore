...
pub fn validate(
    ...
    if shape_map.is_empty() {
        ...
        for shape_decl in &schema.shapes {
            ...
            for node in &focus_nodes {
                let status = evaluate_shape_expr(
                    store,
                    schema,
                    node,
    ...
    ShExReport {
        conforms: results.iter().all(|r| matches!(r.status, ShExStatus::Conformant)),
        results,
    }
}
...
fn collect_predicates(expr: &ShapeExpr, predicates: &mut Vec<String>) {
    match expr {
        ...
        ShapeExpr::ShapeRef(_)
        | ShapeExpr::NodeConstraint(_)
        | ShapeExpr::NodeConstraintAny => {}
    }
}
...
fn evaluate_shape_expr(
    ...
    match expr {
        ...
        ShapeExpr::Shape {
            ...
            closed,
            extra,
        } => evaluate_shape(store, schema, focus_node, expression, *closed, extra, visited),
        ...
        ShapeExpr::ShapeRef(iri) => {
            ...
            if visited.contains(&key) {
                ...
                debug!("Recursive shape reference detected: {} @ {}", focus_node, iri);
                return ShExStatus::Conformant;
            }
            visited.insert(key);
...
fn evaluate_shape(
    ...
    if let Some(te) = expression {
        ...
        let result =
            evaluate_triple_expr(store, schema, focus_node, te, &outgoing, &mut consumed, visited);
...
fn evaluate_triple_expr(
    ...
    match te {
        TripleExpr::TripleConstraint {
            ...
            let pred_str = format!("<{}>", predicate.trim_start_matches('<').trim_end_matches('>'));
            ...
            for (i, (p, obj)) in outgoing.iter().enumerate() {
                ...
                if p_clean == pred_bare || *p == pred_str || *p == *predicate {
                    ...
                    if let Some(ve) = value_expr {
                        let obj_status =
                            evaluate_shape_expr(store, schema, obj, ve, visited);
