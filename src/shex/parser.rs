...
impl<'a> ShExParser<'a> {
    ...
    fn parse_shape_or(&mut self) -> Result<ShapeExpr, String> {
        ...
        loop {
            self.skip_ws();
            if self.starts_with("OR") && self.remaining().as_bytes().get(2).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(2);
                self.skip_ws();
    ...
    fn parse_shape_and(&mut self) -> Result<ShapeExpr, String> {
        ...
        loop {
            self.skip_ws();
            if self.starts_with("AND") && self.remaining().as_bytes().get(3).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(3);
                self.skip_ws();
    ...
    fn parse_shape_not(&mut self) -> Result<ShapeExpr, String> {
        self.skip_ws();
        if self.starts_with("NOT") && self.remaining().as_bytes().get(3).map_or(true, |b| b.is_ascii_whitespace()) {
            self.advance(3);
            self.skip_ws();
    ...
    fn try_parse_value_expr(&mut self) -> Result<Option<Box<ShapeExpr>>, String> {
        ...
        match self.peek() {
            ...
            Some('.') => {
                ...
                if after_dot.map_or(true, |c| c.is_whitespace() || c == ';' || c == '}' || c == '|' || c == '*' || c == '+' || c == '?') {
                    self.advance(1);
    ...
    fn parse_string_facets(&mut self) -> Vec<StringFacet> {
        ...
        loop {
            self.skip_ws();
            if self.starts_with_ci("PATTERN") && self.remaining().as_bytes().get(7).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(7);
                self.skip_ws();
                if let Ok(pat) = self.parse_quoted_string() {
                    facets.push(StringFacet::Pattern(pat, None));
                }
            } else if self.starts_with_ci("MINLENGTH") && self.remaining().as_bytes().get(9).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(9);
                self.skip_ws();
                if let Some(n) = self.parse_usize() {
                    facets.push(StringFacet::MinLength(n));
                }
            } else if self.starts_with_ci("MAXLENGTH") && self.remaining().as_bytes().get(9).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(9);
                self.skip_ws();
                if let Some(n) = self.parse_usize() {
                    facets.push(StringFacet::MaxLength(n));
                }
            } else if self.starts_with_ci("LENGTH") && self.remaining().as_bytes().get(6).map_or(true, |b| b.is_ascii_whitespace()) {
                self.advance(6);
                self.skip_ws();
                if let Some(n) = self.parse_usize() {
