...
"#,
    ...
    let geo_functions = [
        "sfContains", "sfCrosses", "sfDisjoint", "sfEquals",
        "sfIntersects", "sfOverlaps", "sfTouches", "sfWithin",
        "ehContains", "ehCoveredBy", "ehCovers", "ehDisjoint",
        "ehEquals", "ehInside", "ehMeet", "ehOverlap",
        "rcc8dc", "rcc8ec", "rcc8po", "rcc8tppi",
        "rcc8tpp", "rcc8ntpp", "rcc8ntppi", "rcc8eq",
        "boundary", "buffer", "convexHull", "difference",
        "envelope", "intersection", "symDifference", "union",
        "distance", "area", "getSRID",
    ];

    for (i, func) in geo_functions.iter().enumerate() {
        let sep = if i < geo_functions.len() - 1 { " ," } else { " ;" };
