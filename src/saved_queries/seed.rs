...
fn try_seed(state: &AppState) -> anyhow::Result<()> {
    ...
    let _ = state.auth_db.add_org_member(&owner.id, &org_id, Role::Admin);
    ...
    for ds in seed_data::datasets() {
        ...
        if let Ok(Some(dsrec)) = state.auth_db.get_dataset(&ds_id) {
            let entries = state
                ...
                .unwrap_or_default();
            dataset_graph::write_dataset_metadata_graph(&state.store, &state.base_url, &dsrec, &entries);
        }
...
fn load_graph(state: &AppState, graph_iri: &str, g: &GraphSpec) -> anyhow::Result<()> {
    match g.fmt {
        Fmt::SparqlStarUpdate => {
            state
                .store
                .update(&format!("INSERT DATA {{ GRAPH <{graph_iri}> {{ {} }} }}", g.data))
