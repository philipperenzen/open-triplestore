...
fn record(
    ...
    if let Err(e) = sq_store.insert_test(&test) {
        tracing::warn!(
            "saved-query version test: failed to record result for {}: {e}",
            sq.id
        );
    }
...
mod tests {
    ...
    async fn version_bump_records_ok_then_changed() {
        ...
        state
            .store
            .update(&format!(
                "INSERT DATA {{ GRAPH <{GRAPH}> {{ <urn:a> <urn:p> \"1\" }} }}"
            ))
            .unwrap();
        ...
        state
            .store
            .update(&format!(
                "INSERT DATA {{ GRAPH <{GRAPH}> {{ <urn:b> <urn:p> \"2\" }} }}"
            ))
            .unwrap();
    ...
    async fn broken_query_records_error() {
        ...
        let sq = sqs
            .create(
                QueryScope::Dataset,
                ds,
                &create_req("SELECT ?s WHERE { ?s ?p ?o "),
                "u1",
            )
            .unwrap();
        snapshot(&state, ds, "1.0.0");
