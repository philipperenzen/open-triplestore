...
mod tests {
    ...
    fn count(store: &TripleStore, type_iri: &str) -> usize {
        ...
        if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
            if let Some(row) = sols.flatten().next() {
                if let Some(oxigraph::model::Term::Literal(l)) =
    ...
    fn ask(store: &TripleStore, pattern: &str) -> bool {
        matches!(
            store.query(&format!(
                "ASK {{ GRAPH <{REGISTRY_GRAPH}> {{ {pattern} }} }}"
            )),
    ...
    fn projects_immutable_records() {
        ...
        let t = QueryTest {
            ...
            prev_version: None,
            status: "ok".into(),
            result_hash: Some("abc".into()),
            result_rowcount: Some(3),
            error_message: None,
            acknowledged: false,
        ...
        assert_eq!(
            count(&store, &format!("{APS}Revision")),
            2,
            "both revisions kept (immutable)"
        );
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1> <{APS}dataset> <{base}/dataset/ds1>")
            ),
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1> <{DCT}isPartOf> <{base}/dataset/ds1>")
            ),
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1/revision/1> <{DCT}title> \"v1 — initial\"")
            ),
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1/revision/1> <{RDFS}label> \"v1 — initial\"")
            ),
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1/revision/1> <{ADMS}versionNotes> \"first cut\"")
            ),
        ...
        assert!(
            ask(
                &store,
                &format!("<{base}/api-service/svc1/revision/2> <{OWL}versionInfo> \"2\"")
            ),
