...
fn check_read(
    ...
    let ok = match scope {
        QueryScope::Dataset => {
            ...
            state.auth_db.can_access_dataset(uid, &ds).map_err(internal)?
        }
...
fn can_write_scope(
    ...
    match scope {
        QueryScope::Dataset => state
            ...
            .ok()
            .flatten()
            .map(|ds| state.auth_db.can_write_dataset(&u.user_id, &ds).unwrap_or(false))
            .unwrap_or(false),
        QueryScope::Organisation => matches!(
            state.auth_db.get_org_membership(&u.user_id, owner_id).ok().flatten(),
            Some(Role::Admin) | Some(Role::Member)
        ),
        QueryScope::Group => matches!(
            state.auth_db.get_group_membership(&u.user_id, owner_id).ok().flatten(),
            Some(Role::Admin) | Some(Role::Member)
        ),
    }
}
...
fn check_write<'a>(
    ...
    let u = user.ok_or_else(|| {
        AppError::Unauthorized("sign in with editor or owner rights to save API services".to_string())
    })?;
    if !u.write_access {
        return Err(AppError::Forbidden("this API token lacks write scope".to_string()));
    }
    if can_write_scope(state, Some(u), scope, owner_id) {
        Ok(u)
    } else {
        Err(AppError::Forbidden("editor or owner rights required for this scope".to_string()))
    }
}
...
async fn list_core(
    ...
    let name_of = |id: &str| all_datasets.iter().find(|d| d.id == id).map(|d| d.name.clone());
    ...
    for q in &queries {
        ...
        if let Value::Object(ref mut m) = v {
            m.insert("reads_from".to_string(), reads_from_of(q, &name_of, &union_datasets));
            m.insert(
                "can_write".to_string(),
...
async fn create_core(
    ...
    if req.name.trim().is_empty() || req.sparql.trim().is_empty() {
        return Err(AppError::BadRequest("name and sparql are required".to_string()));
    }
...
async fn update_core(
    ...
    if let Some(ref s) = req.sparql {
        let specs = req.parameters.clone().unwrap_or_else(|| sq.parameters.clone());
...
async fn delete_core(
    ...
    metadata::record_retired(&state.store, &state.base_url, &sq.id, &chrono::Utc::now().to_rfc3339());
...
async fn ack_core(
    ...
    store.acknowledge_test(&t.id, &u.user_id).map_err(internal)?;
...
async fn repair_core(
    ...
    let sq = store
        ...
        .map_err(internal)?
        .ok_or_else(|| AppError::NotFound("saved query not found".to_string()))?;
    let broken = body.sparql.or_else(|| sq.sparql.clone()).unwrap_or_default();
    let error = body
        .error
        .or_else(|| {
    ...
    let res = llm::repair_query(&broken, &error, body.schema_hint.as_deref(), body.model.as_deref()).await?;
    let mut saved_revision = None;
    if body.save {
        let rev = store
            .add_revision(&sq.id, &res.sparql, Some("LLM repair"), None, "llm_repair", &u.user_id)
            .map_err(internal)?;
        metadata::record_revision(
            &state.store,
            &state.base_url,
...
async fn run_core(
    ...
    if let Ok(ref mut resp) = result {
        let label = prep
            .version_label
            .clone()
            .unwrap_or_else(|| if prep.is_live { "latest".to_string() } else { "none".to_string() });
    ...
    ($modname:ident, $scope:expr) => {
        pub mod $modname {
            ...
            pub async fn run_get(
                ...
                run_core(&state, user.as_deref(), $scope, &owner, &slug, version, provided, &accept).await
            }
