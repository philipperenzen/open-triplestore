...
pub fn list_versions(store: &TripleStore, base_url: &str, dataset_id: &str) -> Vec<DatasetVersion> {
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        for row in sols.flatten() {
            ...
            let derived_from = var_str(&vals, 6).and_then(|i| i.rsplit('/').next().map(str::to_string));
...
pub fn get_version(
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        if let Some(row) = sols.flatten().next() {
            ...
            let snapshot_graphs = get_snapshot_graphs(store, &ver_iri);
            let source_map = get_graph_map(store, &ver_iri);
            let derived_from = var_str(&vals, 5).and_then(|i| i.rsplit('/').next().map(str::to_string));
...
fn get_snapshot_graphs(store: &TripleStore, ver_iri: &str) -> Vec<String> {
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        for row in sols.flatten() {
            if let Some(g) = var_str(&row.values().to_vec(), 0) { out.push(g); }
        }
    }
    out
}
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        for row in sols.flatten() {
            ...
            if let (Some(snap), Some(src)) = (var_str(&vals, 0), var_str(&vals, 1)) {
                out.push(GraphMapping { snapshot_graph: snap, source_graph: src });
            }
        }
    }
    out
...
pub fn insert_version(
    ...
    let creator = record.created_by.as_deref()
        .map(|u| format!("    dct:creator <{u}> ;\n")).unwrap_or_default();
    let derived = record.derived_from.as_deref()
        .map(|v| format!("    prov:wasDerivedFrom <{}> ;\n", version_iri(base_url, &record.dataset_id, v)))
        .unwrap_or_default();
    let notes = record.notes.as_deref().map(|n| {
        let e = n.replace('\\', "\\\\").replace('"', "\\\"");
        format!("    adms:versionNotes \"{e}\"@en ;\n")
    }).unwrap_or_default();
    let branch = record.branch.as_deref().map(|b| {
        let e = b.replace('\\', "\\\\").replace('"', "\\\"");
        format!("    ver:branch \"{e}\" ;\n")
    }).unwrap_or_default();
    let sub_graphs: String = record.snapshot_graphs.iter()
    ...
    for (i, m) in record.source_map.iter().enumerate() {
        ...
        let qm = format!(
            ...
            "#,
            snap = m.snapshot_graph, src = m.source_graph,
        );
        store.update(&qm)?;
    }
    Ok(())
...
pub fn update_latest_published(
    store: &TripleStore, base_url: &str, dataset_id: &str, version: &str,
) -> Result<(), crate::store::engine::StoreError> {
...
fn set_pointer(
    store: &TripleStore, base_url: &str, dataset_id: &str, version: &str, pred: &str,
) -> Result<(), crate::store::engine::StoreError> {
    let ds_iri = dataset_iri(base_url, dataset_id);
    let ver_iri = version_iri(base_url, dataset_id, version);
    let q = format!(
...
pub fn clear_latest_draft(
    store: &TripleStore, base_url: &str, dataset_id: &str,
) -> Result<(), crate::store::engine::StoreError> {
    let ds_iri = dataset_iri(base_url, dataset_id);
    let q = format!(
        r#"PREFIX ver: <{VER}>
...
pub fn update_version_notes(
    store: &TripleStore, base_url: &str, dataset_id: &str, version: &str, notes: Option<&str>,
) -> Result<(), crate::store::engine::StoreError> {
    let ver_iri = version_iri(base_url, dataset_id, version);
    let q_del = format!(
        r#"PREFIX adms: <{ADMS}>
...
pub fn get_pointers(
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&q) {
        if let Some(row) = sols.flatten().next() {
            let vals = row.values().to_vec();
            let to_label = |s: Option<String>| s.and_then(|i| i.rsplit('/').next().map(str::to_string));
...
pub fn version_exists(store: &TripleStore, base_url: &str, dataset_id: &str, version: &str) -> bool {
