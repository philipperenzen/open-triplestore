...
async fn run_validation(
    ...
    let base = std::env::var("VALIDATION_API_URL")
        ...
        .ok_or_else(|| {
            AppError::BadRequest("Validation Platform not configured (set VALIDATION_API_URL)".to_string())
        })?;
...
pub async fn validate_and_commit(
    ...
    let (dataset, graph_iri): (Dataset, String) = match c.target.as_str() {
        "new" => {
            ...
            let (owner_type, owner_id) = match c.org_id.as_deref().filter(|s| !s.is_empty()) {
                Some(org) => {
                    ...
                    {
                        Some(Role::Admin) | Some(Role::Member) => (OwnerType::Organisation, org.to_string()),
                        _ => {
            ...
            let name = c.name.clone().unwrap_or_else(|| "Validated import".to_string());
            let ds = state
                .auth_db
                .create_dataset(&ds_id, &name, None, owner_type, &owner_id, Visibility::Private, None)
                .map_err(|e| AppError::Internal(e.to_string()))?;
            let graph_iri = c
                .graph
                .clone()
        ...
        "dataset" => {
            let ds_id = c
                .dataset_id
                .clone()
                .ok_or_else(|| AppError::BadRequest("target 'dataset' requires datasetId".to_string()))?;
            let ds = state
                .auth_db
                .get_dataset(&ds_id)
                .map_err(|e| AppError::Internal(e.to_string()))?
        ...
        other => return Err(AppError::BadRequest(format!("unknown commit target '{other}'"))),
    };
    ...
    crate::server::routes::validate_on_write(&state, Some(&graph_iri), &data_ttl, RdfFormat::Turtle)?;
    ...
    let source_map = snapshot::snapshot_graphs(&state.store, &state.base_url, &dataset.id, &version, &source_graphs)
    ...
    let record = DatasetVersion {
        ...
        graph_iri: format!("{}/dataset/{}/version/{}", state.base_url, dataset.id, version),
        snapshot_graphs,
        source_map,
    ...
    registry::update_latest_draft(&state.store, &state.base_url, &dataset.id, &version).map_err(AppError::from)?;
    ...
    if let Err(e) = crate::shacl_studio::bindings::snapshot_dataset_bindings(
        &state.store, &state.base_url, &dataset.id, &version, &source_graphs,
    ) {
        tracing::warn!("failed to snapshot validation bindings for {} v{version}: {e}", dataset.id);
    }
    ...
    let shapes_ref = c
        ...
        .or_else(|| if body.shapes.use_dataset_shapes == Some(true) { body.shapes.dataset_id.clone() } else { None });
...
mod tests {
    ...
    fn empty_source() -> GraphSourceIn {
        GraphSourceIn { ttl: None, format: None, dataset_id: None, graph: None, use_dataset_shapes: None }
    }
    ...
    async fn missing_inline_data_is_bad_request() {
        ...
        let res = validate_and_commit(State(state), Extension(test_user()), HeaderMap::new(), Json(body)).await;
