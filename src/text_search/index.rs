...
impl TextIndex {
    ...
    pub fn open(index_dir: &Path) -> Result<Self, TextSearchError> {
        ...
        std::fs::create_dir_all(index_dir).map_err(|e| {
            TextSearchError::Store(format!("Cannot create tantivy dir: {e}"))
        })?;
    ...
    pub fn search(
        ...
        for (score, doc_address) in top_docs {
            ...
            hits.push(SearchHit { subject, predicate, score });
            if hits.len() >= limit {
                break;
            }
        }
    ...
    pub fn reindex_from_store(&self, store: &TripleStore) -> Result<usize, TextSearchError> {
        ...
        let results = store.query(query).map_err(|e| TextSearchError::Store(e.to_string()))?;
