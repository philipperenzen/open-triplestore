...
pub fn insert_commit(
    ...
    let commit_iri = format!("{}/commit/{}", base_url.trim_end_matches('/'), rec.commit_id);
...
impl CommitsParams {
    pub fn to_query(&self) -> CommitQuery {
        let clean = |o: &Option<String>| o.clone().map(|s| s.trim().to_string()).filter(|s| !s.is_empty());
        CommitQuery {
...
pub fn list_commits(store: &TripleStore, scope: &CommitScope, q: &CommitQuery) -> Vec<CommitRecord> {
    ...
    if let Ok(QueryResults::Solutions(sols)) = store.query(&query) {
        for row in sols.flatten() {
            ...
            let commit_id = commit_iri.rsplit('/').next().unwrap_or(&commit_iri).to_string();
...
pub fn resolve_actors(auth_db: &crate::auth::db::AuthDb, commits: &mut [CommitRecord]) {
    ...
    for c in commits.iter_mut() {
        let Some(iri) = c.actor_iri.clone() else { continue };
        let uid = iri.rsplit('/').next().unwrap_or(&iri).to_string();
        let entry = cache.entry(uid.clone()).or_insert_with(|| {
...
mod tests {
    ...
    fn list_by_graphs_and_branch_filter() {
        ...
        assert!(list_commits(&store, &CommitScope::Graphs(vec![]), &CommitQuery::default()).is_empty());

        // Branch filter narrows results.
        let wrong_branch = CommitQuery { branch: Some("main".into()), ..Default::default() };
        assert!(list_commits(
            &store,
            &CommitScope::Subject("http://x/data-model/dm1".into()),
            &wrong_branch,
