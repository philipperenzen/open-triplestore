    ...
    fn clone(&self) -> Self {
        match &self.backend {
            Backend::S3 { client, bucket } => Self {
                backend: Backend::S3 { client: client.clone(), bucket: bucket.clone() },
            },
            Backend::Local { base_path } => Self {
                backend: Backend::Local { base_path: base_path.clone() },
            },
        }
    }
}
...
impl ObjectStore {
    ...
    pub async fn new(
        ...
        Ok(Self {
            backend: Backend::S3 { client, bucket: bucket.to_string() },
        })
    }
    ...
    pub fn local(base_path: PathBuf) -> anyhow::Result<Self> {
        ...
        Ok(Self { backend: Backend::Local { base_path } })
    }
