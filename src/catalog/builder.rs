...
pub fn build_catalog_turtle(
    ...
    let data_models: Vec<_> = dm_registry::list_data_models(store)
        ...
        .filter(|d| {
            auth_db
                .can_access_ontology(user_id, d.is_public, d.owner_type.as_deref(), d.owner_id.as_deref())
                .unwrap_or(false)
        })
        .collect();
    let vocabs: Vec<_> = vocab_registry::list_vocabularies(store)
        ...
        .filter(|v| {
            auth_db
                .can_access_ontology(user_id, v.is_public, v.owner_type.as_deref(), v.owner_id.as_deref())
                .unwrap_or(false)
        })
        .collect();
    ...
    for d in data_models.iter().filter(|d| d.latest_published.is_some()) {
        ...
        writeln!(out, "    dct:type <http://purl.org/adms/assettype/Ontology> ;").unwrap();
        writeln!(out, "    foaf:page <{}> ;", d.namespace).unwrap();
        writeln!(out, "    dcat:landingPage <{base_url}/data-model/{}> ;", d.id).unwrap();
    ...
    for v in vocabs.iter().filter(|v| v.latest_published.is_some()) {
        ...
        writeln!(out, "    dct:type <http://purl.org/adms/assettype/CodeList> ;").unwrap();
        writeln!(out, "    foaf:page <{}> ;", v.namespace).unwrap();
        writeln!(out, "    dcat:landingPage <{base_url}/vocabulary/{}> ;", v.id).unwrap();
        writeln!(out, "    dcat:hasVersion \"{}\" ;", esc_literal(version)).unwrap();
