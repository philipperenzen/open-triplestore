...
//! `<urn:system:vocabulary-registry>` inside Oxigraph.

use oxigraph::model::*;
use oxigraph::sparql::QueryResults;
use crate::store::TripleStore;
use super::models::{SubGraphStatus, VocabularyRecord, VocabularyVersion, VersionStatus};

// ─── Vocabulary constants ─────────────────────────────────────────────────────

pub const REGISTRY_GRAPH: &str = "urn:system:vocabulary-registry";
...
/// List all vocabulary records from the registry.
pub fn list_vocabularies(store: &TripleStore) -> Vec<VocabularyRecord> {
    let q = format!(
        r#"
        PREFIX ver: <{VER}>
        PREFIX dct: <{DCT}>
    ...
    if let Ok(QueryResults::Solutions(solutions)) = store.query(&q) {
        for row in solutions.flatten() {
            ...
            // Extract vocabulary_id from the IRI (last path segment after /vocabulary/)
            let vocabulary_id = id.rsplit('/').next().unwrap_or(&id).to_string();
            ...
            let latest_published = latest_pub_iri.as_deref().and_then(|iri| {
                iri.rsplit('/').next().map(str::to_string)
            });
            let latest_draft_iri = var_str(&vals, 4);
            let latest_draft = latest_draft_iri.as_deref().and_then(|iri| {
                iri.rsplit('/').next().map(str::to_string)
            });
...
fn count_versions(store: &TripleStore, vocabulary_iri_str: &str) -> usize {
    let q = format!(
        r#"
        PREFIX ver: <{VER}>
        SELECT (COUNT(?v) AS ?cnt) WHERE {{
          GRAPH <{REGISTRY_GRAPH}> {{
            ?v ver:vocabulary <{vocabulary_iri_str}> .
          }}
        }}
        "#
    );
...
pub fn get_vocabulary(store: &TripleStore, base_url: &str, vocabulary_id: &str) -> Option<VocabularyRecord> {
    let ont_iri = format!("{}/vocabulary/{}", base_url, vocabulary_id);
    let q = format!(
        r#"
        PREFIX ver: <{VER}>
        PREFIX dct: <{DCT}>
...
/// Insert a new vocabulary record into the registry.
#[allow(clippy::too_many_arguments)]
pub fn insert_vocabulary(
    store: &TripleStore,
    base_url: &str,
    vocabulary_id: &str,
    title: &str,
...
/// Update editable metadata fields on a vocabulary.
#[allow(clippy::too_many_arguments)]
...
pub fn update_version_notes(
    ...
    let ver_iri = format!("{}/vocabulary/{}/version/{}", base_url, vocabulary_id, version);
...
/// Delete a vocabulary record and all its version records from the registry.
...
/// List all versions for a vocabulary, ordered newest first.
pub fn list_versions(store: &TripleStore, base_url: &str, vocabulary_id: &str) -> Vec<VocabularyVersion> {
    let ont_iri = format!("{}/vocabulary/{}", base_url, vocabulary_id);
    let q = format!(
        r#"
        PREFIX ver: <{VER}>
    ...
    if let Ok(QueryResults::Solutions(solutions)) = store.query(&q) {
        for row in solutions.flatten() {
            let vals: Vec<Option<Term>> = row.values().to_vec();
            let ver_iri = match var_str(&vals, 0) { Some(v) => v, None => continue };
            let semver = match var_str(&vals, 1) { Some(v) => v, None => continue };
            ...
            let graph_iri = match var_str(&vals, 3) { Some(v) => v, None => continue };
            let sub_graphs = get_sub_graphs(store, &ver_iri);
            let sub_graph_status = get_sub_graph_statuses(store, &ver_iri);
            let derived_from = var_str(&vals, 6).and_then(|iri| {
                iri.rsplit('/').next().map(str::to_string)
            });
            records.push(VocabularyVersion {
        ...
        for row in sols.flatten() {
            ...
            if let (Some(g), Some(s)) = (var_str(&vals, 0), var_str(&vals, 1)) {
                if let Some(status) = VersionStatus::from_str(&s) {
                    out.push(SubGraphStatus { graph_iri: g, status });
                }
            }
        }
    }
...
pub fn get_version(
    ...
    vocabulary_id: &str,
    version: &str,
) -> Option<VocabularyVersion> {
    let _vocab_iri = format!("{}/vocabulary/{}", base_url, vocabulary_id);
    let ver_iri = format!("{}/vocabulary/{}/version/{}", base_url, vocabulary_id, version);
    let q = format!(
        r#"
        PREFIX ver: <{VER}>
    ...
    if let Ok(QueryResults::Solutions(solutions)) = store.query(&q) {
        if let Some(row) = solutions.flatten().next() {
            ...
            let sub_graphs = get_sub_graphs(store, &ver_iri);
            let sub_graph_status = get_sub_graph_statuses(store, &ver_iri);
            let derived_from = var_str(&vals, 5).and_then(|iri| {
                iri.rsplit('/').next().map(str::to_string)
            });
            return Some(VocabularyVersion {
                vocabulary_id: vocabulary_id.to_string(),
                version: semver,
                status,
                graph_iri,
...
pub fn insert_version(
    ...
    let ont_iri = format!("{}/vocabulary/{}", base_url, &record.vocabulary_id);
    let ver_iri = format!(
        "{}/vocabulary/{}/version/{}",
        base_url, &record.vocabulary_id, &record.version
    );

    let creator_triple = record.created_by.as_deref()
        .map(|u| format!("    dct:creator <{}> ;\n", u))
        .unwrap_or_default();

    let derived_triple = record.derived_from.as_deref()
        .map(|v| format!(
            "    prov:wasDerivedFrom <{}/vocabulary/{}/version/{}> ;\n",
            base_url, &record.vocabulary_id, v
        ))
        .unwrap_or_default();

    let notes_triple = record.notes.as_deref()
        .map(|n| {
            let escaped = n.replace('\\', "\\\\").replace('"', "\\\"");
    ...
    let branch_triple = record.branch.as_deref()
        .map(|b| {
    ...
    let sub_graph_triples: String = record.sub_graphs.iter()
    ...
    let q = format!(
        ...
        INSERT DATA {{
          GRAPH <{REGISTRY_GRAPH}> {{
            <{ver_iri}> a ver:VocabularyVersion ;
              owl:versionInfo "{version}" ;
              ver:vocabulary <{ont_iri}> ;
              ver:status "{status}" ;
...
/// Update the ver:latestPublished pointer on a vocabulary.
...
/// Update the ver:latestDraft pointer on a vocabulary.
...
/// Remove the ver:latestDraft pointer from a vocabulary (when a draft is staged/published).
...
pub fn version_exists(
    ...
    get_version(store, base_url, vocabulary_id, version).is_some()
}

/// Check whether a vocabulary IRI already exists in the registry.
pub fn vocabulary_exists(store: &TripleStore, base_url: &str, vocabulary_id: &str) -> bool {
    get_vocabulary(store, base_url, vocabulary_id).is_some()
}
