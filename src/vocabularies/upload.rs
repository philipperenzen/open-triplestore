...
/// Detect RDF format from MIME type.
...
fn slugify_last_segment(iri: &str) -> String {
    ...
    if slug.is_empty() { "data".to_string() } else { slug }
}
...
/// - `vocabulary_id`       — ontology identifier
...
pub fn parse_and_load(
    ...
    let base_graph = format!("{base_url}/vocabulary/{vocabulary_id}/version/{version}");
    ...
    if !file_has_version_info {
        let subject_nn = detect_ontology_subject(&quads)
            .unwrap_or_else(|| {
                NamedNode::new(format!("{base_url}/vocabulary/{vocabulary_id}").as_str())
    ...
    Ok(LoadResult { version, sub_graphs })
}
...
pub fn clone_graphs_as_draft(
    ...
    let source_prefix = format!("{base_url}/vocabulary/{vocabulary_id}/version/{source_version}");
    let target_prefix = format!("{base_url}/vocabulary/{vocabulary_id}/version/{target_version}");
    ...
    let graph_pairs: Vec<(String, String)> = if source_graphs.is_empty() {
        ...
        source_graphs
            ...
            .map(|src_iri| {
                let draft_iri =
                    format!("{}{}", target_prefix, &src_iri[source_prefix.len()..]);
    ...
    for (draft_iri, src_iri) in &graph_pairs {
        ...
        for q in store.quads_for_graph(src_graph)? {
            all_quads.push(Quad::new(q.subject, q.predicate, q.object, draft_target.clone()));
        }
    }
