...
use super::models::{
    ...
    SubGraphActionRequest, UpdateVocabularyRequest, UpdateVersionRequest, VersionDataParams,
    VersionStatus,
};
use super::registry;
use super::upload;
...
pub async fn create_draft(
    ...
    let now = Utc::now().to_rfc3339();
    let graph_iri = format!("{}/vocabulary/{}/version/{}", state.base_url, id, target_ver);
...
pub async fn create_branch(
    ...
    if !state
        ...
        .map_err(|e| AppError::Internal(e.to_string()))?
    {
        return Err(AppError::Unauthorized("Write access to this vocabulary required".to_string()));
    }
...
pub async fn list_branches(
    ...
    let views: Vec<crate::data_models::handlers::BranchVersionView> =
        registry::list_versions(&state.store, &state.base_url, &id)
            ...
            .collect();
    Ok(Json(crate::data_models::handlers::build_branches(&state.store, &views)))
}
...
fn resolve_sub_graph(record: &super::models::VocabularyVersion, wanted: &str) -> Option<String> {
    ...
    record.sub_graphs.iter().find(|g| g.ends_with(wanted)).cloned()
}
