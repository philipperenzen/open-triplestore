...
fn validate_inline_shape(
    ...
    for constraint in &shape.constraints {
        results.extend(evaluate_constraint(
            store, shapes, shape_iri, focus_node,
            constraint, None, data_graphs, severity,
        ));
    }
    ...
    for prop_shape in &shape.property_shapes {
        ...
        for constraint in &prop_shape.constraints {
            results.extend(evaluate_constraint(
                store, shapes, ps_iri, focus_node,
                constraint, Some(&prop_shape.path), data_graphs, severity,
            ));
        }
    }
    ...
    match constraint {
        Constraint::Class(class_iri) => {
            ...
            if values.is_empty() && path.is_none() {
                ...
                let query = format!(
                    "ASK {{ <{}> a <{}> }}",
                    escape_sparql_iri(focus_node), escape_sparql_iri(class_iri)
                );
                if let Ok(oxigraph::sparql::QueryResults::Boolean(is_instance)) = store.query(&query) {
                    if !is_instance {
                ...
                for value in &values {
                    if value.starts_with("http://") || value.starts_with("https://") || value.starts_with("urn:") {
                        let query = format!("ASK {{ <{}> a <{}> }}", escape_sparql_iri(value), escape_sparql_iri(class_iri));
                        if let Ok(oxigraph::sparql::QueryResults::Boolean(is_instance)) = store.query(&query) {
                            if !is_instance {
                                results.push(ValidationResult {
                                    severity: severity.clone(),
                                    ...
                                    source_shape: shape_iri.to_string(),
                                    source_constraint: format!("sh:class <{}>", class_iri),
                                    message: format!("Value <{}> does not have class <{}>", value, class_iri),
                                });
                            }
                        }
                    }
            ...
            if path.is_none() {
                ...
                if !is_valid {
                    results.push(ValidationResult {
                        ...
                        message: format!("Focus node does not match expected node kind {:?}", expected),
                    });
                }
            } else {
                ...
                for (value, dt, _lang) in get_value_terms(store, focus_node, path, data_graphs) {
                    if !value_matches_node_kind(expected, &value, &dt) {
                        results.push(ValidationResult {
                            ...
                            source_shape: shape_iri.to_string(),
                            source_constraint: format!("sh:nodeKind {:?}", expected),
                            message: format!("Value does not match expected node kind {:?}", expected),
                        });
                    }
                }
            }
            ...
            for value in &values {
                if value.len() < *min_len {
                    results.push(ValidationResult {
                        ...
                        message: format!("Value length {} is less than minimum {}", value.len(), min_len),
                    });
                }
            }
        }
                    ...
                    if let Some(p) = solution.get("p") {
                        ...
                        if !ignored_properties.contains(&p_str) {
                            results.push(ValidationResult {
                                ...
                                message: format!("Property <{}> is not allowed by closed shape", p_str),
                            });
                        }
                    }
                }
        ...
        Constraint::Equals(prop_iri) => {
            let path_values: BTreeSet<String> =
                get_values(store, focus_node, path, data_graphs).into_iter().collect();
        ...
        Constraint::Disjoint(prop_iri) => {
            ...
            let intersection: BTreeSet<_> =
                path_values.intersection(&other_values).collect();
        ...
        Constraint::Not(inner_shape) => {
            ...
            let inner_violations =
                validate_inline_shape(store, shapes, focus_node, inner_shape, data_graphs, severity);
        ...
        Constraint::And(inner_shapes) => {
            ...
            for inner in inner_shapes {
                results.extend(validate_inline_shape(
                    store, shapes, focus_node, inner, data_graphs, severity,
                ));
            }
        }
        ...
        Constraint::Node(ref_shape_iri) => {
            ...
            if let Some(ref_shape) = shapes.iter().find(|s| &s.iri == ref_shape_iri) {
                ...
                results.extend(validate_inline_shape(
                    store, shapes, focus_node, &ref_shape, data_graphs, severity,
                ));
            }
        ...
        Constraint::QualifiedValueShape { shape_iri: qvs_iri, min_count, max_count } => {
            if let Some(qvs) = shapes.iter().find(|s| &s.iri == qvs_iri) {
                let qvs = qvs.clone();
                // Collect values along the path; count those that conform to qvs.
                let values = get_values(store, focus_node, path, data_graphs);
                let conforming_count = values.iter().filter(|v| {
...
fn get_values(
    ...
    let results = if let Ok(oxigraph::sparql::QueryResults::Solutions(solutions)) = store.query(&query) {
        solutions
            .filter_map(|s| s.ok())
            .filter_map(|s| {
...
fn get_value_terms(
    ...
    if let Ok(oxigraph::sparql::QueryResults::Solutions(solutions)) = store.query(&query) {
        solutions
            .filter_map(|s| s.ok())
            .map(|s| {
                let value = s.get("value").map(|v| match v {
                    oxigraph::model::Term::Literal(lit) => lit.value().to_string(),
                    oxigraph::model::Term::NamedNode(nn) => nn.as_str().to_string(),
                    _ => v.to_string(),
                }).unwrap_or_default();
