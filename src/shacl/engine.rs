...
fn load_single_shape(
    ...
    let deactivated = ask(
        store,
        &format!(
            "ASK {{ GRAPH <{shapes_graph}> {{ <{shape_iri}> <{SH}deactivated> true }} }}"
        ),
    );

    // Severity
    let severity = single_value(
        store,
        shapes_graph,
        shape_iri,
        &format!("{}severity", SH),
...
fn load_constraints(
    ...
    if let Some(v) = single_value(store, shapes_graph, shape_iri, &format!("{}minExclusive", SH)) {
        constraints.push(Constraint::MinExclusive(v));
    }
    if let Some(v) = single_value(store, shapes_graph, shape_iri, &format!("{}minInclusive", SH)) {
        constraints.push(Constraint::MinInclusive(v));
    }
    if let Some(v) = single_value(store, shapes_graph, shape_iri, &format!("{}maxExclusive", SH)) {
        constraints.push(Constraint::MaxExclusive(v));
    }
    if let Some(v) = single_value(store, shapes_graph, shape_iri, &format!("{}maxInclusive", SH)) {
    ...
    // sh:lessThanOrEquals
    for v in multi_values(store, shapes_graph, shape_iri, &format!("{}lessThanOrEquals", SH)) {
    ...
    let lang_values =
        load_rdf_list(store, shapes_graph, shape_iri, &format!("{}languageIn", SH));
    ...
    let is_closed = ask(
        store,
        &format!(
            "ASK {{ GRAPH <{shapes_graph}> {{ <{shape_iri}> <{SH}closed> true }} }}"
        ),
    );
    if is_closed {
        let ignored =
            load_rdf_list(store, shapes_graph, shape_iri, &format!("{}ignoredProperties", SH));
    ...
    if let Some(qvs_iri) =
        single_value(store, shapes_graph, shape_iri, &format!("{}qualifiedValueShape", SH))
    {
        let min_count =
            single_value(store, shapes_graph, shape_iri, &format!("{}qualifiedMinCount", SH))
                .and_then(|v| v.parse::<usize>().ok());
        let max_count =
            single_value(store, shapes_graph, shape_iri, &format!("{}qualifiedMaxCount", SH))
                .and_then(|v| v.parse::<usize>().ok());
...
fn load_property_shapes_inner(
    ...
    for ps_iri in &ps_iris {
        ...
        let description =
            single_value(store, shapes_graph, ps_iri, &format!("{}description", SH));
...
fn multi_values(
    ...
    let query = format!(
        "SELECT ?v WHERE {{ GRAPH <{shapes_graph}> {{ <{subject}> <{predicate}> ?v }} }}"
    );
...
mod tests {
    ...
    fn min_count_violation_in_named_data_graph() {
        ...
        let store = TripleStore::in_memory().unwrap();
        store.load_str(SHAPES, RdfFormat::Turtle, Some("urn:shapes")).unwrap();
        let data = r#"
            @prefix ex: <http://example.org/> .
            ex:alice a ex:Person ; ex:name "Alice" .
            ex:bob a ex:Person .
        "#;
        store.load_str(data, RdfFormat::Turtle, Some("urn:data")).unwrap();

        let report = validate(&store, "urn:shapes", &["urn:data".to_string()]).unwrap();

        assert!(!report.conforms, "expected non-conformance for bob in a named graph");
        assert!(
            report.results.iter().any(|r| r.focus_node.contains("bob")),
    ...
    fn fully_valid_data_conforms() {
        ...
        assert!(report.conforms, "expected conformance, got {:?}", report.results);
    ...
    fn node_kind_iri_on_property_flags_literal_value() {
        ...
        assert!(!report.conforms, "a literal value violates sh:nodeKind sh:IRI");
        assert!(
            report.results.iter().any(|r| r.focus_node.contains("carol")),
            "expected a nodeKind violation naming ex:carol, got {:?}",
            report.results,
        );
        assert!(
            !report.results.iter().any(|r| r.focus_node.contains("alice")),
