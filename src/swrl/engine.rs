...
impl SwrlArg {
    ...
    fn to_sparql(&self) -> String {
        match self {
            ...
            SwrlArg::Individual(iri) => format!("<{}>", iri.trim_start_matches('<').trim_end_matches('>')),
            SwrlArg::Literal {
                value,
                datatype: Some(dt),
            } => format!("\"{}\"^^<{}>", value, dt),
...
pub fn execute_rules(
    ...
    info!("Executing {} SWRL rules (max {} iterations)", rules.len(), max_iterations);
...
fn builtin_to_filter(builtin: &str, args: &[SwrlArg]) -> Option<String> {
    ...
    match builtin_local {
        ...
        "contains" if sparql_args.len() == 2 => Some(format!(
            "CONTAINS({}, {})",
            sparql_args[0], sparql_args[1]
        )),
...
mod tests {
    ...
    fn test_swrl_arg_to_sparql() {
        assert_eq!(
            SwrlArg::Variable("?x".to_string()).to_sparql(),
            "?x"
        );
        assert_eq!(
