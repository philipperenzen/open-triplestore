...
async fn main() -> anyhow::Result<()> {
    ...
    let prefix_registry = Arc::new(
        prefixes::PrefixRegistry::open(cli.data_dir.join("prefix_cache.json"))?,
    );
    info!("Prefix registry ready (cache at {:?})", cli.data_dir.join("prefix_cache.json"));
    ...
    let text_index = {
        ...
        match text_search::TextIndex::open(&tantivy_dir) {
            ...
            Err(e) => {
                tracing::warn!("Text search index could not be opened: {}. Continuing without it.", e);
                None
            }
        }
    };
    ...
    let trusted_cidrs: Vec<ipnet::IpNet> = cli.trusted_proxy_cidrs
        .split(',')
        .map(str::trim)
        .filter(|s| !s.is_empty())
