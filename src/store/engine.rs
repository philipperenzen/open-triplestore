...
impl GraphIndex {
    ...
    fn remove(&self, graph_iri: Option<&str>) {
        match graph_iri {
            Some(iri) => { self.counts.remove(&Some(iri.to_string())); }
            None => { self.counts.remove(&None); }
        }
    }
...
impl TripleStore {
    ...
    pub fn query(&self, sparql: &str) -> Result<QueryResults, StoreError> {
        ...
        let prefix_end = (0..=sparql.len().min(200)).rfind(|&i| sparql.is_char_boundary(i)).unwrap_or(0);
    ...
    pub fn update_targeted(
        ...
        if full_rebuild || affected_iris.is_empty() {
            ...
            let graphs: Vec<Option<String>> = affected_iris.iter().map(|s| Some(s.clone())).collect();
            self.graph_index.recount_specific_graphs(&self.store, &graphs);
        }
        Ok(())
    }
    ...
    pub fn batch_update(&self, statements: &[String]) -> Result<Vec<Result<(), String>>, StoreError> {
        // Parse all upfront
    ...
    pub fn bulk_insert_quads(
        ...
        let iris: Vec<Option<String>> =
            affected_graphs.iter().map(|s| Some(s.clone())).collect();
...
pub fn detect_format_from_path(path: &Path) -> Result<RdfFormat, StoreError> {
    match path.extension().and_then(|e| e.to_str()) {
        ...
        None => Err(StoreError::UnsupportedFormat(
...
mod tests {
    ...
    fn preserve_mode_duplicates_blank_nodes_on_reimport() {
        ...
        let store = TripleStore::in_memory().unwrap().with_blank_node_mode(BlankNodeMode::Preserve);
        store.load_str(BNODE_TTL, RdfFormat::Turtle, None).unwrap();
        store.load_str(BNODE_TTL, RdfFormat::Turtle, None).unwrap();
        assert_eq!(distinct_bnodes(&store).len(), 2, "preserve mode duplicates the bnode");
    }
    ...
    fn canonical_mode_makes_blank_nodes_durable_and_idempotent() {
        ...
        let store = TripleStore::in_memory().unwrap().with_blank_node_mode(BlankNodeMode::Canonical);
        store.load_str(BNODE_TTL, RdfFormat::Turtle, None).unwrap();
        let first = distinct_bnodes(&store);
        store.load_str(BNODE_TTL, RdfFormat::Turtle, None).unwrap();
        let second = distinct_bnodes(&store);
        assert_eq!(first.len(), 1, "one anonymous node");
        assert_eq!(first, second, "re-import must reproduce the same durable label");
        assert!(
            first.iter().next().unwrap().starts_with(opengraph::canonical::STABLE_PREFIX),
    ...
    fn skolem_mode_replaces_blank_nodes_with_genid_iris() {
        let store = TripleStore::in_memory().unwrap().with_blank_node_mode(BlankNodeMode::Skolem);
        store.load_str(BNODE_TTL, RdfFormat::Turtle, None).unwrap();
        // No blank nodes remain; the anonymous node is now a durable genid IRI.
        assert!(distinct_bnodes(&store).is_empty(), "no blank nodes after Skolemization");
    ...
    fn test_load_and_query() {
        ...
        "#;
        store
            .load_str(ttl, RdfFormat::Turtle, None)
            .unwrap();
