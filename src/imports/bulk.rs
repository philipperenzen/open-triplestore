...
use crate::vocabularies::upload::{
    format_from_filename, format_from_media_type, parse_quads,
};
...
fn parse_one(input: &InputFile) -> Result<(Vec<Quad>, Vec<String>), String> {
    ...
    let target_node: Option<GraphName> = match input.target_graph.as_deref() {
        Some(iri) => Some(GraphName::NamedNode(
            NamedNode::new(iri).map_err(|e| format!("Invalid target graph IRI '{iri}': {e}"))?,
        )),
        None => None,
    };
    ...
    for q in parsed {
        let final_graph = if do_split {
            ...
            target_node.clone().ok_or_else(|| {
                "No target graph supplied for triple-format file".to_string()
            })?
        } else {
...
pub fn parse_and_load_bulk(
    ...
    let parsed: Vec<Result<(Vec<Quad>, Vec<String>), String>> = inputs
        .par_iter()
        .map(parse_one)
        .collect();
    ...
    if !replace_list.is_empty() {
        ...
        let changes: Vec<GraphChange> = replace_list
            ...
            .map(|g| {
                ...
                Ok(GraphChange { graph: g.clone(), changed: incoming != live })
            })
...
mod tests {
    ...
    fn seed_one(store: &TripleStore, graph: &str) {
        ...
        store.bulk_insert_quads(vec![q], &[graph.to_string()]).unwrap();
    }
    ...
    fn replace_with_identical_data_is_flagged_unchanged() {
        ...
        assert!(!changes[0].changed, "identical re-upload should be unchanged");
    }
