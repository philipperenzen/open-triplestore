...
pub async fn bulk_import(
        ...
        match name.as_str() {
            ...
            "meta" => {
                ...
                if !txt.trim().is_empty() {
                    meta = serde_json::from_str(&txt).map_err(|e| {
                        AppError::BadRequest(format!("Invalid meta JSON: {e}"))
                    })?;
                }
            }
            "dataset_id" => {
    ...
    let before_replace = move |changes: &[GraphChange]| -> Result<(), String> {
        let Some(ds_id) = archive_ds.as_deref() else { return Ok(()) };
        ...
        if !changed.is_empty() {
            let existing =
                crate::dataset_versions::registry::list_versions(&archive_store, &archive_base, ds_id);
            ...
            crate::dataset_versions::snapshot_as_version(
                ...
                Some(&format!("Archived previous data before replace via import ({archive_bump} bump)")),
            )?;
    ...
    let result = tokio::task::spawn_blocking(move || parse_and_load_bulk(&store, inputs, before_replace))
        .await
    ...
    if let Some(ds_id) = meta.dataset_id.as_deref() {
        for file_result in &result.file_results {
            ...
            for iri in &file_result.graph_iris {
                ...
                if let Some(role) = explicit_role {
                    ...
                    let already_has_role = entries.iter()
        ...
        if let Ok(Some(ds)) = state.auth_db.get_dataset(ds_id) {
            let entries = state.auth_db.list_dataset_graph_entries(ds_id).unwrap_or_default();
...
fn detect_and_store_graph_role(state: &AppState, dataset_id: &str, graph_iri: &str) {
    ...
    let Ok(graph_name) = oxigraph::model::NamedNode::new(graph_iri) else { return };
    let Ok(quads) = state.store.quads_for_graph(GraphNameRef::NamedNode(graph_name.as_ref())) else { return };
    let detected = kind_detector::detect(&quads);
    if let Some(role) = detected.to_graph_role() {
        let _ = state.auth_db.set_dataset_graph_role(dataset_id, graph_iri, Some(role));
    }
}
