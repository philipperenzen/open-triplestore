...
fn bench_geosparql_polygon_complexity(c: &mut Criterion) {
    ...
    for &n in &[50_usize, 200] {
        let store_pts = {
            let s = fresh_store();
            s.load_str(&gen_geo_ttl(n), RdfFormat::Turtle, None).unwrap();
            s
        };
        let store_poly = {
            let s = fresh_store();
            s.load_str(&gen_polygon_geo_ttl(n), RdfFormat::Turtle, None).unwrap();
            s
        };
        ...
        group.bench_with_input(
            BenchmarkId::new("points", n),
            &store_pts,
            |b, s| b.iter(|| consume_solutions(s.query(query).unwrap())),
        );
        group.bench_with_input(
            BenchmarkId::new("polygons", n),
            &store_poly,
            |b, s| b.iter(|| consume_solutions(s.query(query).unwrap())),
        );
    }
...
fn bench_shacl_validate_clean(c: &mut Criterion) {
    ...
    for &n in &[100_usize, 500, 1_000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &store, |b, s| {
            ...
            b.iter(|| {
                open_triplestore::shacl::validate(s, shapes_graph, &dg).unwrap()
            });
        });
    }
    group.finish();
