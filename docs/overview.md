...
- **AI Assistance** — Natural-language → SPARQL plus a chat assistant, served by a pluggable LLM gateway; every accept / edit / reject is fed back to improve the model. Hidden gracefully when no gateway is configured. See [API Services & AI Queries](/docs/api-services).
