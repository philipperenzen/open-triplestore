...
| SHACL Core | `tests/shacl_conformance.rs` | spec-derived | 20 |  |
...
724 conformance tests across 26 suites; a further 390 tests in 49 integration, security and regression suites under `tests/`, plus the crate's unit tests. Only the 3 **vendored** rows run a published corpus; every other suite is hand-written and derived from the specification text.
