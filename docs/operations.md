...
[change log](versioning.md#change-log) and, if you ask it to, send that
instance some of your queries.

**Routing is off by default, and you should read "What the measurement
found" below before turning it on.** Setting `OTS_QLEVER_URL` starts the
feeder, which keeps the remote index current and costs the platform
nothing; it does not send a single query. `OTS_QLEVER_ROUTE` does that,
and the reason it is not on by default is that a measured head-to-head
found QLever answering differently from this store's own engine on 13 of
29 query shapes, and slower on all but two of them at a size the
in-memory mirror can hold. Nothing in the store depends on QLever; with
routing off it is a warm standby index and nothing more.
...
2. **Point this node at it.** `OTS_QLEVER_URL=http://qlever:7001` and
   ...
   present — and from then on applies the change log. No query is routed
   yet.
3. **Decide whether to read from it.** `OTS_QLEVER_ROUTE=analytical` (or
   `all`) begins sending queries. Do this for a store far larger than RAM,
   where the in-memory copies are off and the alternative is RocksDB — and
   having read the next section.
4. **Watch it.** `GET /api/admin/qlever/status` (admin) says whether the feed is
...
`tests/qlever_live.rs` runs 29 query shapes through this store's engine and
through a QLever holding the same 501 000 triples, and compares the
solutions as multisets. Reproduce it with the recipe in that file's
documentation. Against QLever 2026-09:

| | |
|---|--:|
| answered identically | 9 of 29 |
| answered differently | 17 |
| refused or truncated | 3 |

**Thirteen of the seventeen differ on datatype.** QLever reports an
`xsd:integer` literal as `xsd:int`. In RDF those are different terms, so
`DATATYPE(?x)` returns something else, `sameTerm` and `=` change answer,
and every `COUNT`, `SUM`, `AVG`, `MIN` and `MAX` comes back with a
datatype SPARQL does not specify for it. This is the reason routing is
off by default: it is a correctness difference, not a preference.

**Four differ only in which rows a `LIMIT` returns** with no `ORDER BY`.
SPARQL leaves that order open, so neither answer is wrong — but the
platform's answer would change depending on whether a backend happens to
be configured, which is its own kind of surprise.

**Two were truncated.** QLever answers `200 OK`, streams a partial result
and appends a plain-text error into the body — its own message says HTTP/1.1
leaves it no better way to report a failure that happens mid-export. One
100 000-row query came back with 85 852 rows that way. This store refuses
such a response outright (`parse_results` looks for the marker as well as
parsing strictly) and falls through to the engine, so a partial answer is
never served as a whole one — but an operator should know the remote can
do it.

**On speed, at this size, QLever lost.** Medians over five runs, 501 000
triples, for the queries where both agreed:

| query shape | engine | QLever |
|---|--:|--:|
| `ASK` | 0.3 ms | 48 ms |
| `VALUES` + join, 20 000 rows | 79 ms | 6 851 ms |
| `UNION`, 20 000 rows | 63 ms | 3 574 ms |
| `MINUS`, 90 000 rows | 351 ms | 15 466 ms |
| `FILTER` on an integer, 18 456 rows | 137 ms | 3 265 ms |
| `DISTINCT`, 10 rows | 105 ms | 48 ms |
| `REGEX`, 10 rows | 113 ms | 52 ms |

That is not an argument that QLever is slow — it is an argument about
*where*. Half a million triples fit in this store's in-memory mirror,
which is the case QLever is not built for; its index and its query planner
are built for hundreds of millions of triples on disk, where this store's
copies are switched off and RocksDB answers a join one point lookup at a
time. Turn routing on there, and measure it there.
...
|---|---|
| `off` (default) | nothing; the feed keeps running |
| `analytical` | an `ASK`, or a query with an aggregate anywhere in it (`GROUP BY`, `COUNT`, `SUM`, …), after the in-memory copies — with the mirror published, what the copies decline; above the cap, every such query |
...
An unrecognised value is `off`: a typo must not start routing queries away
from the engine.
...
whole. Routing adds a round trip to QLever and the parse of its result. It is a
read backend, not a replica: it holds no identity database, serves none of this
server's routes, and can be dropped at any time.

Three limits are known and not fixed. A graph resync split across several
`INSERT DATA` operations turns one blank node into several on the remote; a full
resync does not clear a graph the endpoint holds that the local store no longer
does; and `CONSTRUCT` is never routed, because QLever answers it in an RDF
serialisation rather than a results document. All three are recorded in
