...
| `OTS_QLEVER_URL` | *(unset)* | A QLever endpoint kept current from this node's change log. Setting it starts the feeder only; no query is routed until `OTS_QLEVER_ROUTE` says so. See [operations.md](operations.md#qlever-as-a-read-backend). |
...
| `OTS_QLEVER_ROUTE` | `off` | Which queries QLever sees: `off` (none — the default, and what an unrecognised value means), `analytical` (an `ASK` or a query with an aggregate, after the in-memory copies), `all` (every `SELECT`/`ASK`, after the copies), `first` (before the copies; for measurements). **Read [operations.md](operations.md#what-the-measurement-found) before turning this on**: a measured head-to-head found QLever answering differently from this store's engine on 13 of 29 query shapes, mostly by reporting `xsd:integer` literals as `xsd:int`. |
