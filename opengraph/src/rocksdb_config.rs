...
impl RocksDbConfig {
    ...
    pub fn estimated_memory_mb(&self) -> usize {
        self.block_cache_mb
            + (self.write_buffer_mb * self.max_write_buffer_number as usize)
    }
...
mod tests {
    ...
    fn test_bulk_ingest_disables_bloom() {
        ...
        assert_eq!(cfg.bloom_bits_per_key, 0, "Bloom filter should be off during bulk ingest");
        assert!(cfg.write_buffer_mb >= 256, "Write buffer should be large for bulk ingest");
    }

    #[test]
    fn test_query_heavy_has_large_cache() {
        let cfg = RocksDbConfig::for_workload(WorkloadProfile::QueryHeavy);
        assert!(cfg.block_cache_mb >= 1024, "Block cache should be large for query-heavy");
        assert!(cfg.bloom_bits_per_key >= 10, "Bloom filter should be enabled for queries");
    }
