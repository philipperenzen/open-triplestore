...
mod tests {
    ...
    fn sample() -> Vec<Quad> {
        ...
        vec![
            ...
            Quad::new(Subject::BlankNode(bnode("x")), iri("http://ex/name"),
                      Term::Literal(Literal::new_simple_literal("Alice")), GraphName::DefaultGraph),
            Quad::new(Subject::BlankNode(bnode("x")), iri("http://ex/knows"),
                      Term::BlankNode(bnode("y")), GraphName::DefaultGraph),
            Quad::new(Subject::BlankNode(bnode("y")), iri("http://ex/name"),
                      Term::Literal(Literal::new_simple_literal("Bob")), GraphName::DefaultGraph),
        ]
    }
    ...
    fn skolemizes_all_blank_nodes_to_genid_iris() {
        ...
        for iri in map.values() {
            assert!(iri.starts_with(&format!("{DEFAULT_SKOLEM_BASE}{GENID_PATH}")), "got {iri}");
            assert!(is_skolem_iri(iri, DEFAULT_SKOLEM_BASE));
        }
        // No blank nodes survive in the output.
        let joined = out.iter().map(|q| q.to_string()).collect::<Vec<_>>().join("\n");
        assert!(!joined.contains("_:"), "no blank nodes should remain: {joined}");
    }
    ...
    fn skolem_iris_are_stable_across_relabel_and_reorder() {
        ...
        let mut b = vec![
            Quad::new(Subject::BlankNode(bnode("q2")), iri("http://ex/name"),
                      Term::Literal(Literal::new_simple_literal("Bob")), GraphName::DefaultGraph),
            Quad::new(Subject::BlankNode(bnode("q1")), iri("http://ex/knows"),
                      Term::BlankNode(bnode("q2")), GraphName::DefaultGraph),
            Quad::new(Subject::BlankNode(bnode("q1")), iri("http://ex/name"),
                      Term::Literal(Literal::new_simple_literal("Alice")), GraphName::DefaultGraph),
            Quad::new(Subject::NamedNode(iri("http://ex/a")), iri("http://ex/p"),
                      Term::BlankNode(bnode("q1")), GraphName::DefaultGraph),
        ];
        b.reverse();
    ...
    fn roundtrip_skolemize_then_deskolemize_is_isomorphic() {
        ...
        let set = |qs: &[Quad]| qs.iter().map(|q| q.to_string()).collect::<std::collections::BTreeSet<_>>();
    ...
    fn custom_base_is_respected_and_trailing_slash_normalized() {
        ...
        assert_eq!(m1.values().collect::<std::collections::BTreeSet<_>>(),
