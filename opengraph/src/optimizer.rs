...
fn optimize_pattern(pattern: GraphPattern) -> GraphPattern {
    match pattern {
        GraphPattern::Bgp { patterns } => {
            ...
            GraphPattern::Bgp { patterns: reordered }
        }
...
impl QueryOptimizer {
    ...
    pub fn reorder_bgp(sparql: &str) -> String {
        ...
        let rewritten = match query {
            ...
            Query::Construct { template, pattern, dataset, base_iri } => {
                let optimized = optimize_pattern(pattern);
                Query::Construct {
                    template,
                    pattern: optimized,
...
mod tests {
    ...
    fn test_reorder_moves_bound_first() {
        ...
        assert!(alice_pos < age_pos, "Bound pattern should appear before unbound ones");
    }
    ...
    fn test_reorder_passthrough_on_update() {
        ...
        assert_eq!(result, update, "UPDATE statements should be returned unchanged");
    }
