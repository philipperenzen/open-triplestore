...
fn relabel_quad(q: &Quad, m: &BTreeMap<String, String>) -> Quad {
    let subject = match &q.subject {
        Subject::BlankNode(b) => Subject::BlankNode(BlankNode::new_unchecked(m[b.as_str()].clone())),
        other => other.clone(),
    };
...
pub fn canonicalize(quads: &[Quad]) -> Canonicalized {
    ...
    ids.sort_by(|a, b| colors[a.as_str()].cmp(&colors[b.as_str()]).then_with(|| a.cmp(b)));
...
mod tests {
    ...
    fn q_lit(s: Subject, p: &str, lit: &str) -> Quad {
        Quad::new(s, iri(p), Term::Literal(Literal::new_simple_literal(lit)), GraphName::DefaultGraph)
    }
    ...
    fn no_bnodes_returns_input_unchanged() {
        let quads = vec![q_nn(Subject::NamedNode(iri("http://ex/a")), "http://ex/p", "http://ex/b")];
    ...
    fn relabels_blank_nodes_to_c14n() {
        ...
        let quads = vec![
            Quad::new(Subject::NamedNode(iri("http://ex/a")), iri("http://ex/p"),
                      Term::BlankNode(bnode("x")), GraphName::DefaultGraph),
        ...
        let joined = c.quads.iter().map(|q| q.to_string()).collect::<Vec<_>>().join("\n");
    ...
    fn deterministic_across_relabel_and_reorder() {
        ...
        let a = vec![
            Quad::new(Subject::NamedNode(iri("http://ex/a")), iri("http://ex/p"),
                      Term::BlankNode(bnode("b1")), GraphName::DefaultGraph),
            q_lit(Subject::BlankNode(bnode("b1")), "http://ex/name", "Alice"),
            Quad::new(Subject::BlankNode(bnode("b1")), iri("http://ex/knows"),
                      Term::BlankNode(bnode("b2")), GraphName::DefaultGraph),
        ...
        let mut b = vec![
            ...
            Quad::new(Subject::BlankNode(bnode("zzz")), iri("http://ex/knows"),
                      Term::BlankNode(bnode("aaa")), GraphName::DefaultGraph),
            q_lit(Subject::BlankNode(bnode("zzz")), "http://ex/name", "Alice"),
            Quad::new(Subject::NamedNode(iri("http://ex/a")), iri("http://ex/p"),
                      Term::BlankNode(bnode("zzz")), GraphName::DefaultGraph),
        ];
        b.reverse();

        let ca = canonicalize(&a);
        let cb = canonicalize(&b);
        assert_eq!(nquad_set(&ca.quads), nquad_set(&cb.quads),
    ...
    fn rdf_list_repeated_values_are_distinguished() {
        ...
        let make = |l1: &str, l2: &str, l3: &str| {
            vec![
                ...
                Quad::new(Subject::BlankNode(bnode(l1)), iri(RDF_REST),
                          Term::BlankNode(bnode(l2)), GraphName::DefaultGraph),
                q_lit(Subject::BlankNode(bnode(l2)), RDF_FIRST, "x"),
                Quad::new(Subject::BlankNode(bnode(l2)), iri(RDF_REST),
                          Term::BlankNode(bnode(l3)), GraphName::DefaultGraph),
                q_lit(Subject::BlankNode(bnode(l3)), RDF_FIRST, "x"),
                Quad::new(Subject::BlankNode(bnode(l3)), iri(RDF_REST),
                          Term::NamedNode(iri(RDF_NIL)), GraphName::DefaultGraph),
            ]
        };
        let c = canonicalize(&make("l1", "l2", "l3"));
        let labels: BTreeSet<&String> = c.mapping.values().collect();
        assert_eq!(labels.len(), 3, "three list cells must get three distinct labels");
    ...
    fn stable_relabel_is_collision_free_and_durable() {
        // _:x :p "v"
        let make = |label: &str| {
            vec![q_lit(Subject::BlankNode(bnode(label)), "http://ex/p", "v")]
        };
        ...
        assert_eq!(nquad_set(&a), nquad_set(&b), "same structure must yield same label");
        ...
        let other = stable_relabel(&vec![q_lit(Subject::BlankNode(bnode("y")), "http://ex/DIFFERENT", "v")]);
