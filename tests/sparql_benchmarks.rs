...
fn sp2b_q1_year_of_first_article() {
    ...
    assert!(
        r[0][0].contains("2008") || r[0][0].contains("2009"),
        "First article year: {}",
        r[0][0]
    );
}
...
fn sp2b_q3b_articles_with_subject_filter() {
    ...
    assert!(
        r[0][1].to_lowercase().contains("sparql"),
        "Title should contain SPARQL: {}",
        r[0][1]
    );
}
...
fn sp2b_q6_people_coauthoring_with_homepage() {
    ...
    assert!(
        !r.is_empty(),
        "Q6 should find persons with homepage-having coauthors"
    );
}
...
fn sp2b_q8_subquery_count_papers_per_journal() {
    ...
    assert!(
        r[0][2].contains("2") || r[0][2].contains("1"),
        "Paper count: {}",
        r[0][2]
    );
}
...
fn sp2b_q11_all_publications_optional_props() {
    ...
    assert!(
        !r.is_empty(),
        "Q11 should return publications with optional properties"
    );
}
...
fn bsbm_q1_find_products_by_features() {
    ...
    assert!(
        !r.is_empty(),
        "Q1 should find products with features 1 and 2, numProp1 in (50,200)"
    );
...
fn bsbm_q4_find_products_all_features() {
    ...
    assert!(
        !r.is_empty(),
        "Q4 should find products with matching features"
    );
}
...
fn bsbm_q5_find_offers_for_product() {
    ...
    assert!(
        r[0][1].contains("89.99") || r[0][1].contains("89"),
...
fn bsbm_q9_get_cheapest_product_from_vendor() {
    ...
    assert!(
        r[0][2].contains("89.99") || r[0][2].contains("89"),
        "Cheapest price: {}",
        r[0][2]
    );
}
...
fn perf_simple_triple_pattern_under_1ms() {
    ...
    assert!(
        ...
        "Simple pattern on 100 triples should be fast: {}ms",
        elapsed.as_millis()
    );
    assert_eq!(r.len(), 10);
}
...
fn perf_aggregation_under_100ms() {
    ...
    let r = select(
        &s,
        "SELECT (COUNT(*) AS ?c) (SUM(?v) AS ?s) (AVG(?v) AS ?avg) WHERE { ?x <http://ex/val> ?v }",
    );
    let elapsed = start.elapsed();
    assert!(
        elapsed < Duration::from_millis(500),
        "Aggregation on 500 triples: {}ms",
        elapsed.as_millis()
    );
