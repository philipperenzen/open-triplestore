...
async fn gsp_put_replaces_post_merges() {
    ...
    // PUT s1
    let (st, ..) = send(&app, Method::PUT, g.clone(), Some(&token), Some("text/turtle"), None,
        "<http://ex/s1> <http://ex/p> <http://ex/o1> .").await;
    assert!(st.is_success(), "PUT 1 => {st}");

    // POST s2 — RDF merge: both present
    let (st, ..) = send(&app, Method::POST, g.clone(), Some(&token), Some("text/turtle"), None,
        "<http://ex/s2> <http://ex/p> <http://ex/o2> .").await;
    assert!(st.is_success(), "POST => {st}");
    let (st, body, _) = send(&app, Method::GET, g.clone(), Some(&token), None, Some("text/turtle"), "").await;
    assert!(st.is_success(), "GET after merge => {st}");
    assert!(body.contains("s1") && body.contains("s2"), "POST merges: {body}");

    // PUT s3 — replaces the whole graph
    let (st, ..) = send(&app, Method::PUT, g.clone(), Some(&token), Some("text/turtle"), None,
        "<http://ex/s3> <http://ex/p> <http://ex/o3> .").await;
    ...
    let (st, ..) = send(&app, Method::DELETE, g.clone(), Some(&token), None, None, "").await;
    assert!(st.is_success(), "DELETE => {st}");
    let (_st, body, _) = send(&app, Method::GET, g.clone(), Some(&token), None, Some("text/turtle"), "").await;
    assert!(!body.contains("s3"), "after DELETE the graph is empty: {body}");
}
...
async fn gsp_write_requires_auth() {
    ...
    let app = test_app(state);
    let (st, ..) = send(&app, Method::PUT, graph_uri("http://example.org/g"), None,
        Some("text/turtle"), None, "<http://ex/s> <http://ex/p> <http://ex/o> .").await;
    assert_eq!(st, StatusCode::UNAUTHORIZED, "unauthenticated GSP PUT must be 401, got {st}");
}
...
async fn app_with_data() -> (Router, String) {
    let (state, token) = admin_state();
    let app = test_app(state);
    let (st, ..) = send(&app, Method::PUT, graph_uri("http://ex/data"), Some(&token),
        Some("text/turtle"), None,
        "<http://ex/alice> <http://ex/name> \"Alice\" . <http://ex/bob> <http://ex/name> \"Bob\" .").await;
...
async fn sparql_select_content_negotiation() {
    ...
    // SELECT → JSON
    let (st, body, ct) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("application/sparql-results+json"), "").await;
    assert_eq!(st, StatusCode::OK, "json select => {st}");
    assert!(ct.as_deref().unwrap_or("").contains("json"), "json content-type, got {ct:?}");
    assert!(body.contains("Alice") && body.contains("Bob"), "json body: {body}");

    // SELECT → XML
    let (st, _b, ct) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("application/sparql-results+xml"), "").await;
    assert_eq!(st, StatusCode::OK);
    assert!(ct.as_deref().unwrap_or("").contains("xml"), "xml content-type, got {ct:?}");

    // SELECT → CSV
    let (st, body, ct) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("text/csv"), "").await;
    assert_eq!(st, StatusCode::OK);
    assert!(ct.as_deref().unwrap_or("").contains("csv"), "csv content-type, got {ct:?}");
...
async fn sparql_construct_returns_rdf() {
    ...
    let (st, body, ct) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("text/turtle"), "").await;
    assert_eq!(st, StatusCode::OK, "construct => {st}");
    assert!(ct.as_deref().unwrap_or("").contains("turtle"), "turtle content-type, got {ct:?}");
...
async fn sparql_ask_returns_json_boolean() {
    ...
    let (st, body, _ct) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("application/sparql-results+json"), "").await;
...
async fn sparql_query_via_post_body() {
    let (app, token) = app_with_data().await;
    let (st, body, _) = send(&app, Method::POST, "/sparql".to_string(), Some(&token),
        Some("application/sparql-query"), Some("application/sparql-results+json"),
        "SELECT ?n WHERE { GRAPH <http://ex/data> { ?s <http://ex/name> ?n } }").await;
...
async fn sparql_malformed_query_is_400() {
    ...
    let (st, ..) = send(&app, Method::GET, format!("/sparql?query={q}"), Some(&token),
        None, Some("application/sparql-results+json"), "").await;
    assert_eq!(st, StatusCode::BAD_REQUEST, "malformed query must be 400, got {st}");
}
...
async fn sparql_update_requires_auth() {
    let (state, _token) = admin_state();
    let app = test_app(state);
    let (st, ..) = send(&app, Method::POST, "/sparql".to_string(), None,
        Some("application/sparql-update"), None,
        "INSERT DATA { <http://ex/a> <http://ex/b> <http://ex/c> }").await;
    assert_eq!(st, StatusCode::UNAUTHORIZED, "unauthenticated UPDATE must be 401, got {st}");
}
...
async fn sparql_update_authenticated_succeeds() {
    let (state, token) = admin_state();
    let app = test_app(state);
    let (st, ..) = send(&app, Method::POST, "/sparql".to_string(), Some(&token),
        Some("application/sparql-update"), None,
        "INSERT DATA { GRAPH <http://ex/g> { <http://ex/a> <http://ex/b> <http://ex/c> } }").await;
    assert!(st.is_success(), "authenticated UPDATE must succeed, got {st}");
}
...
async fn app_with_shacl_on_write() -> (Router, String) {
    ...
    state
        .auth_db
        .create_dataset("d1", "DS", None, OwnerType::Organisation, "o1", Visibility::Public, None)
        .unwrap();
    state.auth_db.add_dataset_graph("d1", "urn:data:d1").unwrap();
    state
        .auth_db
...
async fn shacl_on_write_rejects_violation_422() {
    ...
    let (st, body, _) = send(&app, Method::PUT, graph_uri("urn:data:d1"), Some(&token),
        Some("text/turtle"), None,
        "<http://example.org/p1> a <http://example.org/Person> .").await;
    assert_eq!(
        st,
