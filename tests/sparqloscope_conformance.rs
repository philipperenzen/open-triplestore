...
fn people_dataset(n: usize) -> String {
    let mut ttl = String::from("@prefix ex: <http://example.org/> . @prefix foaf: <http://xmlns.com/foaf/0.1/> .");
    for i in 0..n {
        ttl.push_str(&format!(
...
fn scope_bgp_no_results() {
    let s = ts();
    load(&s, &people_dataset(5));
    let r = sel(&s, "SELECT ?x WHERE { ?x <http://example.org/nonexistent> ?y }");
    assert_eq!(r.len(), 0);
}
...
fn scope_bgp_bound_subject() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p 1 . ex:b ex:p 2 .");
    let r = sel(&s, "SELECT ?v WHERE { <http://example.org/a> <http://example.org/p> ?v }");
    assert_eq!(r.len(), 1);
...
fn scope_bgp_bound_object() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p 42 . ex:b ex:p 42 . ex:c ex:p 99 .");
...
fn scope_bgp_all_bound_triple() {
    ...
    assert!(ask(&s, "ASK { <http://example.org/a> <http://example.org/b> <http://example.org/c> }"));
    assert!(!ask(&s, "ASK { <http://example.org/a> <http://example.org/b> <http://example.org/x> }"));
}
...
fn scope_filter_comparison_operators() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:v 10 . ex:b ex:v 20 . ex:c ex:v 30 .");
    let q_lt = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v < 20) }");
    assert_eq!(q_lt.len(), 1);
    let q_le = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v <= 20) }");
    assert_eq!(q_le.len(), 2);
    let q_gt = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v > 10) }");
    assert_eq!(q_gt.len(), 2);
    let q_ge = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v >= 20) }");
    assert_eq!(q_ge.len(), 2);
    let q_eq = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v = 20) }");
    assert_eq!(q_eq.len(), 1);
    let q_ne = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v != 20) }");
...
fn scope_filter_logical_and_or_not() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:v 5 . ex:b ex:v 15 . ex:c ex:v 25 .");
    let r_and = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v > 3 && ?v < 20) }");
    assert_eq!(r_and.len(), 2);
    let r_or = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v < 10 || ?v > 20) }");
    assert_eq!(r_or.len(), 2);
    let r_not = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(!((?v < 10) || (?v > 20))) }");
...
fn scope_filter_string_functions() {
    ...
    let r_starts = sel(&s, "SELECT ?n WHERE { ?s <http://example.org/n> ?n FILTER(STRSTARTS(?n, \"foo\")) }");
    assert_eq!(r_starts.len(), 2);
    let r_ends = sel(&s, "SELECT ?n WHERE { ?s <http://example.org/n> ?n FILTER(STRENDS(?n, \"bar\")) }");
    assert_eq!(r_ends.len(), 1);
    let r_contains = sel(&s, "SELECT ?n WHERE { ?s <http://example.org/n> ?n FILTER(CONTAINS(?n, \"oo\")) }");
    assert_eq!(r_contains.len(), 2);
    let r_regex = sel(&s, "SELECT ?n WHERE { ?s <http://example.org/n> ?n FILTER(REGEX(?n, \"^foo\")) }");
...
fn scope_filter_type_checking() {
    let s = ts();
    load(&s, r#"@prefix ex: <http://example.org/> .
        ex:a ex:p <http://example.org/iri> .
        ex:b ex:p "literal" .
        ex:c ex:p 42 .
    "#);
    let r_iri = sel(&s, "SELECT ?o WHERE { ?s <http://example.org/p> ?o FILTER(isIRI(?o)) }");
    assert_eq!(r_iri.len(), 1);
    let r_lit = sel(&s, "SELECT ?o WHERE { ?s <http://example.org/p> ?o FILTER(isLiteral(?o)) }");
    assert_eq!(r_lit.len(), 2);
    let r_num = sel(&s, "SELECT ?o WHERE { ?s <http://example.org/p> ?o FILTER(isNumeric(?o)) }");
...
fn scope_filter_sameas() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:knows ex:b . ex:a ex:knows ex:a .");
...
fn scope_filter_in_notin() {
    ...
    let r_in = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v IN (1, 3)) } ORDER BY ?v");
    assert_eq!(r_in.len(), 2);
    assert!(r_in[0][0].contains("1"));
    assert!(r_in[1][0].contains("3"));
    let r_notin = sel(&s, "SELECT ?v WHERE { ?s <http://example.org/v> ?v FILTER(?v NOT IN (1, 3)) } ORDER BY ?v");
...
fn scope_modifier_distinct() {
    ...
    let with_d = sel(&s, "SELECT DISTINCT ?o WHERE { ?s <http://example.org/p> ?o }");
...
fn scope_modifier_limit_exact() {
    let s = ts();
    for i in 0..20 {
        update(&s, &format!("INSERT DATA {{ <http://ex/s{}> <http://ex/p> \"v{}\" }}", i, i));
    }
...
fn scope_modifier_offset_pagination() {
    ...
    let page1 = sel(&s, "SELECT ?s WHERE { ?s ?p ?o } ORDER BY ?s LIMIT 3 OFFSET 0");
    let page2 = sel(&s, "SELECT ?s WHERE { ?s ?p ?o } ORDER BY ?s LIMIT 3 OFFSET 3");
    let page3 = sel(&s, "SELECT ?s WHERE { ?s ?p ?o } ORDER BY ?s LIMIT 3 OFFSET 6");
...
fn scope_agg_group_concat_separator() {
    ...
    assert!(s_val.contains("a") && s_val.contains("b") && s_val.contains("c"),
            "GROUP_CONCAT: {}", s_val);
...
fn scope_agg_count_distinct() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . @prefix : <http://example.org/> . ex:a ex:p :x . ex:b ex:p :x . ex:c ex:p :y .");
    let r = sel(&s, "SELECT (COUNT(DISTINCT ?o) AS ?cnt) WHERE { ?s <http://example.org/p> ?o }");
    assert_eq!(r.len(), 1);
...
fn scope_path_sequence_multi_hop() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p ex:b . ex:b ex:q ex:c . ex:c ex:r ex:d .");
    let r = sel(
        &s,
...
fn scope_path_one_or_more_no_self() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:sub ex:b . ex:b ex:sub ex:c .");
    let r = sel(
        &s,
    ...
    assert_eq!(r.len(), 2);
    assert!(!r.iter().any(|row| row[0].contains("/a>")), "a should not appear in 1+ path: {:?}", r);
}

#[test]
fn scope_path_inverse_simple() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:parent ex:hasChild ex:child .");
    let r = sel(
        &s,
...
fn scope_path_negated_property_set() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p ex:x . ex:a ex:q ex:y . ex:a ex:r ex:z .");
    let r = sel(
        &s,
...
fn scope_path_complex_combination() {
    ...
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p1 ex:b . ex:b ex:p2 ex:c . ex:c ex:p1 ex:d .");
    let r = sel(
        &s,
...
fn scope_bind_arithmetic() {
    ...
    assert!(r[0][3].contains("3.5") || r[0][3].contains("3"), "div: {}", r[0][3]);
}

#[test]
fn scope_bind_string_operations() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:first \"hello\" ; ex:last \"world\" .");
    let r = sel(
        &s,
...
fn scope_bind_if_expression() {
    ...
    assert!(grades.iter().any(|g| g.contains("\"B\"")), "85 should be B: {:?}", grades);
    assert!(grades.iter().any(|g| g.contains("\"F\"")), "45 should be F: {:?}", grades);
    assert!(grades.iter().any(|g| g.contains("\"C\"")), "60 should be C: {:?}", grades);
}
...
fn scope_bind_coalesce() {
    ...
    assert!(r[1][0].contains("Bob"), "Bob falls back to name: {}", r[1][0]);
}

#[test]
fn scope_bind_iri_construction() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:id \"42\" . ex:b ex:id \"99\" .");
    let r = sel(
        &s,
...
fn scope_values_with_undef() {
    let s = ts();
    load(&s, "@prefix ex: <http://example.org/> . ex:a ex:p \"pa\" ; ex:q \"qa\" . ex:b ex:p \"pb\" .");
    let r = sel(
        &s,
...
fn scope_negation_exists_vs_minus_equivalence() {
    ...
    assert_eq!(r_fne.len(), r_minus.len(), "FNE and MINUS give same results: {:?} vs {:?}", r_fne, r_minus);
...
fn scope_update_full_lifecycle() {
    ...
    // INSERT DATA
    update(&s, "INSERT DATA { <http://ex/s> <http://ex/p> \"initial\" }");
    assert_eq!(s.len().unwrap(), 1);

    // INSERT WHERE (copy with transformation)
    update(&s, "INSERT { <http://ex/s> <http://ex/p2> ?v } WHERE { <http://ex/s> <http://ex/p> ?v }");
    assert_eq!(s.len().unwrap(), 2);

    // DELETE DATA
    update(&s, "DELETE DATA { <http://ex/s> <http://ex/p> \"initial\" }");
    assert_eq!(s.len().unwrap(), 1);
...
fn scope_update_conditional_replace() {
    ...
    update(&s, r#"DELETE { ?s <http://example.org/status> "pending" }
                  INSERT { ?s <http://example.org/status> "active" }
                  WHERE  { ?s <http://example.org/status> "pending" }"#);

    assert!(ask(&s, "ASK { <http://example.org/x> <http://example.org/status> \"active\" }"));
    assert!(ask(&s, "ASK { <http://example.org/y> <http://example.org/status> \"done\" }"));
    assert!(!ask(&s, "ASK { ?s <http://example.org/status> \"pending\" }"));
}
...
fn scope_update_named_graph_operations() {
    ...
    update(&s, "INSERT DATA { GRAPH <http://ex/g> { <http://ex/s> <http://ex/p> \"in-graph\" } }");

    let r = sel(&s, "SELECT ?v WHERE { GRAPH <http://ex/g> { ?s <http://ex/p> ?v } }");
    assert_eq!(r.len(), 1);
    assert!(r[0][0].contains("in-graph"));

    // CLEAR GRAPH
    update(&s, "CLEAR GRAPH <http://ex/g>");
    let r2 = sel(&s, "SELECT ?v WHERE { GRAPH <http://ex/g> { ?s <http://ex/p> ?v } }");
    assert_eq!(r2.len(), 0);
...
fn scope_update_copy_graph() {
    let s = ts();
    update(&s, "INSERT DATA { GRAPH <http://ex/src> { <http://ex/a> <http://ex/p> \"val\" } }");
    update(&s, "COPY <http://ex/src> TO <http://ex/dst>");

    let r = sel(&s, "SELECT ?v WHERE { GRAPH <http://ex/dst> { ?s <http://ex/p> ?v } }");
    assert_eq!(r.len(), 1);
...
fn scope_update_move_graph() {
    ...
    let src = sel(&s, "SELECT ?v WHERE { GRAPH <http://ex/src> { ?s <http://ex/p> ?v } }");
    let dst = sel(&s, "SELECT ?v WHERE { GRAPH <http://ex/dst> { ?s <http://ex/p> ?v } }");
...
fn scope_update_add_graphs() {
    let s = ts();
    update(&s, "INSERT DATA { GRAPH <http://ex/g1> { <http://ex/a> <http://ex/p> \"v1\" } }");
    update(&s, "INSERT DATA { GRAPH <http://ex/g2> { <http://ex/b> <http://ex/p> \"v2\" } }");
    update(&s, "ADD <http://ex/g1> TO <http://ex/g2>");

    let r = sel(&s, "SELECT (COUNT(*) AS ?c) WHERE { GRAPH <http://ex/g2> { ?s ?p ?o } }");
    assert!(r[0][0].contains("2"), "ADD merges g1 into g2: {}", r[0][0]);
    // g1 still exists
    let r1 = sel(&s, "SELECT (COUNT(*) AS ?c) WHERE { GRAPH <http://ex/g1> { ?s ?p ?o } }");
    assert!(r1[0][0].contains("1"), "g1 unchanged after ADD: {}", r1[0][0]);
}
...
fn scope_numeric_all_functions() {
    ...
    let tests: &[(&str, fn(&str) -> bool)] = &[
        ...
        ("SELECT (ROUND(4.5) AS ?v) WHERE {}", |r| r.contains("4") || r.contains("5")),
    ...
    for (q, check) in tests {
        let r = sel(&s, q);
        assert!(!r.is_empty() && check(&r[0][0]), "Query: {}\nResult: {:?}", q, r);
    }
}
...
fn scope_string_all_functions() {
    ...
    let tests: &[(&str, &str)] = &[
        ...
        ("SELECT (STRSTARTS(\"foobar\", \"foo\") AS ?v) WHERE {}", "true"),
        ("SELECT (STRENDS(\"foobar\", \"bar\") AS ?v) WHERE {}", "true"),
        ("SELECT (CONTAINS(\"foobar\", \"oba\") AS ?v) WHERE {}", "true"),
        ("SELECT (STRBEFORE(\"abc\", \"b\") AS ?v) WHERE {}", "a"),
        ("SELECT (STRAFTER(\"abc\", \"b\") AS ?v) WHERE {}", "c"),
        ("SELECT (REPLACE(\"hello\", \"l\", \"L\") AS ?v) WHERE {}", "heLLo"),
        ("SELECT (STR(42) AS ?v) WHERE {}", "42"),
        ("SELECT (REGEX(\"foobar\", \"^foo\") AS ?v) WHERE {}", "true"),
        ("SELECT (REGEX(\"foobar\", \"^baz\") AS ?v) WHERE {}", "false"),
    ];

    for (q, expected) in tests {
        let r = sel(&s, q);
        assert!(!r.is_empty(), "Empty result for: {}", q);
        assert!(r[0][0].contains(expected),
                "Query: {}\nExpected: {}\nGot: {}", q, expected, r[0][0]);
    }
}
...
fn scope_hash_functions_correct_values() {
    ...
    assert!(md5[0][0].contains("900150983cd24fb0d6963f7d28e17f72"),
            "MD5(abc): {}", md5[0][0]);

    let sha1 = sel(&s, "SELECT (SHA1(\"abc\") AS ?v) WHERE {}");
    assert!(sha1[0][0].to_lowercase().contains("a9993e364706816aba3e25717850c26c9cd0d89d"),
            "SHA1(abc): {}", sha1[0][0]);

    let sha256 = sel(&s, "SELECT (SHA256(\"abc\") AS ?v) WHERE {}");
    assert!(sha256[0][0].contains("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"),
            "SHA256(abc): {}", sha256[0][0]);

    let sha512 = sel(&s, "SELECT (SHA512(\"abc\") AS ?v) WHERE {}");
    assert!(sha512[0][0].to_lowercase().starts_with("\"ddaf35a193617aba"),
            "SHA512(abc) prefix: {}", sha512[0][0]);
}
...
fn scope_datetime_extraction() {
    ...
    for (q, expected) in tests {
        let r = sel(&s, q);
        assert!(!r.is_empty(), "Empty result for: {}", q);
        assert!(r[0][0].contains(expected), "Query: {}\nExpected: {}\nGot: {}", q, expected, r[0][0]);
    }
}
...
fn scope_full_coverage_matrix() {
    ...
    // 2. Filter
    let r2 = sel(&s, &pq("SELECT ?n WHERE { ?s foaf:name ?n ; ex:age ?a FILTER(?a > 28) } ORDER BY ?n"));
    ...
    let r6 = sel(&s, &pq("SELECT (AVG(?sal) AS ?avg) WHERE { ?s ex:salary ?sal }"));
    ...
    // 9. Property path
    let r9 = sel(&s, &pq("SELECT ?managed WHERE { ?mgr ex:manages+ ?managed }"));
    ...
    let r12 = sel(&s, &pq("SELECT ?n WHERE { VALUES ?s { ex:alice ex:carol } ?s foaf:name ?n } ORDER BY ?n"));
    assert_eq!(r12.len(), 2);
