...
fn test_rdfs3_range_iri_only() {
    ...
    assert!(
        !ask_in_tg(&s, "\"Alice\" rdf:type <http://example.org/Name> ."),
...
fn test_rdfs5_subproperty_transitivity() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/fatherOf> rdfs:subPropertyOf <http://example.org/ancestorOf> ."
        ),
...
fn test_rdfs11_subclass_transitivity() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/Manager> rdfs:subClassOf <http://example.org/Person> ."
        ),
...
fn test_rdfs11_three_level() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/A> rdfs:subClassOf <http://example.org/D> ."
        ),
...
fn test_rdfs12_container_membership_property() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/_1> rdfs:subPropertyOf rdfs:member ."
        ),
...
fn test_rdfs13_datatype_subclass_literal() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/MyDT> rdfs:subClassOf rdfs:Literal ."
        ),
...
fn test_rdfs6_property_reflexive() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/knows> rdfs:subPropertyOf <http://example.org/knows> ."
        ),
...
fn test_rdfs8_class_subclass_resource() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/Person> rdfs:subClassOf rdfs:Resource ."
        ),
...
fn test_rdfs10_class_reflexive() {
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/Person> rdfs:subClassOf <http://example.org/Person> ."
        ),
...
fn test_rdfs4a_subject_is_resource() {
    ...
    materialize(&s);
    assert!(
        ask_in_tg(&s, "<http://example.org/alice> rdf:type rdfs:Resource ."),
...
fn test_rdfs4b_iri_object_is_resource() {
    ...
    materialize(&s);
    assert!(
        ask_in_tg(&s, "<http://example.org/bob> rdf:type rdfs:Resource ."),
...
fn test_combined_domain_and_subclass() {
    let s = store_with(
        "ex:teaches rdfs:domain ex:Instructor . \
    ...
    assert!(
        ask_in_tg(
            &s,
            "<http://example.org/alice> rdf:type <http://example.org/Instructor> ."
        ),
...
fn test_idempotent_double_materialization() {
        ...
        .unwrap()
    {
        oxigraph::sparql::QueryResults::Solutions(mut sols) => sols
            ...
            .get("c")
            .and_then(|v| {
                if let oxigraph::model::Term::Literal(l) = v {
                    l.value().parse::<usize>().ok()
                } else {
                    None
                }
            })
