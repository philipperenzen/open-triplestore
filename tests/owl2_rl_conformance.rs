...
fn test_prp_dom() {
    ...
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/x> rdf:type <http://example.org/C> ."), "prp-dom");
}
...
fn test_prp_rng() {
    ...
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/y> rdf:type <http://example.org/C> ."), "prp-rng");
}
...
fn test_prp_symp() {
    ...
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/bob> <http://example.org/knows> <http://example.org/alice> ."),
        "prp-symp: symmetric property");
}
...
#[test]
fn test_prp_trp() {
    let s = store_with("ex:partOf rdf:type owl:TransitiveProperty . \
                        ex:a ex:partOf ex:b . ex:b ex:partOf ex:c .");
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/a> <http://example.org/partOf> <http://example.org/c> ."),
...
#[test]
fn test_prp_spo1() {
    let s = store_with("ex:fatherOf rdfs:subPropertyOf ex:parentOf . ex:bob ex:fatherOf ex:alice .");
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/bob> <http://example.org/parentOf> <http://example.org/alice> ."),
        "prp-spo1");
}
...
fn test_prp_fp_merges() {
    let s = store_with("ex:hasSSN rdf:type owl:FunctionalProperty . \
                        ex:alice ex:hasSSN ex:SSN1 . ex:alice ex:hasSSN ex:SSN2 .");
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/SSN1> owl:sameAs <http://example.org/SSN2> ."),
...
fn test_prp_ifp_merges() {
    let s = store_with("ex:hasEmail rdf:type owl:InverseFunctionalProperty . \
...
fn test_cls_svf_typing() {
    let s = store_with("ex:HasChild owl:someValuesFrom ex:Person ; owl:onProperty ex:hasChild . \
                        ex:Employee rdfs:subClassOf ex:HasChild . \
                        ex:alice rdf:type ex:Employee . ex:alice ex:hasChild ex:bob . \
                        ex:bob rdf:type ex:Person .");
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/alice> rdf:type <http://example.org/HasChild> .") ||
            ask_tg(&s, "<http://example.org/alice> rdf:type <http://example.org/Employee> ."),
...
#[test]
fn test_cax_eqc() {
    let s = store_with("ex:Employee owl:equivalentClass ex:Worker . ex:alice rdf:type ex:Employee .");
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/alice> rdf:type <http://example.org/Worker> ."),
...
fn test_cax_dw_inconsistency() {
    ...
    assert!(check_inconsistency(&s), "cax-dw: disjointWith should be inconsistent");
}
...
fn test_cax_adc_inconsistency() {
    ...
    assert!(check_inconsistency(&s), "cax-adc: AllDisjointClasses expansion + inconsistency");
}
...
fn test_scm_cls() {
    ...
    materialize(&s);
    assert!(ask_tg(&s, "<http://example.org/Person> rdfs:subClassOf owl:Thing ."),
...
fn test_scm_int() {
    ...
    materialize(&s);
    assert!(
        ask_tg(&s, "<http://example.org/AB> rdfs:subClassOf <http://example.org/A> ."),
        "scm-int: intersection member entailment A"
    );
    assert!(
        ask_tg(&s, "<http://example.org/AB> rdfs:subClassOf <http://example.org/B> ."),
...
fn test_scm_uni() {
    ...
    materialize(&s);
    assert!(
        ask_tg(&s, "<http://example.org/A> rdfs:subClassOf <http://example.org/AorB> ."),
        "scm-uni: union member entailment A"
    );
    assert!(
        ask_tg(&s, "<http://example.org/B> rdfs:subClassOf <http://example.org/AorB> ."),
...
fn test_prp_npa1_inconsistency() {
    ...
    assert!(check_inconsistency(&s), "prp-npa1: NegativePropertyAssertion violation");
}
...
fn test_prp_npa2_data_inconsistency() {
    ...
    assert!(check_inconsistency(&s), "prp-npa2: NegativePropertyAssertion data violation");
}
...
fn test_cls_maxc1_zero_cardinality() {
    ...
    assert!(check_inconsistency(&s), "cls-maxc1: maxCardinality 0 should be inconsistent");
}
...
fn test_multi_rule_interaction() {
    ...
    assert!(ask_tg(&s, "<http://example.org/bob> rdf:type <http://example.org/Parent> ."),
