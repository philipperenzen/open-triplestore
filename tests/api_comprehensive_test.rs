...
mod helpers {
    ...
    pub fn test_state() -> AppState {
        ...
        let audit = Arc::new(open_triplestore::auth::audit::AuditLogger::new(auth_db.pool()));
        AppState {
    ...
    pub fn mint_token(user_id: &str, username: &str, role: &str) -> String {
        issue_access_token(&JwtConfig::new(JWT_SECRET.to_string(), 30, 30), user_id, username, role).unwrap()
    }

    /// Returns (state, admin_token) — creates a super_admin user in the state.
    pub fn admin_state() -> (AppState, String) {
        let state = test_state();
        state.auth_db.create_user("adm", "admin", "admin@test.com", "hash", SystemRole::SuperAdmin).unwrap();
    ...
    pub async fn register(app: Router, username: &str, email: &str, password: &str) -> (StatusCode, Value) {
...
mod auth {
    ...
    async fn first_user_becomes_super_admin() {
        ...
        assert_eq!(json["user"]["role"], "super_admin", "First user must be super_admin: {json}");
    }

    #[tokio::test]
    async fn second_user_becomes_user() {
        let state = test_state();
        let (s1, _) = register(test_app(state.clone()), "alice", "alice@ex.com", "password123").await;
        assert_eq!(s1, StatusCode::CREATED);
        let (s2, json) = register(test_app(state), "bob", "bob@ex.com", "password123").await;
        assert_eq!(s2, StatusCode::CREATED, "{json}");
        assert_eq!(json["user"]["role"], "user", "Second user must be 'user': {json}");
    }
    ...
    async fn login_returns_tokens() {
        ...
        let (status, json) = register(test_app(state), "testlogin", "tl@ex.com", "password123").await;
        assert_eq!(status, StatusCode::CREATED, "{json}");
        assert!(json["access_token"].is_string(), "access_token missing: {json}");
        assert!(json["refresh_token"].is_string(), "refresh_token missing: {json}");
    }

    #[tokio::test]
    async fn login_wrong_password() {
        let state = test_state();
        register(test_app(state.clone()), "alice2", "a2@ex.com", "correctpassword").await;
    ...
    async fn refresh_token_works() {
        let state = test_state();
        let (_, json) = register(test_app(state.clone()), "alice3", "a3@ex.com", "password123").await;
        ...
        assert!(resp_json["access_token"].is_string(), "New access_token missing: {resp_json}");
    }

    #[tokio::test]
    async fn logout_invalidates_refresh() {
        let state = test_state();
        let (_, json) = register(test_app(state.clone()), "alice4", "a4@ex.com", "password123").await;
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "Revoked refresh token must be rejected");
    }
    ...
    async fn wrong_secret_jwt_rejected() {
        let state = test_state();
        state.auth_db.create_user("u1", "eve", "e@t.com", "h", SystemRole::User).unwrap();
    ...
    async fn tampered_jwt_rejected() {
        let state = test_state();
        state.auth_db.create_user("u1", "frank", "f@t.com", "h", SystemRole::User).unwrap();
    ...
    async fn api_token_create_use_revoke() {
        let state = test_state();
        state.auth_db.create_user("u1", "grace", "g@t.com", "h", SystemRole::User).unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED, "Creating API token must return 201");
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::OK, "API token must work on protected endpoint");
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "Revoked API token must be rejected");
    }

    #[tokio::test]
    async fn change_password() {
        let state = test_state();
        let (_, reg_json) = register(test_app(state.clone()), "hank", "hank@ex.com", "oldpass123").await;
        ...
        assert_eq!(status, StatusCode::UNAUTHORIZED, "Old password must be rejected after change");
    ...
    async fn duplicate_username_rejected() {
        let state = test_state();
        let (s1, _) = register(test_app(state.clone()), "irene", "irene@ex.com", "password123").await;
        assert_eq!(s1, StatusCode::CREATED);
        let (s2, _) = register(test_app(state), "irene", "irene2@ex.com", "password123").await;
        assert_eq!(s2, StatusCode::CONFLICT, "Duplicate username must return 409");
    }

    #[tokio::test]
    async fn admin_promotes_user() {
        let (state, admin_token) = admin_state();
        state.auth_db.create_user("u1", "jack", "j@t.com", "h", SystemRole::User).unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["role"], "admin", "Role must be 'admin' after promotion: {json}");
    }

    #[tokio::test]
    async fn admin_purges_user() {
        let (state, admin_token) = admin_state();
        state.auth_db.create_user("u2", "karen", "k@t.com", "h", SystemRole::User).unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "Deactivate must succeed before purge");
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::NOT_FOUND, "Purged user must not exist");
    }
    ...
    async fn user_cannot_access_admin_endpoint() {
        let state = test_state();
        state.auth_db.create_user("u1", "leo", "l@t.com", "h", SystemRole::User).unwrap();
...
mod sparql_protocol {
    ...
    fn insert_public_graph(state: &open_triplestore::server::AppState) {
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", open_triplestore::auth::models::SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://ex.org/pub").unwrap();
    ...
    async fn get_select_json() {
        ...
        assert!(ct.contains("sparql-results+json"), "Expected sparql-results+json, got: {ct}");
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("hello"), "Results must contain inserted literal: {body}");
    }
    ...
    async fn get_ask_query() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["boolean"], true, "ASK must return true for existing data: {json}");
    }
    ...
    async fn get_construct_turtle() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let ct = resp.headers().get(header::CONTENT_TYPE).unwrap().to_str().unwrap().to_string();
        assert!(ct.contains("turtle") || ct.contains("text/"), "Expected Turtle, got: {ct}");
    }
        ...
        let resp = test_app(state)
            .oneshot(
                Request::builder()
                    ...
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .body(Body::from("INSERT DATA { <http://ex.org/a> <http://ex.org/b> <http://ex.org/c> }"))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert!(resp.status().is_success(), "Authenticated SPARQL UPDATE must succeed");
    }
    ...
    async fn accept_results_xml() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let ct = resp.headers().get(header::CONTENT_TYPE).unwrap().to_str().unwrap().to_string();
        assert!(ct.contains("sparql-results+xml") || ct.contains("xml"), "Expected XML, got: {ct}");
    }
    ...
    async fn malformed_sparql_returns_400() {
        let resp = test_app(test_state())
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!("/sparql?query={}", url_encode("THIS IS NOT@@@ SPARQL")))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert!(resp.status().is_client_error(), "Malformed SPARQL must return 4xx, got {}", resp.status());
        let body = body_text(resp.into_body()).await;
        assert!(!body.to_lowercase().contains("oxigraph"), "Internal lib name must not leak: {body}");
    }
    ...
    async fn missing_query_param_returns_400() {
        let resp = test_app(test_state())
            ...
            .await
            .unwrap();
        assert!(resp.status().is_client_error(), "Missing query param must return 4xx, got {}", resp.status());
    }
    ...
    async fn select_hides_private_graph() {
        ...
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", open_triplestore::auth::models::SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Priv", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://secret.ex.org/graph").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://secret.ex.org/graph> { <s:x> <p:y> \"classified\" } }").unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(!body.contains("classified"), "Unauthenticated must not see private data: {body}");
    }
    ...
    async fn admin_sees_all_graphs() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Priv", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://secret.ex.org/admin-test").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://secret.ex.org/admin-test> { <s:x> <p:y> \"admin-visible\" } }").unwrap();
        state.auth_db.create_user("adm", "adm", "adm@t.com", "h", open_triplestore::auth::models::SystemRole::SuperAdmin).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("admin-visible"), "Admin must see private graph data: {body}");
    }
...
mod graph_store {
    ...
    async fn put_and_get_named_graph() {
        ...
        let resp = test_app(state.clone())
            .oneshot(
                Request::builder()
                    ...
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .body(Body::from("<http://ex.org/a> <http://ex.org/b> <http://ex.org/c> ."))
                    .unwrap(),
            )
            .await
            .unwrap();
        assert!(resp.status().is_success(), "PUT must succeed: {}", resp.status());
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("ex.org/a") || body.contains("ex.org/c"), "Returned graph must contain inserted triples: {body}");
    }
    ...
    async fn put_replaces_graph() {
        ...
        put("<http://ex.org/first> <http://ex.org/p> \"first\" .", state.clone(), token.clone()).await;
        put("<http://ex.org/second> <http://ex.org/p> \"second\" .", state.clone(), token.clone()).await;
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let body = body_text(resp.into_body()).await;
        assert!(!body.contains("first"), "First triple must be gone after replace: {body}");
        assert!(body.contains("second"), "Second triple must be present: {body}");
    }
    ...
    async fn post_merges_graph() {
            ...
            test_app(state.clone())
                .oneshot(
                    Request::builder()
                        ...
                        .header(header::CONTENT_TYPE, "text/turtle")
                        .header(header::AUTHORIZATION, format!("Bearer {token}"))
                        .body(Body::from(format!("<http://ex.org/s> <http://ex.org/p> {literal} .")))
                        .unwrap(),
                )
                .await
                .unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("first"), "POST must merge: first triple missing: {body}");
        assert!(body.contains("second"), "POST must merge: second triple missing: {body}");
    }
    ...
    async fn delete_graph() {
        ...
        test_app(state.clone())
            .oneshot(
                Request::builder()
                    ...
                    .header(header::CONTENT_TYPE, "text/turtle")
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .body(Body::from("<http://ex.org/x> <http://ex.org/p> <http://ex.org/y> ."))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        assert!(!body.contains("ex.org/x"), "Graph must be empty after DELETE: {body}");
    }
    ...
    async fn payload_too_large_returns_413() {
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::PAYLOAD_TOO_LARGE, "52 MB body must return 413");
    }
}
...
mod datasets {
    ...
    async fn create_and_get_dataset() {
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED, "Create dataset must return 201");
        let created = body_json(resp.into_body()).await;
    ...
    async fn public_dataset_visible_to_anon() {
        let state = test_state();
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", SystemRole::SuperAdmin).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Public DS", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("Public DS"), "Public dataset must appear in unauthenticated list: {body}");
    }

    #[tokio::test]
    async fn private_dataset_hidden_from_anon() {
        let state = test_state();
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Secret DS", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(!body.contains("Secret DS"), "Private dataset must not appear for anon: {body}");
    }
    ...
    async fn update_dataset() {
        let (state, token) = admin_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Old Name", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "Update must succeed: {}", resp.status());
    ...
    async fn delete_dataset() {
        let (state, token) = admin_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "To Delete", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let body = body_text(resp.into_body()).await;
        assert!(!body.contains("To Delete"), "Deleted dataset must not appear in list: {body}");
    }
    ...
    async fn org_crud() {
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED, "Create org must return 201: {}", resp.status());
        let created = body_json(resp.into_body()).await;
    ...
    async fn group_crud() {
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED, "Create group must return 201: {}", resp.status());
    ...
    async fn create_sparql_service() {
        let (state, token) = admin_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "DS", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED, "Create service must return 201: {}", resp.status());
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::OK, "List services must return 200: {}", resp.status());
    ...
    async fn non_owner_cannot_delete_dataset() {
        ...
        state.auth_db.create_user("u_alice", "alice", "a@t.com", "h", SystemRole::SuperAdmin).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Alice DS", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();

        // Another user without org membership
        state.auth_db.create_user("u_bob", "bob", "b@t.com", "h", SystemRole::User).unwrap();
        ...
        assert!(
            ...
            "Non-owner must not delete dataset, got {}", resp.status()
        );
    }
}
...
"#;
        ...
        assert!(
            ...
            "SHACLC parse must return 2xx or 4xx, got {}", resp.status()
        );
    }
...
"#;
        ...
        assert!(
            resp.status().is_success() || resp.status().is_client_error(),
            "SHACLC serialize must return 2xx or 4xx, got {}", resp.status()
        );
    }
    ...
    async fn shapes_put_get_roundtrip() {
        let (state, token) = admin_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "DS", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();
        state.auth_db.update_dataset_shacl("d1", false, Some("http://ex.org/shapes")).unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "PUT shapes must succeed: {}", resp.status());
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("NodeShape") || body.contains("shacl"), "Shapes response must contain SHACL: {body}");
    }
}
...
"#;
    ...
    async fn put_rml_mapping() {
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "PUT RML mapping must succeed: {}", resp.status());
    }
    ...
    async fn rml_preview() {
        ...
        assert!(
            resp.status().is_success() || resp.status().is_client_error(),
            "RML preview must return 2xx or 4xx, got {}", resp.status()
        );
    }
}
...
mod acl {
    ...
    use tower::ServiceExt as _;
    use open_triplestore::auth::{
        models::{OwnerType, SystemRole, Visibility},
    };
    ...
    async fn grant_and_revoke_graph_permission() {
        let (state, admin_token) = admin_state();
        state.auth_db.create_user("u1", "dave", "d@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Priv", None, OwnerType::Organisation, "o1", Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://priv.ex.org/g").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://priv.ex.org/g> { <s:x> <p:y> \"private-val\" } }").unwrap();
        ...
        let resp = test_app(state.clone())
            ...
            .unwrap();
        let body = body_text(resp.into_body()).await;
        assert!(!body.contains("private-val"), "User must not see private graph before grant: {body}");
        ...
        let resp = test_app(state.clone())
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "Grant permission must succeed: {}", resp.status());
        ...
        let resp = test_app(state.clone())
            ...
            .unwrap();
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("private-val"), "User must see private graph after read grant: {body}");
        ...
        if !acl_id.is_empty() {
            ...
            let resp = test_app(state)
                ...
                .unwrap();
            let body = body_text(resp.into_body()).await;
            assert!(!body.contains("private-val"), "User must not see private graph after revoke: {body}");
        }
    }
    ...
    async fn endpoint_acl_deny_blocks_user() {
        let state = test_state();
        state.auth_db.create_user("u1", "bob", "b@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_user("adm", "admin2", "adm@t.com", "h", SystemRole::Admin).unwrap();
        state.auth_db.create_endpoint_acl_rule("rule1", "user", "u1", "/api/browse/**", "*", "deny", 10, "adm").unwrap();
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::FORBIDDEN, "Deny ACL rule must block user");
    }
    ...
    async fn triple_security_label_created() {
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert!(resp.status().is_success(), "Creating triple security label must succeed: {}", resp.status());
    }

    #[tokio::test]
    async fn acl_admin_endpoints_require_admin() {
        let state = test_state();
        state.auth_db.create_user("u1", "carol", "c@t.com", "h", SystemRole::User).unwrap();
        let token = mint_token("u1", "carol", "user");

        for path in &["/api/admin/acl/endpoints", "/api/admin/acl/graphs", "/api/admin/acl/triples"] {
            let resp = test_app(state.clone())
                .oneshot(
                    Request::builder()
                        .method(Method::GET)
                ...
                .await
                .unwrap();
            assert_eq!(resp.status(), StatusCode::FORBIDDEN, "Non-admin must be forbidden from {path}");
        }
    }
}
...
mod performance {
    ...
    async fn bulk_insert_ntriples(n: usize, max_secs: u64) {
        ...
        assert!(resp.status().is_success(), "Bulk insert of {n} triples must succeed: {}", resp.status());
        assert!(elapsed < Duration::from_secs(max_secs), "Bulk insert of {n} triples took {:?}, expected < {max_secs}s", elapsed);
    }
    ...
    async fn aggregation_query_perf() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).ok();
        state.auth_db.create_dataset("d1", "PerfDS", None, OwnerType::Organisation, "o1", Visibility::Public, None).ok();
        state.auth_db.add_dataset_graph("d1", "http://perf.ex.org/agg-graph").ok();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["results"]["bindings"][0]["count"]["value"], "5000", "COUNT must return 5000: {json}");
        assert!(elapsed < Duration::from_secs(10), "Aggregation took {:?}, expected < 10s", elapsed);
    }
    ...
    async fn concurrent_reads_10() {
        ...
        let state = test_state();
        state.auth_db.create_user("u", "u", "u@t.com", "h", open_triplestore::auth::models::SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "DS", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://perf.ex.org/concurrent").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://perf.ex.org/concurrent> { <s:x> <p:y> \"v\" } }").unwrap();
        ...
        let elapsed = start.elapsed();
        assert!(elapsed < Duration::from_secs(10), "10 concurrent reads took {:?}, expected < 10s", elapsed);
    }
}
...
mod security {
    ...
    async fn sql_injection_in_username() {
        ...
        let state = test_state();
        let (status, _) = register(test_app(state), "'; DROP TABLE users; --", "inject@ex.com", "password123").await;
        assert_ne!(status, StatusCode::INTERNAL_SERVER_ERROR, "SQL injection must not cause 500");
    ...
    async fn iri_injection_in_graph_name() {
        ...
        assert!(
            ...
            "IRI with angle bracket must be rejected, got {}", resp.status()
        );
    }
    ...
    async fn browse_triples_iri_injection() {
        ...
        let resp = test_app(test_state())
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!("/api/browse/triples?subject={}", url_encode(malicious)))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "IRI injection attempt must return 400");
    }
    ...
    async fn cors_no_allow_origin_without_config() {
        ...
        if let Some(val) = acao {
            assert_ne!(val, "http://evil.example.com", "Evil origin must not be reflected in CORS header");
        }
    }
    ...
    async fn payload_too_large_sparql_batch_413() {
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::PAYLOAD_TOO_LARGE, "Oversized batch body must return 413");
    }
    ...
    async fn csp_header_on_all_responses() {
        ...
        assert!(csp.to_str().unwrap().contains("default-src 'self'"), "CSP must contain default-src 'self', got: {:?}", csp);
    }
    ...
    async fn internal_error_detail_not_leaked() {
        let resp = test_app(test_state())
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!("/sparql?query={}", url_encode("THIS IS NOT VALID SPARQL!!!")))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert!(resp.status().is_client_error(), "Invalid SPARQL must return 4xx, got {}", resp.status());
        let body = body_text(resp.into_body()).await;
        assert!(!body.to_lowercase().contains("oxigraph"), "Internal library name must not leak: {body}");
    }
}
...
mod browse {
    ...
    async fn browse_graphs_empty_store() {
        ...
        assert!(body.contains("[]") || body.contains("\"graphs\":[]") || body == "[]" || body.len() < 100,
    ...
    async fn browse_graphs_returns_public() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://pub.ex.org/visible").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://pub.ex.org/visible> { <s:x> <p:y> <o:z> } }").unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("pub.ex.org"), "Public graph IRI must appear in browse/graphs: {body}");
    }
    ...
    async fn browse_stats_empty() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["total_triples"], 0, "Empty store must report 0 triples: {json}");
    }
    ...
    async fn browse_stats_counts_triples() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://stats.ex.org/g").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://stats.ex.org/g> { <s:a> <p:b> <o:c> ; <p:d> <o:e> } }").unwrap();
    ...
    async fn browse_triples_iri_injection() {
        ...
        let resp = test_app(test_state())
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!("/api/browse/resource?iri={}", url_encode(malicious)))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "IRI with '>' must return 400");
    }
    ...
    async fn browse_suggest() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://suggest.ex.org/g").unwrap();
        ...
        let resp = test_app(state)
            .oneshot(
                Request::builder()
                    .method(Method::GET)
                    .uri(format!("/api/browse/suggest?prefix={}&field=subject", url_encode("http://suggest.ex")))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn browse_triples_default_uses_has_more_not_total() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://has.ex.org/g").unwrap();
        // 3 triples in the graph
        state.store.update("INSERT DATA { GRAPH <http://has.ex.org/g> { <s:a> <p:p> <o:1> , <o:2> , <o:3> } }").unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["triples"].as_array().unwrap().len(), 2, "Must truncate to limit: {json}");
        assert_eq!(json["hasMore"], true, "Third triple exists, hasMore must be true: {json}");
        assert!(json.get("total").is_none(), "Default response must NOT include total: {json}");
    }
    ...
    async fn browse_triples_has_more_false_when_done() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://end.ex.org/g").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://end.ex.org/g> { <s:a> <p:p> <o:1> } }").unwrap();
        ...
        assert_eq!(json["hasMore"], false, "Single row must report hasMore=false: {json}");
    }
    ...
    async fn browse_triples_count_opt_in_returns_total() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://cnt.ex.org/g").unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["total"].as_u64(), Some(4), "count=true must return exact total: {json}");
    ...
    async fn browse_triples_count_is_exact_above_former_cap() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://big.ex.org/g").unwrap();

        let n = 100_002usize;
        let mut data = String::with_capacity(n * 48);
        data.push_str("INSERT DATA { GRAPH <http://big.ex.org/g> {");
        for i in 0..n {
            data.push_str(&format!(" <http://big.ex.org/s/{i}> <http://big.ex.org/p> <http://big.ex.org/o/{i}> ."));
        }
        data.push_str(" } }");
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["total"].as_u64(), Some(n as u64), "count must be exact above the former cap: {json}");
    }
    ...
    async fn browse_triples_count_zero_when_nothing_accessible() {
        let state = test_state();
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_dataset("priv", "Priv", None, OwnerType::User, "u1", Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("priv", "http://priv.ex.org/g").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://priv.ex.org/g> { <s:secret> <p:p> <o:1> } }").unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["triples"].as_array().unwrap().len(), 0, "private data must not leak: {json}");
        assert_eq!(json["total"].as_u64(), Some(0), "empty scope with count=true must report 0: {json}");
    }
    ...
    async fn browse_triples_anonymous_scopes_to_public() {
        ...
        state.auth_db.create_user("u1", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("pub", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.create_dataset("priv", "Priv", None, OwnerType::User, "u1", Visibility::Private, None).unwrap();
        state.auth_db.add_dataset_graph("pub", "http://pub.ex.org/g").unwrap();
        state.auth_db.add_dataset_graph("priv", "http://priv.ex.org/g").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://pub.ex.org/g> { <s:pub> <p:p> <o:1> } }").unwrap();
        state.store.update("INSERT DATA { GRAPH <http://priv.ex.org/g> { <s:secret> <p:p> <o:1> } }").unwrap();
        ...
        assert!(!body.contains("s:secret"), "Private subject must NOT appear: {body}");
    }
    ...
    async fn accessible_graphs_cache_consistency() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset("d1", "Pub", None, OwnerType::Organisation, "o1", Visibility::Public, None).unwrap();
        state.auth_db.add_dataset_graph("d1", "http://cache.ex.org/g").unwrap();

        let (uncached, _) = state.auth_db.get_accessible_graph_iris(None).unwrap();
        let cached = state.auth_db.get_accessible_graph_iris_cached(None).unwrap();
        assert_eq!(uncached, cached.0, "Cached result must match uncached");
        // Second call hits the cache path; must still match.
        let cached2 = state.auth_db.get_accessible_graph_iris_cached(None).unwrap();
        assert_eq!(cached.0, cached2.0);

        // Invalidation must force a refresh without breaking consistency.
        state.auth_db.add_dataset_graph("d1", "http://cache.ex.org/g2").unwrap();
        state.auth_db.invalidate_accessible_graphs_cache();
        let refreshed = state.auth_db.get_accessible_graph_iris_cached(None).unwrap();
        assert!(refreshed.0.contains("http://cache.ex.org/g2"), "Post-invalidation read must see new graph");
    }
    ...
    async fn health_check() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let json = body_json(resp.into_body()).await;
        assert_eq!(json["status"], "ok", "Health check must return status=ok: {json}");
    }
}
...
mod visibility_leaks {
    ...
    async fn member_can_get_organisation() {
        let state = test_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_user("u_alice", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.add_org_member("u_alice", "o1", Role::Member).unwrap();
    ...
    async fn void_excludes_private_datasets_for_anon() {
        let state = test_state();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset(
            "ds_pub", "Public DS", None, OwnerType::Organisation, "o1", Visibility::Public, None,
        ).unwrap();
        state.auth_db.create_dataset(
            "ds_priv", "Secret DS", None, OwnerType::Organisation, "o1", Visibility::Private, None,
        ).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("/dataset/ds_pub"),
            "Public dataset must be in anon VoID: {body}");
        assert!(!body.contains("/dataset/ds_priv"),
            "Private dataset must NOT be in anon VoID: {body}");
        assert!(!body.contains("Secret DS"),
    ...
    async fn catalog_excludes_private_data_models_for_anon() {
        ...
        dmr::insert_data_model(
            &state.store, base, "pub-model",
            "Public Model", "http://ex.org/pub#",
            None, true, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
        dmr::update_latest_published(&state.store, base, "pub-model", "1.0").unwrap();

        dmr::insert_data_model(
            &state.store, base, "priv-model",
            "Private Model", "http://ex.org/priv#",
            None, false, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("/data-model/pub-model"),
            "Public data-model must be in anon catalog: {body}");
        assert!(!body.contains("/data-model/priv-model"),
    ...
    async fn catalog_excludes_private_vocabularies_for_anon() {
        ...
        vr::insert_vocabulary(
            &state.store, base, "pub-voc",
            "Public Voc", "http://ex.org/vpub#",
            None, true, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
        ...
        vr::insert_vocabulary(
            &state.store, base, "priv-voc",
            "Private Voc", "http://ex.org/vpriv#",
            None, false, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("/vocabulary/pub-voc"),
            "Public vocabulary must be in anon catalog: {body}");
        assert!(!body.contains("/vocabulary/priv-voc"),
            "Private vocabulary must NOT be in anon catalog: {body}");
        assert!(!body.contains("Private Voc"),
    ...
    async fn dereference_hides_triples_in_private_graphs() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset(
            "ds_pub", "Public", None, OwnerType::Organisation, "o1", Visibility::Public, None,
        ).unwrap();
        state.auth_db.create_dataset(
            "ds_priv", "Private", None, OwnerType::Organisation, "o1", Visibility::Private, None,
        ).unwrap();
        ...
        let body = body_text(resp.into_body()).await;
        assert!(body.contains("PUBLIC_OBJECT"),
            "Public triple must be returned: {body}");
        assert!(!body.contains("SECRET_OBJECT"),
    ...
    async fn non_owner_publisher_cannot_patch_data_model() {
        ...
        state.auth_db.create_user("u_alice", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.update_user_can_publish("u_alice", true).unwrap();
        // bob is also a publisher but unrelated
        state.auth_db.create_user("u_bob", "bob", "b@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.update_user_can_publish("u_bob", true).unwrap();
        let bob_token = mint_token("u_bob", "bob", "user");

        dmr::insert_data_model(
            &state.store, base, "alice-model",
            "Alice Model", "http://ex.org/alice#",
            None, false, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
    ...
    async fn owner_can_patch_their_data_model() {
        ...
        assert!(resp.status().is_success(),
            "Owner must be able to PATCH their own data-model: {}", resp.status());
    }
    ...
    async fn non_owner_publisher_cannot_patch_data_model_version_data() {
        ...
        dmr::insert_version(&state.store, base, &DataModelVersion {
    ...
    async fn create_draft_records_commit() {
        ...
        dmr::insert_data_model(&state.store, &base, "m1", "M1", "http://ex.org/m1#",
            None, false, Some("user"), Some("u_alice"), None, "2026-01-01T00:00:00Z").unwrap();
        dmr::insert_version(&state.store, &base, &DataModelVersion {
            data_model_id: "m1".into(), version: "1.0.0".into(), status: VersionStatus::Published,
            graph_iri: format!("{base}/data-model/m1/version/1.0.0"), sub_graphs: vec![],
            created_at: "2026-01-01T00:00:00Z".into(), created_by: None, derived_from: None,
            notes: None, branch: None, sub_graph_status: vec![],
        }).unwrap();

        let draft = serde_json::json!({ "target_version": "2.0.0" });
        let resp = test_app(state.clone()).oneshot(Request::builder()
            .method(Method::POST)
            .uri("/api/data-models/m1/versions/1.0.0/draft")
            .header(header::CONTENT_TYPE, "application/json")
            .header(header::AUTHORIZATION, format!("Bearer {tok}"))
            .body(Body::from(serde_json::to_string(&draft).unwrap())).unwrap()
        ).await.unwrap();
        assert!(resp.status().is_success(), "create draft failed: {}", resp.status());

        let resp = test_app(state).oneshot(Request::builder()
            .method(Method::GET)
            .uri("/api/data-models/m1/commits")
            .header(header::AUTHORIZATION, format!("Bearer {tok}"))
            .body(Body::empty()).unwrap()
        ).await.unwrap();
        assert!(resp.status().is_success(), "list commits failed: {}", resp.status());
        let commits = body_json(resp.into_body()).await;
        let arr = commits.as_array().expect("commits array");
        assert!(
            arr.iter().any(|c|
                c["kind"].as_str() == Some("data-model")
                && c["version"].as_str() == Some("2.0.0")
                && c["message"].as_str().unwrap_or("").contains("Created draft 2.0.0")),
    ...
    async fn merge_apply_records_commit() {
        ...
        dmr::insert_data_model(&state.store, &base, "m2", "M2", "http://ex.org/m2#",
            None, false, Some("user"), Some("u_alice"), None, "2026-01-01T00:00:00Z").unwrap();
        for (ver, st, branch) in [
            ...
            dmr::insert_version(&state.store, &base, &DataModelVersion {
                data_model_id: "m2".into(), version: ver.into(), status: st,
                graph_iri: format!("{base}/data-model/m2/version/{ver}"), sub_graphs: vec![],
                created_at: "2026-01-01T00:00:00Z".into(), created_by: None, derived_from: None,
                notes: None, branch, sub_graph_status: vec![],
            }).unwrap();
        }
        ...
        assert!(resp.status().is_success(), "merge failed: {}", resp.status());
        ...
        assert!(merge_commit.is_some(), "expected a merge commit, got: {commits}");
        let mc = merge_commit.unwrap();
        assert_eq!(mc["message"].as_str(), Some("Bring feature work into the trunk"),
            "merge must honor the custom commit message: {mc}");
        assert!(mc["added"].as_u64().unwrap_or(0) >= 1,
    ...
    async fn rebase_version_records_commit() {
        ...
        dmr::insert_data_model(&state.store, &base, "m3", "M3", "http://ex.org/m3#",
            None, false, Some("user"), Some("u_alice"), None, "2026-01-01T00:00:00Z").unwrap();
        for (ver, st, branch, derived) in [
            ("1.0.0", VersionStatus::Published, None, None),
            ("feature", VersionStatus::Draft, Some("feature".to_string()), Some("1.0.0".to_string())),
        ] {
            dmr::insert_version(&state.store, &base, &DataModelVersion {
                data_model_id: "m3".into(), version: ver.into(), status: st,
                graph_iri: format!("{base}/data-model/m3/version/{ver}"), sub_graphs: vec![],
                created_at: "2026-01-01T00:00:00Z".into(), created_by: None, derived_from: derived,
                notes: None, branch, sub_graph_status: vec![],
            }).unwrap();
        }
        ...
        assert!(resp.status().is_success(), "rebase failed: {}", resp.status());
        ...
        assert!(
            arr.iter().any(|c|
                ...
                && c["message"].as_str().unwrap_or("").contains("Rebased feature onto 1.0.0")),
    ...
    async fn create_draft_records_commit_vocabulary() {
        use open_triplestore::vocabularies::registry as vr;
        use open_triplestore::vocabularies::models::{VocabularyVersion, VersionStatus};
        let state = test_state();
        let base = state.base_url.to_string();
        state.auth_db.create_user("u_alice", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.update_user_can_publish("u_alice", true).unwrap();
        ...
        vr::insert_vocabulary(&state.store, &base, "v1", "V1", "http://ex.org/v1#",
            None, false, Some("user"), Some("u_alice"), None, "2026-01-01T00:00:00Z").unwrap();
        vr::insert_version(&state.store, &base, &VocabularyVersion {
            vocabulary_id: "v1".into(), version: "1.0.0".into(), status: VersionStatus::Published,
            graph_iri: format!("{base}/vocabulary/v1/version/1.0.0"), sub_graphs: vec![],
            created_at: "2026-01-01T00:00:00Z".into(), created_by: None, derived_from: None,
            notes: None, branch: None, sub_graph_status: vec![],
        }).unwrap();
        ...
        assert!(resp.status().is_success(), "vocab create draft failed: {}", resp.status());
    ...
    async fn non_owner_publisher_cannot_patch_vocabulary() {
        ...
        vr::insert_vocabulary(
            &state.store, base, "alice-voc",
            "Alice Voc", "http://ex.org/av#",
            None, false, Some("user"), Some("u_alice"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
    ...
    async fn org_member_cannot_patch_org_owned_data_model() {
        ...
        let base = state.base_url.as_str();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_user("u_member", "member", "m@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.update_user_can_publish("u_member", true).unwrap();
        state.auth_db.add_org_member("u_member", "o1", Role::Member).unwrap();
        let member_token = mint_token("u_member", "member", "user");

        dmr::insert_data_model(
            &state.store, base, "org-model",
            "Org Model", "http://ex.org/o#",
            None, false, Some("organisation"), Some("o1"),
            None, "2026-01-01T00:00:00Z",
        ).unwrap();
    ...
    async fn org_admin_can_patch_org_owned_data_model() {
        ...
        let base = state.base_url.as_str();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_user("u_admin", "orgadm", "oa@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.update_user_can_publish("u_admin", true).unwrap();
        state.auth_db.add_org_member("u_admin", "o1", Role::Admin).unwrap();
        ...
        let resp = test_app(state)
            ...
            .unwrap();
        assert!(resp.status().is_success(),
            "Org admin must be able to mutate org-owned data-model: {}", resp.status());
    }
    ...
    async fn anon_cannot_get_private_dataset_by_id() {
        ...
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.create_dataset(
            "ds_priv", "Secret", None, OwnerType::Organisation, "o1", Visibility::Private, None,
        ).unwrap();
    ...
    async fn anon_list_datasets_excludes_private() {
        ...
        assert!(!body.contains("ds_priv"), "Private ds must NOT appear: {body}");
        assert!(!body.contains("Secret"),  "Private ds name must NOT leak: {body}");
    }
    ...
    async fn anon_cannot_delete_dataset() {
        ...
        assert!(state.auth_db.get_dataset("ds_priv").unwrap().is_some(),
    ...
    async fn anon_cannot_delete_public_dataset() {
        ...
        assert!(state.auth_db.get_dataset("ds_pub").unwrap().is_some(),
    ...
    async fn non_owner_cannot_delete_dataset_via_api() {
        ...
        state.auth_db.create_user("u_alice", "alice", "a@t.com", "h", SystemRole::User).unwrap();
        state.auth_db.create_organisation("o1", "Acme", "acme", None, None).unwrap();
        state.auth_db.add_org_member("u_alice", "o1", Role::Admin).unwrap();
        state.auth_db.create_dataset(
            "ds1", "DS", None, OwnerType::Organisation, "o1", Visibility::Public, None,
        ).unwrap();
        // bob is unrelated
        state.auth_db.create_user("u_bob", "bob", "b@t.com", "h", SystemRole::User).unwrap();
        let bob_token = mint_token("u_bob", "bob", "user");
        ...
        assert!(state.auth_db.get_dataset("ds1").unwrap().is_some(),
    ...
    async fn bulk_import_anon_rejected() {
        ...
        let resp = test_app(state)
            ...
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "Anon must not bulk-import");
    }
    ...
    async fn bulk_import_non_owner_into_dataset_rejected() {
        ...
        state.auth_db.add_org_member("u_alice", "o1", Role::Admin).unwrap();
        state.auth_db.create_dataset(
            "ds_alice", "Alice DS", None, OwnerType::Organisation, "o1", Visibility::Private, None,
        ).unwrap();
    ...
    async fn bulk_import_owner_into_own_dataset_allowed() {
        ...
        let body = multipart_body(boundary, &[
            ...
            ("default_target_graph", "text/plain", None, b"http://ex.org/g/import"),
            ("file", "text/turtle", Some("a.ttl"),
             b"<http://ex.org/s> <http://ex.org/p> <http://ex.org/o> ."),
        ]);
        let resp = test_app(state)
            ...
            .unwrap();
        assert!(resp.status().is_success(),
            "Admin must be able to bulk-import: {}", resp.status());
    }
    ...
    async fn asset_upload_over_ceiling_returns_413() {
        ...
        state.auth_db.create_dataset(
            "ds_a", "Assets", None, OwnerType::User, "adm", Visibility::Private, None,
        ).unwrap();
        ...
        assert_eq!(resp.status(), StatusCode::PAYLOAD_TOO_LARGE,
            "asset over the ceiling must return 413, got {}", resp.status());
    }
