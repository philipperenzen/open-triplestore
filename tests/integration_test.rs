...
fn select_results(store: &open_triplestore::store::TripleStore, query: &str) -> Vec<Vec<String>> {
...
fn test_describe() {
    ...
    let results = store.query("DESCRIBE <http://example.org/alice>").unwrap();
...
fn test_aggregate_count() {
    ...
    let results = select_results(&store, "SELECT (COUNT(*) as ?count) WHERE { ?s ?p ?o }");
...
fn test_string_functions() {
    ...
    let results = select_results(
        &store,
        "SELECT (CONCAT(\"hello\", \" \", \"world\") AS ?str) WHERE {}",
    );
    ...
    let handles: Vec<_> = (0..10)
        .map(|_| {
            ...
            std::thread::spawn(move || {
                let results = select_results(&store, "SELECT (COUNT(*) as ?c) WHERE { ?s ?p ?o }");
                assert!(results[0][0].contains("3"));
            })
        })
        .collect();
