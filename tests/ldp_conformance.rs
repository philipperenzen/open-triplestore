...
fn get_container_type_basic() {
    ...
    assert_eq!(container::get_container_type(&store, "http://ex.org/bc/"), ContainerType::Basic);
}
...
fn count_members_empty() {
    ...
    assert_eq!(container::count_members(&store, "http://ex.org/empty/").unwrap(), 0);
}
...
fn list_members_pagination() {
    ...
    for i in 1..=5 {
        container::add_member(&store, "http://ex.org/pg/", &format!("http://ex.org/pg/i{i}")).unwrap();
    }
    assert_eq!(container::list_members(&store, "http://ex.org/pg/", 0, 2).unwrap().len(), 2);
    assert_eq!(container::list_members(&store, "http://ex.org/pg/", 2, 2).unwrap().len(), 2);
    assert_eq!(container::list_members(&store, "http://ex.org/pg/", 4, 2).unwrap().len(), 1);
}
...
fn get_container_type_direct() {
    ...
    container::ensure_direct_container(
        &store, "http://ex.org/dc/",
        "http://ex.org/res", container::LDP_MEMBER, None,
    ).unwrap();
    assert_eq!(container::get_container_type(&store, "http://ex.org/dc/"), ContainerType::Direct);
}
...
fn get_membership_info_direct() {
    ...
    container::ensure_direct_container(
        &store, "http://ex.org/dc/",
        "http://ex.org/res", "http://ex.org/hasItem", None,
    ).unwrap();
    let info = container::get_membership_info(&store, "http://ex.org/dc/").unwrap().unwrap();
...
fn get_membership_info_basic_returns_none() {
    let store = fresh();
    container::ensure_container(&store, "http://ex.org/bc/").unwrap();
    assert!(container::get_membership_info(&store, "http://ex.org/bc/").unwrap().is_none());
}
...
fn add_remove_direct_membership_triple() {
    ...
    container::add_direct_membership_triple(
        &store, "http://ex.org/res", "http://ex.org/has", "http://ex.org/item1",
    ).unwrap();
    assert!(ask(&store, "ASK { <http://ex.org/res> <http://ex.org/has> <http://ex.org/item1> }"));
...
fn list_membership_triples_direct() {
    let store = fresh();
    container::ensure_direct_container(
        &store, "http://ex.org/dc2/",
        "http://ex.org/mr", "http://ex.org/hasMember", None,
    ).unwrap();
    container::add_direct_membership_triple(
        &store, "http://ex.org/mr", "http://ex.org/hasMember", "http://ex.org/a",
    ).unwrap();
    container::add_direct_membership_triple(
        &store, "http://ex.org/mr", "http://ex.org/hasMember", "http://ex.org/b",
    ).unwrap();
    let triples = container::list_membership_triples(&store, "http://ex.org/dc2/").unwrap();
    assert_eq!(triples.len(), 2);
    assert!(triples.iter().all(|(s, p, _)| s == "http://ex.org/mr" && p == "http://ex.org/hasMember"));
}
...
fn get_container_type_indirect() {
    ...
    container::ensure_indirect_container(
        &store, "http://ex.org/ic/",
        "http://ex.org/res", container::LDP_MEMBER, "http://ex.org/via",
    ).unwrap();
    assert_eq!(container::get_container_type(&store, "http://ex.org/ic/"), ContainerType::Indirect);
}
...
fn indirect_container_inserted_content_relation() {
    let store = fresh();
    container::ensure_indirect_container(
        &store, "http://ex.org/ic2/",
        "http://ex.org/col", "http://ex.org/hasBook", "http://ex.org/bookIRI",
    ).unwrap();
    let info = container::get_membership_info(&store, "http://ex.org/ic2/").unwrap().unwrap();
    assert_eq!(info.inserted_content_relation.as_deref(), Some("http://ex.org/bookIRI"));
}
...
fn get_container_type_non_rdf() {
    ...
    assert_eq!(container::get_container_type(&store, "http://ex.org/img"), ContainerType::NonRdfSource);
}
...
fn binary_content_type_preserved() {
    ...
    let (ct, _) = container::get_binary_resource(&store, "http://ex.org/vid").unwrap().unwrap();
...
fn get_binary_resource_missing_returns_none() {
    let store = fresh();
    assert!(container::get_binary_resource(&store, "http://ex.org/missing").unwrap().is_none());
}
...
fn binary_typed_as_non_rdf_source() {
    let store = fresh();
    container::store_binary_resource(&store, "http://ex.org/bin", "application/pdf", b"%PDF").unwrap();
    assert!(ask(&store, &format!(
        "ASK {{ <http://ex.org/bin> <{}> <{}> }}",
        container::RDF_TYPE, container::LDP_NON_RDF_SOURCE
    )));
}
...
mod http_tests {
    ...
    async fn options_includes_patch_in_allow() {
        ...
        let resp = router
            .oneshot(Request::builder().method("OPTIONS").uri("/ldp/col/").body(Body::empty()).unwrap())
            .await.unwrap();
        let allow = resp.headers().get("allow").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(allow.contains("PATCH"), "Allow should include PATCH, got: {allow}");
    }
    ...
    async fn options_has_accept_patch_header() {
        ...
        let resp = router
            .oneshot(Request::builder().method("OPTIONS").uri("/ldp/col2/").body(Body::empty()).unwrap())
            .await.unwrap();
        let ap = resp.headers().get("accept-patch").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(ap.contains("application/sparql-update"), "Accept-Patch should include sparql-update, got: {ap}");
    }
    ...
    async fn get_basic_container_link_headers() {
        ...
        let resp = router
            .oneshot(Request::builder().uri("/ldp/bc/").body(Body::empty()).unwrap())
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let link = resp.headers().get("link").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(link.contains("BasicContainer"), "Link should mention BasicContainer, got: {link}");
    }
    ...
    async fn get_direct_container_link_headers() {
        let (router, store) = make_router();
        container::ensure_direct_container(
            &store, "http://localhost/ldp/dc/",
            "http://localhost/ldp/dc/res", container::LDP_MEMBER, None,
        ).unwrap();

        let resp = router
            .oneshot(Request::builder().uri("/ldp/dc/").body(Body::empty()).unwrap())
            .await.unwrap();
        let link = resp.headers().get("link").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(link.contains("DirectContainer"), "Link should mention DirectContainer, got: {link}");
    }
    ...
    async fn get_constrained_by_link_header() {
        ...
        let resp = router
            .oneshot(Request::builder().uri("/ldp/cb/").body(Body::empty()).unwrap())
            .await.unwrap();
        let link = resp.headers().get("link").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(link.contains("constrainedBy"), "Link should include constrainedBy, got: {link}");
    }
    ...
    async fn get_prefer_representation_includes_contains() {
        ...
        container::add_member(&store, "http://localhost/ldp/pref/", "http://localhost/ldp/pref/item1").unwrap();

        let resp = router
            .oneshot(Request::builder().uri("/ldp/pref/").header("prefer", "return=representation").body(Body::empty()).unwrap())
            .await.unwrap();
        let body = body_string(resp.into_body()).await;
        assert!(body.contains("item1") || body.contains("ldp#contains"),
    ...
    async fn get_prefer_minimal_omits_contains() {
        ...
        container::add_member(&store, "http://localhost/ldp/min/", "http://localhost/ldp/min/item1").unwrap();

        let resp = router
            .oneshot(Request::builder().uri("/ldp/min/").header("prefer", "return=minimal").body(Body::empty()).unwrap())
            .await.unwrap();
        let body = body_string(resp.into_body()).await;
        assert!(!body.contains("ldp/min/item1"),
    ...
    async fn get_preference_applied_header_present() {
        ...
        let resp = router
            .oneshot(Request::builder().uri("/ldp/pa/").header("prefer", "return=minimal").body(Body::empty()).unwrap())
            .await.unwrap();
        assert!(resp.headers().contains_key("preference-applied"),
    ...
    async fn get_non_rdf_source_binary() {
        ...
        container::store_binary_resource(
            &store, "http://localhost/ldp/img.png", "image/png", data,
        ).unwrap();

        let resp = router
            .oneshot(Request::builder().uri("/ldp/img.png").body(Body::empty()).unwrap())
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn post_turtle_creates_member() {
        ...
        let resp = router
            .oneshot(
                Request::builder()
                    .method("POST").uri("/ldp/ttl/")
                    .header("content-type", "text/turtle")
                    .header("slug", "new-item")
                    .body(Body::from("<http://localhost/ldp/ttl/new-item> <http://example.org/p> \"v\" .\n"))
                    .unwrap()
            )
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let location = resp.headers().get("location").and_then(|v| v.to_str().ok()).unwrap_or("");
        assert!(location.contains("new-item"), "Location should reference slug, got: {location}");
    }
    ...
    async fn patch_applies_sparql_update() {
        ...
        assert!(ask(&store, "ASK { <http://localhost/ldp/patch-me> <http://example.org/val> \"new\" }"));
    }
    ...
    async fn patch_wrong_content_type_415() {
        ...
        let resp = router
            .oneshot(
                Request::builder()
                    .method("PATCH").uri("/ldp/r1")
                    .header("content-type", "text/turtle")
                    .body(Body::from("<http://localhost/ldp/r1> <http://example.org/p> \"v2\" ."))
                    .unwrap()
            )
            .await.unwrap();
    ...
    async fn patch_missing_resource_404() {
        ...
        let resp = router
            .oneshot(
                Request::builder()
                    .method("PATCH").uri("/ldp/does-not-exist")
                    .header("content-type", "application/sparql-update")
                    .body(Body::from("INSERT DATA { <http://ex.org/x> <http://ex.org/p> <http://ex.org/y> }"))
                    .unwrap()
            )
            .await.unwrap();
    ...
    async fn patch_if_match_mismatch_412() {
        ...
        let resp = router
            .oneshot(
                Request::builder()
                    .method("PATCH").uri("/ldp/etag-test")
                    .header("content-type", "application/sparql-update")
    ...
    async fn put_if_match_wrong_etag_412() {
        ...
        let resp = router
            .oneshot(
                Request::builder()
                    .method("PUT").uri("/ldp/put-etag")
                    .header("content-type", "text/turtle")
                    .header("if-match", "\"wrong\"")
                    .body(Body::from("<http://localhost/ldp/put-etag> <http://example.org/p> \"v2\" .\n"))
                    .unwrap()
            )
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::PRECONDITION_FAILED);
    ...
    async fn put_if_match_correct_etag_204() {
        ...
        store.load_str(nt, oxigraph::io::RdfFormat::NTriples, None).unwrap();
        let body_bytes = container::describe_resource(&store, "http://localhost/ldp/put-ok").unwrap();
        let etag = container::compute_etag(&body_bytes);

        let resp = router
            .oneshot(
                Request::builder()
                    .method("PUT").uri("/ldp/put-ok")
                    .header("content-type", "text/turtle")
                    .header("if-match", &etag)
                    .body(Body::from("<http://localhost/ldp/put-ok> <http://example.org/p> \"v2\" .\n"))
                    .unwrap()
            )
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::NO_CONTENT);
    ...
    async fn head_returns_etag() {
        ...
        let resp = router
            .oneshot(Request::builder().method("HEAD").uri("/ldp/head-me").body(Body::empty()).unwrap())
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        assert!(resp.headers().contains_key("etag"), "HEAD response should have ETag");
    }
    ...
    async fn delete_direct_member_removes_membership_triple() {
        ...
        container::add_member(&store, "http://localhost/ldp/del-dc/", "http://localhost/ldp/del-dc/item").unwrap();
        container::add_direct_membership_triple(
            &store,
            "http://localhost/ldp/del-dc/",
            container::LDP_MEMBER,
        ...
        let resp = router
            .oneshot(Request::builder().method("DELETE").uri("/ldp/del-dc/item").body(Body::empty()).unwrap())
            .await.unwrap();
        assert_eq!(resp.status(), StatusCode::NO_CONTENT);
