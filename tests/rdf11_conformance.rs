...
fn load_ok(s: &open_triplestore::store::TripleStore, data: &str, fmt: RdfFormat) {
    s.load_str(data, fmt, None)
        .expect("Expected valid RDF to parse");
}
...
fn rdf_turtle_neg_bad_prefix() {
    ...
    load_err("undefined:s undefined:p undefined:o .", RdfFormat::Turtle);
}
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
  ...
  <rdf:Description rdf:about="http://example.org/s">
    ...
    assert!(ask(
        &s,
        "ASK { <http://example.org/s> <http://example.org/age> 42 }"
    ));
}
...
fn rdf11_blank_node_scope() {
    ...
    match s.query(q).unwrap() {
        QueryResults::Solutions(sols) => {
            ...
            assert!(
                c.contains("2"),
                "Expected 2 distinct blank nodes, got: {}",
                c
            );
        }
        _ => panic!(),
...
fn rdf11_iri_equality() {
    ...
    assert!(ask(
        &s,
        "ASK { <http://example.org/a> <http://example.org/p> <http://example.org/o> }"
    ));
    assert!(!ask(
        &s,
        "ASK { <HTTP://EXAMPLE.ORG/a> <http://example.org/p> <http://example.org/o> }"
...
fn rdf11_typed_literal_values() {
    ...
    assert!(ask(
        &s,
        "ASK { ?s <http://example.org/p> ?v . FILTER(?v = 1) }"
    ));
}
...
fn rdf11_named_graph_put_get() {
    ...
    let g1_data = s
        .dump(RdfFormat::NTriples, Some("http://example.org/g1"))
        .unwrap();
...
fn rdf11_dataset_default_and_named() {
    ...
    let q_default = match s
        .query("SELECT ?v WHERE { <http://example.org/s> <http://example.org/p> ?v }")
        .unwrap()
    {
