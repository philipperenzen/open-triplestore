...
async fn seed_creates_public_org_with_all_category_datasets() {
    ...
    for spec in demo_datasets() {
        ...
        for g in spec.graphs {
            ...
            assert!(registered.contains(&iri), "graph <{iri}> must be registered");
...
async fn every_saved_query_runs_through_the_api() {
    ...
    for (ds_id, ds_name) in org_datasets(&state, &org_id) {
        ...
        let resp = get(&state, &token, &format!("/api/datasets/{ds_id}/api-services")).await;
    ...
    assert!(total_services >= 18, "expected the full set of demo services, ran {total_services}");
}
...
async fn representative_services_return_expected_data() {
    ...
    assert!(txt.contains("Amsterdam"), "bbox query should include Amsterdam: {txt}");
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let txt = body_text(resp.into_body()).await;
    assert!(txt.contains("Animal"), "subclass closure should reach Animal: {txt}");
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let txt = body_text(resp.into_body()).await;
    assert!(txt.contains("GeoSPARQL"), "capabilities should list GeoSPARQL: {txt}");
}
