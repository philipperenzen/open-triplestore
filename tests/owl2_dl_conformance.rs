...
use open_triplestore::reasoning::owl2_dl::{
    ExternalReasoner, ExternalReasonerBridge, NativeTableauStub, Owl2DLReasoner,
    DL_EXACT_CARDINALITY, DL_EXACT_QUAL_CARDINALITY, DL_MIN_CARDINALITY, DL_MIN_QUAL_CARDINALITY,
};
...
fn dl_bridge_stub_now_succeeds() {
    ...
    assert!(
        result.is_ok(),
        "bridge with stub should succeed after native DL rules run"
    );
}
...
fn dl_bridge_with_mock_external_reasoner() {
    ...
    impl ExternalReasoner for MockReasoner {
        ...
        fn get_inferences(&self, _: &str) -> Result<String, ReasoningError> {
            Ok(
                "<http://example.org/mock> <http://example.org/prop> <http://example.org/val> .\n"
                    .to_string(),
            )
        }
    }
