...
fn format_cases() -> Vec<FormatCase> {
    vec![
        FormatCase { label: "Turtle",    filename: "data.ttl",    mime: "text/turtle",              body: TRIPLES_NT, graph: "http://ex.org/std/ttl",    embeds_graph: false },
        FormatCase { label: "N-Triples", filename: "data.nt",     mime: "application/n-triples",    body: TRIPLES_NT, graph: "http://ex.org/std/nt",     embeds_graph: false },
        FormatCase { label: "RDF/XML",   filename: "data.rdf",    mime: "application/rdf+xml",      body: RDFXML,     graph: "http://ex.org/std/rdf",    embeds_graph: false },
        FormatCase { label: "OWL/XML",   filename: "data.owl",    mime: "application/rdf+xml",      body: RDFXML,     graph: "http://ex.org/std/owl",    embeds_graph: false },
        FormatCase { label: "JSON-LD",   filename: "data.jsonld", mime: "application/ld+json",      body: JSONLD,     graph: "http://ex.org/std/jsonld", embeds_graph: false },
        FormatCase { label: "N-Quads",   filename: "data.nq",     mime: "application/n-quads",      body: NQUADS,     graph: "http://ex.org/std/nq",     embeds_graph: true },
        FormatCase { label: "TriG",      filename: "data.trig",   mime: "application/trig",         body: TRIG,       graph: "http://ex.org/std/trig",   embeds_graph: true },
    ]
}
...
fn shared_parser_handles_all_formats() {
    for c in format_cases() {
        ...
        assert_eq!(quads.len(), 3, "{}: expected 3 triples, got {}", c.label, quads.len());
    }
}
...
async fn bulk_import_all_formats_per_graph() {
    ...
    for c in format_cases() {
        ...
        let parts: Vec<(&str, &str, Option<&str>, &[u8])> = if c.embeds_graph {
            ...
            vec![
                ("default_target_graph", "text/plain", None, c.graph.as_bytes()),
        ...
        assert_eq!(json["success"], true, "{}: import not successful: {json}", c.label);

        let count = state.store.count_graph(Some(c.graph)).unwrap();
        assert_eq!(count, 3, "{}: graph <{}> should hold 3 triples", c.label, c.graph);
    }
}
...
async fn graph_store_put_all_triple_formats() {
    ...
    for c in format_cases().into_iter().filter(|c| !c.embeds_graph) {
        ...
        assert_eq!(count, 3, "{}: GSP graph <{}> should hold 3 triples", c.label, gsp_graph);
    }
}
...
async fn sparql_query_forms() {
    ...
    assert!(txt.contains("http://ex.org/std/b"), "SELECT must return the object: {txt}");
    ...
    .await;
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(body_json(resp.into_body()).await["boolean"], true, "ASK must be true");
...
async fn sparql_update_operations() {
    ...
    assert_eq!(state.store.count_graph(Some(g_copy)).unwrap(), 1, "after COPY");
...
async fn graph_store_protocol_crud() {
    ...
    assert!(body_text(resp.into_body()).await.contains("http://ex.org/std/x"));
...
async fn service_description_advertises_capabilities() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let txt = body_text(resp.into_body()).await;
    assert!(txt.contains("sd:Service"), "must advertise an sd:Service: {txt}");
    assert!(txt.contains("SPARQL11Query"), "must advertise SPARQL 1.1 Query");
    assert!(txt.contains("SPARQL11Update"), "must advertise SPARQL 1.1 Update");
}
...
async fn void_dataset_description() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let txt = body_text(resp.into_body()).await;
    assert!(txt.contains("void"), "VoID description must reference the void vocabulary: {txt}");
    assert!(
...
async fn shacl_core_min_count() {
    ...
    assert!(!report.conforms, "a Person with no name must violate minCount 1");
    assert!(report.results_count >= 1, "expected at least one validation result");
...
async fn jwt_required_for_writes() {
    let (state, token) = admin_state();
    let update = "INSERT DATA { <http://ex.org/std/a> <http://ex.org/std/b> <http://ex.org/std/c> }";
    ...
    let resp = app(state.clone())
        ...
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::UNAUTHORIZED, "anonymous UPDATE must be 401");

    // Valid JWT → accepted.
    let resp = sparql_update(&state, &token, update).await;
    assert!(resp.status().is_success(), "JWT-authenticated UPDATE must succeed");
}
...
async fn rdf_star_quoted_triple_roundtrip() {
    ...
    assert!(
        ask(
            &state.store,
            &format!(
                "ASK {{ GRAPH <{g}> {{ << ?s ?p ?o >> <http://ex.org/note> ?v }} }}"
            )
        ),
...
async fn rdfs_subclass_entailment() {
    ...
    let resp = app(state.clone())
        ...
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "entailment=rdfs must be accepted");
}
...
async fn owl2_ql_query_rewriting() {
    ...
    assert!(rows >= 1, "QL rewriting must return the Manager instance as an Employee");
}
...
async fn oauth_oidc_providers_endpoint() {
    ...
    let resp = app(state)
        ...
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "OIDC providers list must be reachable");
    assert!(
...
async fn swrl_execute_endpoint_wired() {
    ...
    let resp = app(state)
        ...
        .await
        .unwrap();
    assert_ne!(resp.status(), StatusCode::NOT_FOUND, "SWRL route must be mounted");
    assert!(
