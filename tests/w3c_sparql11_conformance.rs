...
fn w3c_agg_count_all() {
    ...
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :p 1 . :b :p 2 . :c :p 3 .");
...
fn w3c_agg_count_distinct() {
    ...
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :p 1 . :a :p 2 . :b :p 3 .");
    let r = select(&s, "SELECT (COUNT(DISTINCT ?s) AS ?count) WHERE { ?s ?p ?o }");
...
fn w3c_agg_sum() {
    // agg02: SUM
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :val 1 . :b :val 2 . :c :val 3 .");
    let r = select(&s, "SELECT (SUM(?v) AS ?sum) WHERE { ?s <http://example/val> ?v }");
...
fn w3c_agg_min_max() {
    // agg03 / agg04
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :val 10 . :b :val 5 . :c :val 20 .");
    let r = select(
        &s,
...
fn w3c_agg_avg() {
    ...
    let r = select(&s, "SELECT (AVG(?v) AS ?avg) WHERE { ?s <http://example/val> ?v }");
...
fn w3c_agg_sample() {
    ...
    let r = select(&s, "SELECT (SAMPLE(?v) AS ?s) WHERE { ?x <http://example/val> ?v }");
...
fn w3c_agg_group_concat() {
    ...
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :name \"x\" . :b :name \"y\" . :c :name \"z\" .");
    let r = select(
        &s,
    ...
    let joined = &r[0][0];
    assert!(joined.contains("x") || joined.contains("y") || joined.contains("z"),
            "GROUP_CONCAT result: {}", joined);
}
...
fn w3c_bind_coalesce() {
    ...
    assert!(r[1][1].contains("Bob"), "Bob should fall back to name: {}", r[1][1]);
}
...
fn w3c_reduced() {
    ...
    let s = ts();
    load(&s, "@prefix : <http://example/> . :a :p :x . :b :p :x . :c :p :y .");
...
fn w3c_fn_replace() {
    let s = ts();
    let r = select(&s, "SELECT (REPLACE(\"abcd\", \"b\", \"Z\") AS ?v) WHERE {}");
...
fn w3c_graph_variable() {
    ...
    let r = select(&s, "SELECT ?g ?v WHERE { GRAPH ?g { ?s <http://example/p> ?v } } ORDER BY ?g");
    assert_eq!(r.len(), 2);
}
...
fn w3c_path_sequence() {
    ...
    load(
        &s,
        "@prefix : <http://example/> . :a :p :b . :b :q :c .",
    );
    let r = select(
        &s,
...
fn w3c_path_alternative() {
    ...
    load(
        &s,
        "@prefix : <http://example/> . :a :p1 :x . :b :p2 :y .",
    );
    let r = select(
        &s,
...
fn w3c_path_negated_property_set() {
    ...
    load(
        &s,
        "@prefix : <http://example/> . :a :p :b . :a :q :c .",
    );
    let r = select(
        &s,
        "SELECT ?o WHERE { <http://example/a> !<http://example/p> ?o }",
...
fn w3c_update_insert_data() {
    let s = ts();
    s.update("INSERT DATA { <http://example/s> <http://example/p> <http://example/o> }").unwrap();
...
fn w3c_update_delete_data() {
    let s = ts();
    s.update("INSERT DATA { <http://example/s> <http://example/p> <http://example/o> }").unwrap();
    s.update("DELETE DATA { <http://example/s> <http://example/p> <http://example/o> }").unwrap();
...
fn w3c_update_insert_where() {
    ...
    load(&s, "@prefix : <http://example/> . :a :name \"Alice\" .");
    s.update(
        "INSERT { ?s <http://example/label> ?n } WHERE { ?s <http://example/name> ?n }",
    )
    .unwrap();
    assert!(ask(&s, "ASK { <http://example/a> <http://example/label> \"Alice\" }"));
}
...
fn w3c_update_delete_where() {
    ...
    s.update(
        "DELETE WHERE { ?s <http://example/status> \"old\" }",
    )
    .unwrap();
    assert_eq!(s.len().unwrap(), 1);
}
...
fn w3c_update_delete_insert() {
    ...
    .unwrap();
    assert!(ask(&s, "ASK { <http://example/a> <http://example/status> \"retired\" }"));
    assert!(!ask(&s, "ASK { <http://example/a> <http://example/status> \"active\" }"));
}
...
fn w3c_syntax_positive_select() {
    ...
    s.query("SELECT ?s (COUNT(*) AS ?c) WHERE { ?s ?p ?o } GROUP BY ?s").unwrap();
}
