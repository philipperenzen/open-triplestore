...
test('the name keeps its floor in the tightest layout and absorbs the slack', async ({ page }) => {
  ...
  // narrower than that unless the whole name is shorter anyway. Before the fix
  // every one of these was 0.
  const FLOOR = 72;
  for (const row of measured) {
    expect(row.nameWidth, `"${row.text}" is ${row.nameWidth}px`).toBeGreaterThanOrEqual(
      Math.min(row.nameNeeds, FLOOR) - 1,
    );
  }

  // And where a name is too long to fit, it has absorbed every pixel the row
  // had left: the chip takes exactly its own (already 7rem-capped) width and
  // the name takes the rest. That is the reversal of the bug, in which the
  // chip took its full 115px and left the name 0.
  const CHROME = 32; // the database icon plus the two flex gaps
  const clipped = measured.filter((r) => r.nameNeeds > r.nameWidth + 1);
  for (const row of clipped) {
    expect(
      row.nameWidth + row.chipWidth,
      `"${row.text}": name ${row.nameWidth} + chip ${row.chipWidth} does not fill its ${row.titleWidth}px row`,
    ).toBeGreaterThanOrEqual(row.titleWidth - CHROME);
    // The chip never renders wider than it needs, so it cannot be hogging.
    expect(row.chipWidth).toBeLessThanOrEqual(row.chipNeeds + 1);
  }
});
