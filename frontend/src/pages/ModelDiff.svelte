<script>
  ...
  import { getDataModelDiff } from '../lib/api.js';
  ...
  async function load() {
    ...
    diff = null;
    try {
      diff = await getDataModelDiff(id, fromVer, toVer);
    } catch (e) {
      error = e.message;
    }
    loading = false;
...
<div class="space-y-6">
  <div class="flex items-center gap-3">
    <button class="btn btn-ghost btn-sm" on:click={() => navigate(`/models/${id}`)}>
      <ArrowLeft size={16} /> {$i18nT('system.back')}
    </button>
    <h2 class="text-xl font-semibold m-0">{$i18nT('pages.modelDiff.heading')} <code class="text-base font-mono">v{fromVer}</code> → <code class="text-base font-mono">v{toVer}</code></h2>
  </div>

  <div class="flex flex-wrap items-end gap-3">
    <div>
      <label class="label" for="diff-from">{$i18nT('pages.modelDiff.fromVersion')}</label>
      <input id="diff-from" type="text" class="input w-32" bind:value={fromVer} placeholder="1.0.0" />
    </div>
    <div>
      <label class="label" for="diff-to">{$i18nT('pages.modelDiff.toVersion')}</label>
    ...
    <button class="btn btn-primary btn-sm" on:click={load} disabled={!fromVer || !toVer || loading}>
      ...
      {$i18nT('pages.modelDiff.compare')}
    </button>
  </div>

  {#if loading}
    <div class="flex items-center justify-center py-16 text-[var(--ink-400)]">
      <Loader2 size={24} class="animate-spin mr-2" /> {$i18nT('pages.modelDiff.computing')}
    </div>
  {:else if error}
  ...
  {:else if diff}
    <div class="flex gap-4 flex-wrap">
      <div class="px-4 py-2 rounded-xl bg-green-50 border border-green-200">
        ...
        <span class="text-green-600 text-sm ml-1">{$i18nT('pages.modelDiff.added')}</span>
      </div>
      <div class="px-4 py-2 rounded-xl bg-red-50 border border-red-200">
        ...
        <span class="text-red-600 text-sm ml-1">{$i18nT('pages.modelDiff.removed')}</span>
      </div>
      <div class="px-4 py-2 rounded-xl bg-amber-50 border border-amber-200">
        ...
        <span class="text-amber-600 text-sm ml-1">{$i18nT('pages.modelDiff.changed')}</span>
      </div>
    </div>

    {#if diff.summary.added === 0 && diff.summary.removed === 0 && diff.summary.changed === 0}
      <p class="text-[var(--ink-400)] text-sm text-center py-8">{$i18nT('pages.modelDiff.identical')}</p>
    {/if}

    {#if diff.added.length > 0}
      <section>
        <h3 class="text-base font-semibold text-green-700 mb-2 flex items-center gap-1.5">
          <Plus size={16} /> {$i18nT('pages.modelDiff.addedCount', { values: { count: diff.added.length } })}
        </h3>
        <div class="diff-table added">
          <div class="diff-header"><span>{$i18nT('pages.modelDiff.subject')}</span><span>{$i18nT('pages.modelDiff.predicate')}</span><span>{$i18nT('pages.modelDiff.object')}</span></div>
    ...
    {#if diff.removed.length > 0}
      <section>
        <h3 class="text-base font-semibold text-red-700 mb-2 flex items-center gap-1.5">
          <Minus size={16} /> {$i18nT('pages.modelDiff.removedCount', { values: { count: diff.removed.length } })}
        </h3>
    ...
    {#if diff.changed.length > 0}
      <section>
        <h3 class="text-base font-semibold text-amber-700 mb-2">{$i18nT('pages.modelDiff.changedCount', { values: { count: diff.changed.length } })}</h3>
        <div class="space-y-2">
