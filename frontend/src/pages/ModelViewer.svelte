<script>
  ...
  import { getDataModel, listDataModelVersions, getDataModelVersionDataUrl } from '../lib/api.js';
  ...
  onMount(async () => {
    try {
      [model, versions] = await Promise.all([getDataModel(id), listDataModelVersions(id)]);
      selectedVersion = versionId
        ? versions.find(v => v.version === versionId) || versions[0]
        : (versions.find(v => v.status === 'published') || versions[0]);
      if (!selectedVersion) {
        error = $t('pages.modelViewer.noVersions');
      }
    } catch (e) {
      error = e?.message || $t('pages.modelViewer.loadFailed');
    }
    loading = false;
  });
  ...
  function selectVersion(ver) {
    selectedVersion = ver;
    navigate(`/models/${id}/viewer/${ver.version}`);
  }
</script>
...
<div class="space-y-4">
  <div class="flex items-center gap-3 flex-wrap">
    <button class="btn btn-ghost btn-sm" on:click={() => navigate(`/models/${id}`)}>
      <ArrowLeft size={16} /> {$t('pages.modelViewer.backToModel')}
    </button>
    {#if model}
      <h2 class="text-xl font-semibold m-0">{model.title}</h2>
    {/if}
    {#if versions.length > 1}
      <div class="ml-auto flex items-center gap-2">
        <label class="text-sm text-[var(--ink-400)]" for="ver-select">{$t('pages.modelViewer.version')}</label>
        <Select
          id="ver-select"
  ...
  {:else if selectedVersion}
    ...
    <OntologyBrowserPanel
      ...
      rawDataUrl={getDataModelVersionDataUrl(id, selectedVersion.version, 'turtle', 'all')}
    />
  {/if}
</div>
