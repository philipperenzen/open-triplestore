<script>
  ...
  import {
    getVocabulary, listVocabularyVersions, deleteVocabularyVersion, stageVocabularyVersion,
    publishVocabularyVersion, createVocabularyDraft, getVocabularyVersionDataUrl,
    updateVocabulary, updateVocabularyVersionNotes, uploadVocabularyVersion,
    getOrganisation, listOrgMembers, listPublicUsers, getVocabularyCollaborators,
    subgraphActionVocabulary,
  } from '../lib/api.js';
  ...
  function openEdit() {
    editForm = {
      ...
      is_public: vocabulary.is_public,
    };
    editError = '';
    showEdit = true;
  }
  ...
  async function saveNotes(ver) {
    notesSaving = true;
    try {
      await updateVocabularyVersionNotes(id, ver, notesValue);
      editingNotes = '';
      await load();
    } catch (e) {
      alert(e.message);
  ...
  async function load() {
    ...
    error = '';
    try {
      [vocabulary, versions] = await Promise.all([
        getVocabulary(id),
        listVocabularyVersions(id),
      ]);
      loadOwnership(vocabulary);
      getVocabularyCollaborators(id).then(c => { collaborators = c || []; }).catch(() => {});
    } catch (e) {
      error = e.message;
    }
    loading = false;
  ...
  async function loadOwnership(vocab) {
    if (!vocab) return;
    try {
      const users = await listPublicUsers().catch(() => []);
      userMap = Object.fromEntries((users || []).map(u => [String(u.id), u.username || u.email || u.id]));
    } catch {}
    if (vocab.owner_type === 'organisation' && vocab.owner_id) {
      try {
        ownerOrg = await getOrganisation(vocab.owner_id);
        orgMembers = await listOrgMembers(vocab.owner_id).catch(() => []);
      } catch {}
    } else if (vocab.owner_id) {
      ownerUser = userMap[String(vocab.owner_id)] || null;
    }
  }
  ...
  async function doDeleteVersion() {
    ...
    deleteLoading = ver;
    try {
      await deleteVocabularyVersion(id, ver);
      await load();
    } catch (e) {
      alert(e.message);
    }
  ...
  async function handleCreateDraft(fromVer) {
    const targetVer = prompt($t('pages.vocabularyDetail.createDraftPrompt', { values: { version: fromVer } }));
    if (!targetVer) return;
    draftLoading = fromVer;
    try {
      await createVocabularyDraft(id, fromVer, targetVer.trim());
      await load();
    } catch (e) {
      alert(e.message);
    }
  ...
  async function handleStage(ver) {
    stageLoading = ver;
    try {
      await stageVocabularyVersion(id, ver);
      await load();
    } catch (e) {
      alert(e.message);
    }
  ...
  function diffUrl(from, to) {
    return `/vocabularies/${id}/diff?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
  }
  ...
  async function handleSubgraphAction(ver, graph, action) {
    ...
    try {
      await subgraphActionVocabulary(id, ver, action, graph);
      await load();
    } catch (e) {
      alert(e.message);
    }
...
<div class="space-y-6">
  ...
  <div class="flex items-center gap-3">
    <button class="btn btn-ghost btn-sm" on:click={() => navigate('/vocabularies')}>
  ...
  {:else if vocabulary}
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2 flex-wrap">
          <h2 class="text-2xl font-bold m-0">{vocabulary.title}</h2>
          {#if vocabulary.is_public}
            <span class="vis-badge vis-public"><Globe size={11} /> {$t('pages.vocabularyDetail.public')}</span>
          {:else}
            <span class="vis-badge vis-private"><Lock size={11} /> {$t('pages.vocabularyDetail.private')}</span>
          {/if}
        </div>
        <div class="text-sm text-[var(--ink-400)] mt-1">
          <code>{id}</code>
          {#if vocabulary.namespace}
            · <span class="break-all">{vocabulary.namespace}</span>
          {/if}
        </div>
        {#if vocabulary.description}
          <p class="text-sm text-[var(--ink-600)] mt-2 m-0">{vocabulary.description}</p>
        {/if}
      </div>
      <div class="flex items-center gap-2 flex-wrap">
        {#if isPublisher}
          <button class="btn btn-ghost btn-sm" on:click={openEdit} title={$t('pages.vocabularyDetail.editMetadata')}>
            <Pencil size={15} /> {$t('system.edit')}
          </button>
          <button class="btn btn-primary btn-sm" on:click={() => showUpload = true}>
            <Upload size={15} /> {$t('pages.vocabularyDetail.uploadVersion')}
          </button>
        {/if}
      </div>
    </div>
    ...
    {#if showEdit}
      <div class="edit-panel">
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-semibold m-0">{$t('pages.vocabularyDetail.editVocabulary')}</h3>
        ...
        <form on:submit|preventDefault={handleSaveEdit} class="space-y-3">
          <div class="form-row">
            <div>
              <label class="label" for="edit-title">{$t('pages.vocabularyDetail.titleLabel')}</label>
              <input id="edit-title" type="text" class="input" bind:value={editForm.title} required />
            </div>
            <div>
              <label class="label" for="edit-ns">{$t('pages.vocabularyDetail.namespaceUriLabel')}</label>
              <input id="edit-ns" type="text" class="input" bind:value={editForm.namespace} placeholder="https://example.com/vocab/" />
            </div>
          </div>
          <div>
            <label class="label" for="edit-desc">{$t('pages.vocabularyDetail.descriptionLabel')}</label>
            <textarea id="edit-desc" class="input" rows="2" bind:value={editForm.description} placeholder={$t('pages.vocabularyDetail.descriptionPlaceholder')}></textarea>
          </div>
          <label class="flex items-center gap-2 cursor-pointer text-sm">
            <input type="checkbox" bind:checked={editForm.is_public} class="w-4 h-4 accent-[var(--brand-500)]" />
            <span>{$t('pages.vocabularyDetail.publicAccessHint')}</span>
          </label>
          {#if editError}
    ...
    <div class="api-bar">
      ...
      <div class="api-row">
        ...
        <code class="api-url">/api/vocabularies/{id}/versions</code>
        <button class="btn btn-xs btn-ghost copy-btn" title={$t('system.copy')}
          on:click={() => copyApiUrl('versions', `${window.location.origin}/api/vocabularies/${id}/versions`)}>
          {#if copiedApi === 'versions'}<CheckCheck size={12} />{:else}<Copy size={12} />{/if}
        </button>
        <span class="api-note">{$t('pages.vocabularyDetail.apiNoteListVersions')}</span>
      </div>
      <div class="api-row">
        <span class="api-method">GET</span>
        <code class="api-url">/api/vocabularies/{id}/latest/data</code>
        <button class="btn btn-xs btn-ghost copy-btn" title={$t('system.copy')}
          on:click={() => copyApiUrl('latest', `${window.location.origin}/api/vocabularies/${id}/latest/data`)}>
          {#if copiedApi === 'latest'}<CheckCheck size={12} />{:else}<Copy size={12} />{/if}
        </button>
        <span class="api-note">{$t('pages.vocabularyDetail.apiNoteLatestPre')}<code>Accept</code>{$t('pages.vocabularyDetail.apiNoteLatestPost')}</span>
      </div>
      {#if !vocabulary.is_public}
        <div class="api-auth-hint">
          <Lock size={11} /> {$t('pages.vocabularyDetail.bearerTokenRequired')}
        </div>
      {/if}
    </div>

    <!-- Ownership -->
    {#if vocabulary.owner_id}
      <div class="ownership-card">
        <div class="ownership-header">
          {#if vocabulary.owner_type === 'organisation'}
            <Building2 size={15} class="text-[var(--brand-500)]" />
            <span class="ownership-label">{$t('pages.vocabularyDetail.organisation')}</span>
          {:else}
            <User size={15} class="text-[var(--brand-500)]" />
            <span class="ownership-label">{$t('pages.vocabularyDetail.owner')}</span>
          {/if}
        </div>
        <div class="ownership-body">
          {#if vocabulary.owner_type === 'organisation' && ownerOrg}
            <a href="/organisations/{vocabulary.owner_id}" class="ownership-name">{ownerOrg.name}</a>
            ...
            {#if orgMembers.length > 0}
              <div class="members-row">
                ...
                <span class="members-label">{orgMembers.length === 1 ? $t('pages.vocabularyDetail.memberCountOne', { values: { count: orgMembers.length } }) : $t('pages.vocabularyDetail.memberCountOther', { values: { count: orgMembers.length } })}</span>
                <div class="members-list">
                  {#each orgMembers.slice(0, 8) as m}
                    <span class="member-chip" title={m.role}>{m.username || m.user_id}</span>
                  {/each}
                  {#if orgMembers.length > 8}
                    <span class="member-chip member-chip-more">{$t('pages.vocabularyDetail.moreCount', { values: { count: orgMembers.length - 8 } })}</span>
                  {/if}
                </div>
              </div>
            {/if}
          {:else}
            <span class="ownership-name">{ownerUser || vocabulary.owner_id}</span>
          {/if}
        </div>
      </div>
    {/if}
    ...
    {#if collaborators.length > 0}
      <div class="ownership-card">
        <div class="ownership-header">
          ...
          <span class="ownership-label">{$t('pages.vocabularyDetail.collaborators', { values: { count: collaborators.length } })}</span>
        </div>
        <div class="ownership-body">
          <div class="members-list">
    ...
    <!-- Branches -->
    {#if vocabulary}
      <BranchPanel kind="vocabulary" {id} {versions} canWrite={isPublisher} on:created={load} />
    {/if}
    ...
    <!-- Versions -->
    <div>
      <h3 class="text-base font-semibold mb-3">{$t('pages.vocabularyDetail.versions', { values: { count: versions.length } })}</h3>

      {#if versions.length === 0}
        <div class="text-center py-8 text-[var(--ink-400)] text-sm">{$t('pages.vocabularyDetail.noVersions')}</div>
      {:else}
        <div class="space-y-3">
            ...
            <div class="card ver-card" class:ver-browsing={browserVersion === ver.version}>
              <div class="ver-header">
                <button
                  ...
                  title={browserVersion === ver.version ? $t('pages.vocabularyDetail.collapseBrowser') : $t('pages.vocabularyDetail.browseThisVersion')}
                >
                  <span class="font-mono font-bold text-[var(--ink-900)]">v{ver.version}</span>
                  <span class="text-xs px-2 py-0.5 rounded-full font-medium {statusBadge(ver.status)}">{ver.status}</span>
                  {#if ver.derived_from}
                    <span class="text-xs text-[var(--ink-400)]">{$t('pages.vocabularyDetail.fromVersion', { values: { version: ver.derived_from } })}</span>
                  {/if}
                ...
                <div class="ver-actions">
                  <button
                    ...
                    on:click={() => toggleBrowser(ver.version)}
                    title={$t('pages.vocabularyDetail.openInlineBrowser')}
                  >
                    <BookOpen size={13} />
                    {browserVersion === ver.version ? $t('system.close') : $t('pages.vocabularyDetail.browse')}
                  </button>

                  <span class="ver-sep"></span>

                  <a
                    href={getVocabularyVersionDataUrl(id, ver.version, 'trig')}
                    download="{id}-{ver.version}.trig"
                    class="ver-btn"
                    title={$t('pages.vocabularyDetail.downloadTrig')}
                  >
                    <Download size={13} /> {$t('pages.vocabularyDetail.download')}
                  </a>
                  <button
                    class="ver-btn"
                    title={$t('pages.vocabularyDetail.copyDataEndpointUrl')}
                    on:click={() => copyApiUrl(`ver-${ver.version}`, `${window.location.origin}/api/vocabularies/${id}/versions/${ver.version}/data`)}
                  >
                    {#if copiedApi === `ver-${ver.version}`}
                      <CheckCheck size={13} class="text-green-600" /> {$t('system.copied')}
                    {:else}
                      <Copy size={13} /> {$t('pages.vocabularyDetail.copyUrl')}
                    {/if}
                  </button>
                  ...
                  {#if isPublisher && editingNotes !== ver.version}
                    <button class="ver-btn" title={$t('pages.vocabularyDetail.editReleaseNotes')} on:click={() => startEditNotes(ver)}>
                      <Pencil size={13} /> {$t('pages.vocabularyDetail.notes')}
                    </button>
                  {/if}
                  ...
                  {#if versions.indexOf(ver) < versions.length - 1}
                    <a
                      ...
                      class="ver-btn"
                      title={$t('pages.vocabularyDetail.compareWithPrevious')}
                    >
                      <GitCompare size={13} /> {$t('pages.vocabularyDetail.diff')}
                    </a>
                  {/if}
                  ...
                  {#if isPublisher && ver.status === 'published'}
                    ...
                    <button
                      class="ver-btn"
                      title={$t('pages.vocabularyDetail.createDraftFromVersion')}
                      disabled={draftLoading === ver.version}
                      on:click={() => handleCreateDraft(ver.version)}
                    >
                      {#if draftLoading === ver.version}
                        <Loader2 size={13} class="animate-spin" /> {$t('pages.vocabularyDetail.creating')}
                      {:else}
                        <Plus size={13} /> {$t('pages.vocabularyDetail.newDraft')}
                      {/if}
                    </button>
                  {/if}
                  ...
                  {#if isPublisher && ver.status === 'draft'}
                    ...
                    <button
                      ...
                      title={$t('pages.vocabularyDetail.stageForReview')}
                    >
                      {#if stageLoading === ver.version}
                        <Loader2 size={13} class="animate-spin" /> {$t('pages.vocabularyDetail.staging')}
                      {:else}
                        <PlayCircle size={13} /> {$t('pages.vocabularyDetail.stage')}
                      {/if}
                    </button>
                  {/if}
                  ...
                  {#if isPublisher && (ver.status === 'draft' || ver.status === 'staged')}
                    <button class="ver-btn ver-btn-publish" on:click={() => openPublish(ver.version)}>
                      <CheckCircle size={13} /> {$t('pages.vocabularyDetail.publish')}
                    </button>
                  {/if}
                  ...
                  {#if $isAdmin}
                    <button
                      ...
                      title={$t('pages.vocabularyDetail.deleteVersion')}
              ...
              {#if editingNotes === ver.version}
                <div class="mt-2 flex gap-2 items-start">
                  ...
                  <textarea
                    ...
                    placeholder={$t('pages.vocabularyDetail.releaseNotesPlaceholder')}
                    autofocus
                  ></textarea>
              ...
              <div class="text-xs text-[var(--ink-400)] mt-1">
                ...
                {#if ver.sub_graphs?.length}
                  · {ver.sub_graphs.length === 1 ? $t('pages.vocabularyDetail.subGraphCountOne', { values: { count: ver.sub_graphs.length } }) : $t('pages.vocabularyDetail.subGraphCountOther', { values: { count: ver.sub_graphs.length } })}
                {/if}
              </div>
                ...
                <div class="subgraph-list">
                  {#each ver.sub_graphs as sg (sg)}
                    ...
                    <div class="subgraph-row">
                      ...
                      <span class="text-[10px] px-1.5 py-0.5 rounded-full font-medium {statusBadge(sgStatus)}">{sgStatus}</span>
                      {#if isPublisher}
                        ...
                        {/if}
                        {#if sgStatus === 'draft'}
                          <button class="sg-btn" disabled={busy} on:click={() => handleSubgraphAction(ver.version, sg, 'stage')}>{$t('pages.vocabularyDetail.stage')}</button>
                        {/if}
                        {#if sgStatus === 'draft' || sgStatus === 'staged'}
                          <button class="sg-btn sg-btn-publish" disabled={busy} on:click={() => handleSubgraphAction(ver.version, sg, 'publish')}>{$t('pages.vocabularyDetail.publish')}</button>
                        {/if}
                        {#if sgStatus === 'published'}
                          <button class="sg-btn sg-btn-danger" disabled={busy} on:click={() => handleSubgraphAction(ver.version, sg, 'deprecate')}>{$t('pages.vocabularyDetail.deprecate')}</button>
                        {/if}
                      {/if}
                    </div>
                  {/each}
              ...
              {#if browserVersion === ver.version}
                <div class="browser-panel">
                  ...
                  <OntologyBrowserPanel
                    ...
                    rawDataUrl={getVocabularyVersionDataUrl(id, ver.version, 'turtle', 'all')}
                  />
                </div>
              {/if}
            </div>
...
{#if showUpload}
  <UploadVersionDialog
    {id}
    kind="vocabulary"
    uploadFn={uploadVocabularyVersion}
...
{#if showPublishConfirm && publishTarget}
  <PublishConfirmDialog
    ...
    publishFn={publishVocabularyVersion}
...
{#if deleteVersionTarget}
  <ConfirmModal
    title={$t('pages.vocabularyDetail.deleteVersionConfirmTitle', { values: { version: deleteVersionTarget } })}
    message={$t('pages.vocabularyDetail.deleteVersionConfirmMessage')}
    confirmLabel={$t('pages.vocabularyDetail.deleteVersionConfirmLabel')}
...
<style>
  ...
  .ver-btn { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.3rem 0.6rem; font-size: 0.75rem; font-weight: 500; border: 1px solid var(--line-soft, #e2e8f0); border-radius: 7px; background: white; color: var(--ink-600, #475569); cursor: pointer; text-decoration: none; transition: all 0.12s; white-space: nowrap; }
