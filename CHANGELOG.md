...
- **LDES publishing and client sync.** Any dataset can be published as a
  ...
  increments. Domain-neutral; see docs/ldes.md.
...
- **SPARQL federation behind an allowlist.** `SERVICE <endpoint> { … }` works
  again for endpoints whose prefix is listed in `OTS_REMOTE_ALLOWLIST`, with a
  per-call timeout (`OTS_REMOTE_TIMEOUT_SECS`) and row cap
  (`OTS_SERVICE_MAX_ROWS`); anything else still errors (SSRF mitigation), and
  `sd:BasicFederatedQuery` is advertised only when an allowlist is configured.
  The same allowlist governs every outbound request the server makes.
